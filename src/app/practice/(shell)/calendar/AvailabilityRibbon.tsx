          ...
          {c.week.map((d: any) => (
            <li key={d.day}>
              <Link href={`/practice/calendar?date=${d.day}`}
                className={`block rounded-lg border-l-4 border border-gray-100 px-3 py-2 hover:border-[var(--cp-primary)] transition ${
                  d.isChosen ? "border-l-[var(--cp-primary)] bg-[var(--cp-primary)]/5" : "border-l-gray-200"}`}>
                <span className="flex items-baseline gap-2">
                  ...
                  {d.isChosen && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-[var(--cp-primary)]" />}
                </span>
