      ...
      <div className="p-5">
        ...
        {tab === "status" && (
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            {obligations.length === 0 && <p className="sm:col-span-2 text-[11px] text-[var(--cmp-text-warning)]">No obligations registered yet — create one first.</p>}
          </div>
        )}

        <div className="flex items-center gap-2 mt-4">
          <button onClick={act} disabled={busy} className="text-sm font-semibold bg-teal-700 hover:bg-teal-800 text-white rounded-lg px-4 py-2 disabled:opacity-60">{busy ? "Working…" : tab === "create" ? "Register obligation" : "Update status"}</button>
