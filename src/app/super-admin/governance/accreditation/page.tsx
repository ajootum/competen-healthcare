...
const readinessTone = (n: number | null) => (n == null ? "text-gray-500" : n >= 80 ? "text-[var(--cmp-text-success)]" : n >= 50 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-error)]");
const STD_BADGE: Record<string, string> = { met: "bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)]", partially_met: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", not_met: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", not_assessed: "bg-gray-100 text-gray-600" };
    ...
    <div data-wide className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5`}>
          ...
          {d.perFramework.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">No frameworks configured.</p> : (
            <div className="space-y-3">
              {d.perFramework.map((f: any) => (
                <div key={f.id}>
                  ...
                  <p className="text-[9px] text-gray-500 mt-0.5 tabular-nums">{f.assessed}/{f.known} standards assessed{f.assessed ? ` · ${f.met} met · ${f.partially} partial · ${f.notMet} not met` : ""}</p>
                </div>
              ))}
            </div>
          )}
          <p className="text-[10px] text-gray-500 mt-3">Readiness = met 100% · partially 50% · not met 0%, over assessed standards.</p>
        </div>
        ...
        <div className={`${card} p-5 lg:col-span-2`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] text-gray-500">insert-only history · latest wins</span>
          </div>
          {d.recent.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">{d.ready ? "No standards assessed yet — start above." : "Activates with migration 061."}</p> : (
                ...
                <tbody>
                  {d.recent.map((a: any, i: number) => (
                    <tr key={i} className="border-b border-gray-50">
                      ...
                      <td className="px-3 py-2 text-gray-800 tabular-nums">{a.ref}{a.title && <span className="text-[10px] text-gray-500 ml-1.5">{a.title}</span>}</td>
                      <td className="px-3 py-2 text-gray-500 text-[11px] truncate max-w-[220px]">{a.gap ?? "—"}</td>
                      <td className="px-3 py-2 text-center">{a.evidence ? "📎" : <span className="text-gray-500">—</span>}</td>
                      ...
                      <td className="px-3 py-2 text-right text-[11px] text-gray-500">{relTime(a.at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5`}>
          ...
          {d.calendar.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">No dated regulatory or licence obligations.</p> : (
            <div className="space-y-2">
              {d.calendar.map((c: any, i: number) => (
                <div key={i} className="flex items-center gap-2.5 rounded-lg border border-gray-100 p-2.5">
                  ...
                  <div className="min-w-0 flex-1"><p className="text-xs text-gray-700 truncate">{c.title}</p><p className="text-[9px] text-gray-500 capitalize">{c.domain}</p></div>
                  {c.overdue && <span className="text-[9px] font-semibold text-[var(--cmp-text-error)] shrink-0">OVERDUE</span>}
                </div>
              ))}
            </div>
          )}
          <p className="text-[10px] text-gray-500 mt-2">Regulatory and licence obligations from the register (module 3).</p>
        </div>
        ...
        <div className={`${card} p-5`}>
          ...
          <div className="text-center py-2">
            <p className={`text-3xl font-bold tabular-nums ${d.indicators.measured ? readinessTone(Math.round((d.indicators.attained / d.indicators.measured) * 100)) : "text-gray-500"}`}>
          ...
          <p className="text-[10px] text-gray-500 mt-2 pt-2 border-t border-gray-50">Latest EQOS indicator measurements vs their targets ({d.indicators.total} active indicators with targets; {d.indicators.total - d.indicators.measured} unmeasured).</p>
        </div>
        ...
        <div className={`${card} p-5`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] text-gray-500">{d.surveys.completed} completed · {d.surveys.outcomes.passed}✓ {d.surveys.outcomes.conditions}◐ {d.surveys.outcomes.failed}✗</span>
          </div>
          {!d.surveysReady ? (
          ...
          ) : d.surveys.upcoming.length === 0 ? <p className="text-sm text-gray-500 py-4 text-center">No surveys scheduled — plan one above.</p> : (
              ...
              {d.surveys.upcoming.map((s: any) => (
                <div key={s.id} className="flex items-center gap-2.5 rounded-lg border border-gray-100 p-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[9px] text-gray-500 capitalize">{String(s.type).replace(/_/g, " ")}{s.fw ? ` · ${s.fw}` : ""}{s.surveyor ? ` · ${s.surveyor}` : ""}</p>
                  </div>
