...
const STATUS_BADGE: Record<string, string> = { draft: "bg-gray-100 text-gray-600", active: "bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)]", retired: "bg-gray-100 text-gray-600" };
  ...
  return (
    <div data-wide className="space-y-4">
      ...
      <Link href="/super-admin/assistant" className={`${card} px-4 py-3 flex items-center gap-3 hover:border-teal-300 transition-colors`}>
        ...
        <span className="text-sm text-gray-500 flex-1">Search knowledge — natural language, semantic and AI search over {fmt(k.knowledgeObjects + k.cpus)} objects…</span>
        <span className="text-[10px] text-gray-500 border border-gray-200 rounded px-1.5 py-0.5">AI Assistant →</span>
      </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5`}>
          ...
          <p className="text-[10px] text-gray-500 mt-3">Nodes and relationships are live from the knowledge graph; the semantic index powers AI search. Cluster detection activates with the graph analytics job.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5`}>
          ...
          {r.koByType.length === 0 ? <p className="text-sm text-gray-500 py-4 text-center">No knowledge objects yet.</p> : (
        ...
        <div className={`${card} p-5 lg:col-span-2`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] text-gray-500">Most-accessed activates with usage telemetry</span>
          </div>
          {r.recent.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">No knowledge objects yet.</p> : (
            <div className="divide-y divide-gray-50">
              {r.recent.map((o: any, i: number) => (
                <div key={i} className="flex items-center gap-3 py-2">
                  ...
                  <div className="min-w-0 flex-1"><p className="text-sm text-gray-800 truncate">{o.title}</p><p className="text-[10px] text-gray-500 capitalize">{o.type}</p></div>
                  ...
                  <span className="text-[10px] text-gray-500 shrink-0 tabular-nums w-16 text-right">{relTime(o.at)}</span>
                </div>
              ))}
            </div>
          )}
