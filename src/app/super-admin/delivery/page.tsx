...
export default async function DeliveryPlatformPage() {
  ...
  const ORCHESTRATION: Mod[] = [
    { code: "CDP-001", icon: "🧭", label: "Delivery Orchestrator", desc: "Decides which competency/activity to deliver to whom & when, event-triggered. Rules, events & a job runner exist — the event→schedule→deliver loop is the gap.", href: "/competency-office/assignment-rules", status: "partial" },
  ...
  const EXPERIENCE: Mod[] = [
    ...
    { code: "CDP-004", icon: "🔁", label: "Microlearning & Reinforcement", desc: "Spaced repetition, flashcards, retrieval practice, tip-of-the-day. Net-new.", status: "gap" },
  ];
  const PRACTICE: Mod[] = [
