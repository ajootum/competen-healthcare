    ...
    <div className="max-w-4xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Proactive nudges before credentials and competencies expire — once per milestone, straight to the learner.</p>
        </div>
      ...
      {!q.provisioned ? (
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 mb-5">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div>
                ...
                <p className="text-[11px] text-gray-500">Finds credentials & competencies expiring within 30 days (or already expired) and sends each learner a reminder — once per due milestone. Runs daily via the <code className="text-[10px]">learning_reminders</code> cron.</p>
              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-4 py-2.5 border-b border-gray-50"><p className="text-[11px] text-gray-500">Recent reminders</p></div>
            {q.recent.length === 0 ? (
              <p className="text-xs text-gray-500 px-4 py-8 text-center">No reminders sent yet. Run a scan to nudge learners with approaching expiries.</p>
            ) : (
              <div className="divide-y divide-gray-50">
                {q.recent.map((r, idx) => (
                  <div key={idx} className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <span className="text-[10px] text-gray-500 shrink-0">due {r.due_date ?? "—"}</span>
                  </div>
                ))}
              </div>
            )}
