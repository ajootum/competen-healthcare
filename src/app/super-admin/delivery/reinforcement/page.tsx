    ...
    <div className="max-w-5xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Spaced-repetition retrieval practice (SM-2) that keeps achieved competencies from decaying.</p>
        </div>
      ...
      {!q.provisioned ? (
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 mb-5">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div>
                ...
                <p className="text-[11px] text-gray-500">Creates one review card per learner + achieved competency (from competency decisions). Idempotent — existing cards are left alone. Learners review due cards at <code className="text-[10px]">/dashboard/reinforcement</code>.</p>
              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-4 py-2.5 border-b border-gray-50"><p className="text-[11px] text-gray-500">Coverage by competency · {q.subjects.length} subject{q.subjects.length === 1 ? "" : "s"}</p></div>
            {q.subjects.length === 0 ? (
              <p className="text-xs text-gray-500 px-4 py-8 text-center">No cards yet. Generate from achievements to start reinforcing retained competencies.</p>
            ) : (
              <div className="divide-y divide-gray-50">
                {q.subjects.map(s => (
                  <div key={s.subject} className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <span className="text-[11px] text-gray-500 shrink-0">{s.cards} card{s.cards === 1 ? "" : "s"}</span>
