...
import { aisGuard, Head, Tabs, Card, Stat, Pill, Bars, Foot } from "../_ui";
...
export default async function RecommendationsPage() {
  ...
  if (!d.provisioned || !d.hasData) return <div className="max-w-[1500px] space-y-4">{head}<Tabs active="006" /><div className="bg-blue-50 border border-blue-200 rounded-xl p-6 text-sm text-blue-800">No AI recommendation data yet — seed the Administration (ADM-009) and Performance (PA-007) sections; this engine aggregates their real output.</div></div>;
  ...
  return (
    <div className="max-w-[1500px] space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
        ...
        <Stat label="Risks / High Impact" value={k.risks} sub="need attention" tone={k.risks ? "text-rose-600" : undefined} />
        <Stat label="Avg Confidence" value={`${k.avgConfidence}%`} sub="explainable" />
        <Stat label="Est. Benefit" value={money(k.benefit)} sub="if actioned" tone="text-emerald-600" />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <Card title="Confidence Bands">
          <div className="space-y-2 text-[12px]">
            {[["High (≥85%)", d.top.filter((r: any) => r.confidence >= 85).length, "bg-emerald-500"], ["Medium (70–84%)", d.top.filter((r: any) => r.confidence >= 70 && r.confidence < 85).length, "bg-amber-500"], ["Lower (<70%)", d.top.filter((r: any) => r.confidence < 70).length, "bg-gray-400"]].map(([l, n, c]: any) => (
        ...
        <div className="space-y-2">
          {d.top.map((r: any, i: number) => (
            <div key={i} className="flex items-start gap-3 border border-gray-100 rounded-lg p-2.5">
              ...
              <div className="text-right shrink-0"><p className="text-[13px] font-semibold text-gray-800">{r.confidence}%</p>{r.benefit ? <p className="text-[10px] text-emerald-600">{money(r.benefit)}</p> : null}</div>
            </div>
          ))}
        </div>
      </Card>
