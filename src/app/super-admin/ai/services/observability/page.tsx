...
export default async function ObservabilityPage() {
  ...
  return (
    <div className="max-w-[1500px] space-y-4">
      ...
      <Card title="Request & Cost Trend" right={<span className="text-[11px] text-gray-400">last 7 days</span>}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Card title="By Model">{d.byModel.length ? <Bars rows={d.byModel.map((m: any) => ({ label: m.label, n: m.n, extra: `${m.n} · $${m.cost}` }))} /> : <p className="text-sm text-gray-400 py-4 text-center">No data.</p>}</Card>
        <Card title="By Operation">{d.byOperation.length ? <Bars rows={d.byOperation.map((o: any) => ({ label: o.label, n: o.n, extra: `${o.tokens.toLocaleString()}t` }))} /> : <p className="text-sm text-gray-400 py-4 text-center">No data.</p>}</Card>
        <Card title="By Tier">{d.byTier.length ? <Bars rows={d.byTier.map((t: any) => ({ label: t.label, n: t.n, extra: `$${t.cost}` }))} /> : <p className="text-sm text-gray-400 py-4 text-center">No data.</p>}</Card>
      </div>

      <Card title="Recent Requests" right={<span className="text-[11px] text-gray-400">latest {d.recent.length}</span>}>
        {d.recent.length ? (
          <div className="overflow-x-auto"><div className="min-w-[760px] space-y-1">
            <div className="flex items-center text-[10px] text-gray-400 uppercase tracking-wide px-2"><span className="w-24">Operation</span><span className="flex-1">Model</span><span className="w-20">Tier</span><span className="w-20 text-right">Tokens</span><span className="w-20 text-right">Latency</span><span className="w-20 text-right">Cost</span><span className="w-20 text-right">Status</span></div>
