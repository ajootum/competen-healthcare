  ...
  return (
    <div className="max-w-[1500px] space-y-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Card title="Agent Registry" className="xl:col-span-2" right={<span className="text-[11px] text-gray-500">by runs</span>}>
            ...
            {d.agents.map((a: any) => (
              <div key={a.id} className="border border-gray-100 rounded-lg p-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="text-right shrink-0"><p className="text-sm font-bold text-gray-900 tabular-nums">{Number(a.runs).toLocaleString()}</p><p className="text-[10px] text-gray-500">runs</p></div>
                </div>
                <div className="flex items-center gap-2 mt-1.5 flex-wrap"><span className="text-[10px] text-gray-500 font-mono">{a.model_id}</span><span className="text-gray-500">·</span>{(a.skills ?? []).map((s: string) => <span key={s} className="text-[9px] bg-gray-50 border border-gray-100 rounded px-1 py-0.5 text-gray-500 font-mono">{s}</span>)}</div>
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card title="Autonomy Distribution">
          ...
          <p className="text-[10px] text-gray-500 mt-3">assist = answers only · suggest = proposes actions for approval · act = executes governed skills (with confirmation for high-impact). No agent bypasses permissions.</p>
        </Card>
      </div>
