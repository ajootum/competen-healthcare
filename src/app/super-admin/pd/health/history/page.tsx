...
export default async function Page() {
  ...
  return (
    <PdNotBuilt
      ...
      willShow="past degradations and their correlation with releases and configuration changes."
      absence="No reliability instrumentation exists for Competen Practice. Availability, Apdex, P95 latency and error rate are recorded nowhere, so a health state computed from them would be invented rather than observed. Three adjacent signals ARE real and can be built on when this module is: AI request latency, job-run durations and sync-transaction age. Nothing was recorded historically either, so there is no back-fill to draw on."
    />
  );
}
