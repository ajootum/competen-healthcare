...
const BADGE: Record<string, string> = { draft: "bg-gray-100 text-gray-600", onboarding: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", active: "bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)]", suspended: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", archived: "bg-gray-100 text-gray-600" };
    ...
    <div data-wide className="space-y-4">
      ...
      {tab === "Overview" && (
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <div className={card}><h3 className="font-semibold text-gray-900 mb-2">Leadership</h3>
            ...
            <p className="text-[11px] text-gray-500 mt-3">Medical director, nursing director, quality lead, HR and IT contacts are captured when the facility leadership module is provisioned.</p>
          </div>
        </div>
      )}
      ...
      {tab === "Departments" && (
        <div className={card}>
          ...
          {departments.length === 0 ? <p className="text-sm text-gray-500">No departments yet. Build them in the structure builder.</p> : (
            <div className="divide-y divide-gray-50">
              {departments.map((d: any) => (
                <div key={d.id} className="flex items-center gap-3 py-2.5">
                  <span className="text-base">🗂️</span>
                  <div className="flex-1 min-w-0"><p className="text-sm text-gray-800 truncate">{d.name}</p><p className="text-[10px] text-gray-500">{[d.code, d.type, d.head ? `Head: ${d.head}` : null].filter(Boolean).join(" · ") || "—"}</p></div>
                  <span className={`text-[10px] px-2 py-0.5 rounded ${d.status === "archived" ? "bg-gray-100 text-gray-600" : "bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)]"}`}>{d.status}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {tab === "People" && (
        <div className={card}>
          ...
          {users.length === 0 ? <p className="text-sm text-gray-500">No people assigned to this facility.</p> : (
      ...
      {tab === "Governance" && (
        <div className={card}><h3 className="font-semibold text-gray-900 mb-2">Governance &amp; operations</h3>
          ...
          <p className="text-[11px] text-gray-500 mt-2">Facility-level policies, workspaces, integrations, operating hours and accreditation status are provisioned with the facility configuration module.</p>
        </div>
      )}

      {tab === "Audit" && (
        <div className={card}><h3 className="font-semibold text-gray-900 mb-3">Audit history</h3>
          {!auditReady || audit.length === 0 ? <p className="text-sm text-gray-500">{auditReady ? "No audit entries for this facility yet." : "Audit log not available."}</p> : (
