...
export default async function AiOrchestratorPage() {
  ...
  return (
    <div className="max-w-[1400px] space-y-4">
      ...
      <Card title="Orchestration Insights" right={<span className="text-[11px] text-gray-500">rule-based · explainable</span>}>
        {d.recs.length ? <div className="space-y-2">{d.recs.map((r: any, i: number) => (
          <div key={i} className="flex items-start gap-3 border border-gray-100 rounded-lg p-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] text-gray-500 mt-0.5">↳ basis: {r.source}</p>
            </div>
          </div>
        ))}</div> : <p className="text-sm text-gray-500 py-8 text-center">No orchestration insights — the strategic model is healthy. ✅</p>}
      </Card>
