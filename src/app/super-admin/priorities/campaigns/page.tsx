  ...
  return (
    <div className="max-w-[1400px] space-y-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Card title="Initiative Portfolio" className="xl:col-span-2" right={<span className="text-[11px] text-gray-500">{d.campaigns.length} campaigns</span>}>
            ...
            {d.campaigns.map((c: any) => (
              <div key={c.id} className="border border-gray-100 rounded-lg p-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="text-right shrink-0"><p className="text-sm font-bold text-gray-900 tabular-nums">{Math.round(Number(c.progress_pct))}%</p><p className="text-[10px] text-gray-500">{c.budget ? money(Number(c.budget)) : "—"}</p></div>
                </div>
                <div className="mt-2"><Progress pct={Number(c.progress_pct)} /></div>
                <div className="flex items-center gap-3 mt-1.5 text-[10px] text-gray-500"><span>{c.actionCount} actions</span>{c.daysLeft != null && <span className={c.daysLeft < 0 ? "text-rose-500" : c.daysLeft < 30 ? "text-amber-500" : ""}>{c.daysLeft < 0 ? `${-c.daysLeft}d overdue` : `${c.daysLeft}d left`}</span>}</div>
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card title="Campaigns by Theme">
          ...
          ))}</div> : <p className="text-sm text-gray-500 py-4 text-center">No campaigns.</p>}
        </Card>
      </div>
