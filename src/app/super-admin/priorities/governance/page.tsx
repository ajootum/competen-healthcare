  ...
  return (
    <div className="max-w-[1400px] space-y-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <Card title="Approval Queue" right={<span className="text-[11px] text-gray-500">{d.queue.length} open</span>}>
          {d.queue.length ? <div className="space-y-2">{d.queue.map((a: any) => (
            <div key={a.id} className="border border-gray-100 rounded-lg p-2.5">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0"><p className="text-[12px] font-medium text-gray-900 leading-tight">{a.entity_title}</p><p className="text-[10px] text-gray-500 capitalize">{a.entity_type} · {a.scopeLabel} · {a.workflow} workflow</p></div>
                <Pill text={a.state} tone={STATUS_TONE[a.state]} />
              </div>
              <div className="flex items-center justify-between mt-1.5 text-[10px] text-gray-500"><span>by {a.requester} · step {a.step}/{a.total_steps}</span>{a.decision_reason && <span className="text-[var(--cmp-text-warning)] truncate max-w-[55%]">“{a.decision_reason}”</span>}</div>
              {a.state === "pending" && <ApprovalDecisions id={a.id} />}
            </div>
          ))}</div> : <p className="text-sm text-gray-500 py-6 text-center">Queue clear. ✅</p>}
        ...
        <Card title="Recent Decisions">
          {d.decided.length ? <div className="space-y-2">{d.decided.map((a: any) => (
            <div key={a.id} className="flex items-start gap-2"><Pill text={a.state} tone={STATUS_TONE[a.state]} /><div className="min-w-0 flex-1"><p className="text-[12px] text-gray-800 leading-tight truncate">{a.entity_title}</p><p className="text-[10px] text-gray-500">{a.decider ? `by ${a.decider}` : ""} {a.decided_at ? `· ${fmtDate(a.decided_at)}` : ""}{a.decision_reason ? ` · ${a.decision_reason}` : ""}</p></div></div>
          ))}</div> : <p className="text-sm text-gray-500 py-6 text-center">No decisions yet.</p>}
        </Card>
      </div>

      <Card title="Audit Trail" right={<span className="text-[11px] text-gray-500">immutable · {d.audit.length} entries</span>}>
        {d.audit.length ? <div className="space-y-1.5 max-h-80 overflow-y-auto">{d.audit.map((a: any) => (
          <div key={a.id} className="flex items-start gap-2 text-[11px]"><Pill text={a.action} tone="teal" /><span className="text-gray-700 flex-1">{a.detail}</span><span className="text-gray-500 shrink-0">{a.actor} · {fmtDate(a.created_at)}</span></div>
        ))}</div> : <p className="text-sm text-gray-500 py-4 text-center">No audit entries.</p>}
      </Card>
