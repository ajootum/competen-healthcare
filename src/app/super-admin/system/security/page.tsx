...
const postureTone = (n: number | null) => (n == null ? "text-gray-500" : n >= 85 ? "text-[var(--cmp-text-success)]" : n >= 60 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-error)]");
...
const THREAT_TONE: Record<string, string> = { Low: "bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)]", Elevated: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", High: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", Unknown: "bg-gray-100 text-gray-600" };
  ...
  return (
    <div data-wide className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-2">
        ...
        <span className={`text-xs font-semibold px-2.5 py-1 rounded-lg ${THREAT_TONE[k.threatLevel] ?? "bg-gray-100 text-gray-600"}`}>Threat level: {k.threatLevel}</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5 lg:col-span-2`}>
          ...
          {d.incidents.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">✅ No active security incidents.</p> : (
              ...
              {d.incidents.map((i: any) => (
                <div key={i.id} className="flex items-center gap-3 py-2.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-gray-500 capitalize">{String(i.kind).replace(/_/g, " ")} · {relTime(i.at)}</p>
                  </div>
        ...
        <div className={`${card} p-5`}>
          <h2 className="font-semibold text-gray-900 text-[15px] mb-3">Event Categories <span className="text-[10px] text-gray-500">24h</span></h2>
          {d.categories.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">No security events in 24h.</p> : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5 lg:col-span-2`}>
          <h2 className="font-semibold text-gray-900 text-[15px] mb-3">Security Event Trend <span className="text-[10px] text-gray-500">last 7 days · from audit trail</span></h2>
        ...
        <div className={`${card} p-5`}>
          ...
          {!d.risksReady ? <p className="text-xs text-gray-500 py-4 text-center">Risk register not enabled (migration 060).</p> : d.vulnerabilities.length === 0 ? <p className="text-sm text-gray-500 py-4 text-center">No open cybersecurity risks.</p> : (
            <div className="space-y-2">
              {d.vulnerabilities.map((v: any) => (
                <div key={v.id} className="flex items-center gap-2.5 rounded-lg border border-gray-100 p-2.5">
                  ...
                  <span className="text-[9px] text-gray-500 capitalize shrink-0">{String(v.status).replace(/_/g, " ")}</span>
                </div>
              ))}
            </div>
          )}
          <p className="text-[10px] text-gray-500 mt-3">Sourced from the cybersecurity-category risk register. CVE scanning &amp; patch status are a later phase.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5 lg:col-span-2`}>
          ...
          {d.recent.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">No security events.</p> : (
            <div className="divide-y divide-gray-50">
              {d.recent.map((e: any, i: number) => (
                <div key={i} className="flex items-center gap-3 py-1.5">
                  ...
                  <span className="text-[10px] text-gray-500 shrink-0 w-14 text-right">{relTime(e.at)}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className={`${card} p-5`}>
          ...
          <div className="space-y-1.5 pt-2 border-t border-gray-50">
            {d.notMonitored.map((l: string) => (
              <div key={l} className="flex items-center justify-between text-xs"><span className="text-gray-500">{l}</span><span className="text-[10px] font-medium text-gray-500">not monitored</span></div>
            ))}
          </div>
          <p className="text-[10px] text-gray-500 mt-3">Login geography and MFA are handled by the auth provider; threat feeds and SIEM integration are a later phase — shown honestly rather than faked.</p>
        </div>
      </div>

      <p className="text-[11px] text-gray-500 pb-4">The SOC runs on real signals: the security-relevant slice of the immutable audit trail (classified into categories and trended), live incidents from escalations and safety alerts, the cybersecurity risk register as the exposure view, and a posture score computed only from measurable facts. Containment is a real, human-approved, reversible auth ban. Threat mapping, intrusion detection, geolocation and CVE feeds show honest “not monitored” states until those data sources exist (SYS-002 AC-02).</p>
    </div>
  );
}
