    ...
    <div className="max-w-[1400px]">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">What governance rules apply, and how well the competency portfolio complies — the enforced ruleset made explicit, with risk-tiered policy and the real configured thresholds.</p>
        </div>
      ...
      {!d.provisioned ? (
        <div className="bg-white border border-gray-200 rounded-xl p-8 text-center"><p className="text-sm text-gray-500">No governance rules or configuration to evaluate yet — once competencies, blueprints and evidence matrices exist, the ruleset and compliance compute here.</p></div>
      ) : (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-gray-500">evaluated over {d.n} competencies</span>
            </div>
            {d.rules.length === 0 ? (
              <p className="text-[12px] text-gray-500">The governance registry is empty — rule compliance needs competency definitions. Configured thresholds still show below.</p>
            ) : (
              <div className="space-y-2.5">
                {d.rules.map((r: any) => (
                  <div key={r.name} className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] text-gray-500 tabular-nums w-16 text-right shrink-0">{r.met}/{r.total}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              ...
              <p className="text-[10px] text-gray-500">stricter governance for higher risk</p>
            </div>
            {d.tiers.length === 0 ? (
              <div className="p-6 text-center"><p className="text-[12px] text-gray-500">No competencies to tier.</p></div>
            ) : (
          ...
          <div>
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Configured governance thresholds — the real stored rules</p>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {/* Review intervals */}
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                <p className="text-[11px] font-semibold text-gray-500 mb-3">⏱️ Review intervals by risk</p>
                {d.reviewByRisk.length === 0 ? (
                  <p className="text-[12px] text-gray-500">No CPU review intervals configured.</p>
                ) : (
                  <div className="space-y-2">
                    {d.reviewByRisk.map((rv: any) => (
                      <div key={rv.risk} className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-[12px] text-gray-700"><span className="font-bold tabular-nums">{rv.avgMonths}</span> mo <span className="text-[10px] text-gray-500">· {rv.count} CPU{rv.count === 1 ? "" : "s"}</span></span>
                      </div>
                    ))}
                    <p className="text-[10px] text-gray-500 pt-1">Reassessment cadence — shorter for higher risk.</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                ...
                {d.approvalRules.count === 0 ? (
                  <p className="text-[12px] text-gray-500">No assessment blueprints configured.</p>
                ) : (
                  <>
                    ...
                    <p className="text-[10px] font-semibold text-gray-500 uppercase mb-1">Consensus rule</p>
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                ...
                {d.evidenceRules.count === 0 ? (
                  <p className="text-[12px] text-gray-500">No evidence matrix rules configured.</p>
                ) : (
          ...
          <p className="text-[11px] text-gray-500 leading-relaxed">Every rule and threshold here is real: compliance rates are evaluated live over the governance registry, and the configured thresholds are the actual stored governance config — reassessment cadence (CPU reassessment intervals), approval rules (assessment blueprints) and evidence requirements (evidence matrix). Authoring and versioning rules/policies happens in <Link href="/super-admin/policy-manager" className="text-[var(--cmp-text-success)] hover:underline">the policy manager</Link> and <Link href="/super-admin/studio/rules" className="text-[var(--cmp-text-success)] hover:underline">the rules engine</Link>, with rule <Link href="/super-admin/studio/testing" className="text-[var(--cmp-text-success)] hover:underline">simulation &amp; testing</Link> before release. Per the CGR mandate, AI may recommend rule improvements but never creates binding rules or overrides governance authority.</p>
        </div>
      )}
    </div>
  );
