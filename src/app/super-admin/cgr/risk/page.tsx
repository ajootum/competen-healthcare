        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              ...
              <p className="text-[10px] text-gray-400">{ex.active} active · {ex.expired} expired · {ex.revoked} revoked · <Link href="/super-admin/system/audit" className="text-[var(--cmp-text-success)] hover:underline">admin →</Link></p>
            </div>
            {!ex.ready ? (
                ...
                {ex.list.map((g: any, i: number) => (
                  <div key={i} className="flex items-start justify-between gap-3 px-4 py-2.5">
                    <div className="min-w-0">
                      <p className="text-[12px] text-gray-700 leading-tight"><span className="font-semibold">{g.actor}</span> <span className="text-gray-400">· {g.target}</span> <span className={`ml-1 text-[9px] font-bold uppercase rounded px-1 ${g.scope === "act" ? "text-[var(--cmp-text-error)] bg-[var(--cmp-surface-error)]" : "text-gray-500 bg-gray-50"}`}>{g.scope}</span></p>
