    ...
    <div className="max-w-[1400px]">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Are competency systems improving clinical practice? Two lenses: the statistical correlation, and the case-confirmed link between real safety events and specific competencies.</p>
        </div>
      ...
      {!d.provisioned ? (
        <div className="bg-white border border-gray-200 rounded-xl p-8 text-center"><p className="text-sm text-gray-500">No safety events, learning links or governed competencies yet — clinical practice intelligence computes once those exist.</p></div>
      ) : (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              ...
              {!d.corr ? (
                <p className="text-[12px] text-gray-500">Not enough department data to correlate competency with outcomes — the correlation engine will say so rather than overclaim.</p>
              ) : (
                <>
                  ...
                  <p className="text-[10px] text-gray-500 mt-2">Ecological, across {d.corr.departments} departments — directional, never causal. Owned by <Link href="/super-admin/performance/correlation" className="text-[var(--cmp-text-success)] hover:underline">Competency Performance</Link>.</p>
                </>
              )}
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              <p className="text-sm font-bold text-gray-800">Competencies implicated by safety events <span className="text-[10px] font-normal text-gray-500">— aggregated by competency, never by person (§4.4)</span></p>
              <p className="text-[10px] text-gray-500">evidence first · <Link href="/super-admin/cgr/learning" className="text-[var(--cmp-text-success)] hover:underline">manage links →</Link></p>
            </div>
            {d.implicated.length === 0 ? (
              <div className="p-6 text-center"><p className="text-sm text-gray-500">No safety events have been linked to competencies yet. Propose links in <Link href="/super-admin/cgr/learning" className="text-[var(--cmp-text-success)] hover:underline">Organisational Learning</Link> — once governance confirms them, this becomes the case-level evidence that specific competencies need attention.</p></div>
            ) : (
                  ...
                  <tbody>
                    {d.implicated.map((x: any) => (
                      <tr key={x.id} className="border-t border-gray-50">
                        ...
                        <td className="py-2 px-2">{x.risk ? <span className={`text-[10px] font-bold border rounded px-1.5 py-0.5 capitalize ${RISK_META[x.risk] ?? RISK_META.standard}`}>{x.risk}</span> : <span className="text-[10px] text-gray-500">—</span>}</td>
                        <td className="py-2 px-2 text-center text-[12px] text-gray-700 tabular-nums">{x.events}</td>
                        <td className="py-2 px-2 text-center text-[11px] tabular-nums"><span className={x.highCritical ? "text-[var(--cmp-text-error)] font-semibold" : "text-gray-500"}>{x.highCritical}</span></td>
                        <td className="py-2 px-2 text-center text-[11px] text-gray-500 tabular-nums">{x.nearMiss}</td>
                        <td className="py-2 px-2 text-center text-[11px] tabular-nums"><span className={x.confirmed + x.implemented ? "text-[var(--cmp-text-success)] font-semibold" : "text-gray-500"}>{x.confirmed + x.implemented}</span></td>
                        <td className="py-2 px-2 text-center text-[11px] tabular-nums"><span className={x.proposed ? "text-[var(--cmp-text-warning)]" : "text-gray-500"}>{x.proposed}{x.byAi > 0 && <span className="text-[8px] font-bold text-violet-600 ml-0.5">AI</span>}</span></td>
                        <td className="py-2 px-2 text-center">{x.govScore != null ? <span className={`text-[11px] font-bold tabular-nums ${tone(x.govScore)}`}>{x.govScore}</span> : <span className="text-[10px] text-gray-500">—</span>}</td>
                        <td className="py-2 pr-4 pl-2">{x.practiceRisk ? <span className="text-[10px] font-bold text-[var(--cmp-text-error)] bg-[var(--cmp-surface-error)] border border-[var(--cmp-color-error)] rounded px-1.5 py-0.5">practice risk</span> : <span className="text-[10px] text-gray-500">—</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                ...
                <p className="text-[10px] text-gray-500">{d.variationSpread != null ? `event spread ${d.variationSpread}` : "needs ≥2 units with events"}</p>
              </div>
              {d.variation.length === 0 ? (
                <div className="p-6 text-center"><p className="text-[12px] text-gray-500">No unit-attributable events or team states yet.</p></div>
              ) : (
                <div className="overflow-x-auto">
                    ...
                    <tbody>
                      {d.variation.map((v: any) => (
                        <tr key={v.name} className="border-t border-gray-50">
                          ...
                          <td className="py-2 px-2 text-center text-[11px] tabular-nums"><span className={v.highCritical ? "text-[var(--cmp-text-error)] font-semibold" : "text-gray-500"}>{v.highCritical}</span></td>
                          <td className="py-2 px-2 text-center text-[11px] text-gray-500 tabular-nums">{v.nearMiss}</td>
                          <td className="py-2 pr-4 pl-2 text-center">{v.twinState != null ? <span className={`text-[12px] font-bold tabular-nums ${tone(v.twinState)}`}>{v.twinState}</span> : <span className="text-[10px] text-gray-500">no twin</span>}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              ...
              {!d.mm.ready ? (
                <p className="text-[12px] text-gray-500">M&amp;M register not available.</p>
              ) : d.mm.total === 0 ? (
                <p className="text-[12px] text-gray-500">No M&amp;M cases recorded yet.</p>
              ) : (
                <div className="grid grid-cols-2 gap-2">
