...
const STATUS_META: Record<string, { label: string; cls: string }> = {
  proposed: { label: "Proposed", cls: "text-[var(--cmp-text-warning)] bg-[var(--cmp-surface-warning)] border-[var(--cmp-color-warning)]" },
  confirmed: { label: "Confirmed", cls: "text-blue-700 bg-[var(--cmp-surface-information)] border-[var(--cmp-color-information)]" },
      ...
      {!d.provisioned ? (
        ...
        <div className="space-y-4">
            ...
            {!L.ready ? (
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 mb-3">
                  <Kpi label="Loops closed" value={L.byStatus.implemented} sub="change implemented" tone={L.byStatus.implemented ? "text-[var(--cmp-text-success)]" : "text-gray-900"} />
                  <Kpi label="Closure rate" value={L.closureRate == null ? "—" : `${L.closureRate}%`} sub="of non-rejected links" tone={L.closureRate == null ? "text-gray-900" : L.closureRate >= 70 ? "text-[var(--cmp-text-success)]" : "text-[var(--cmp-text-warning)]"} />
                  <Kpi label="Time to improvement" value={L.causalMedian == null ? "—" : `${L.causalMedian}d`} sub={L.causalAvg == null ? "measured, signal→change" : `median · avg ${L.causalAvg}d`} tone="text-[var(--cmp-text-success)]" />
                  <Kpi label="Awaiting review" value={L.awaitingReview} sub="proposed links" tone={L.awaitingReview ? "text-[var(--cmp-text-warning)]" : "text-gray-900"} />
                  <Kpi label="Links asserted" value={L.total} sub={`${L.byStatus.confirmed} confirmed`} />
                  <Kpi label="Signal coverage" value={L.coverage == null ? "—" : `${L.coverage}%`} sub="events with a link" tone={L.coverage != null && L.coverage < 25 ? "text-[var(--cmp-text-warning)]" : "text-gray-900"} />
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
            ...
            <Kpi label="Action recorded" value={k.conversion == null ? "—" : `${k.conversion}%`} sub={`${k.withAction} with corrective action`} tone={k.conversion == null ? "text-gray-900" : k.conversion >= 70 ? "text-[var(--cmp-text-success)]" : k.conversion >= 40 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-error)]"} />
            ...
            <Kpi label="Open signals" value={k.open} sub={`${k.openNoAction} with no action`} tone={k.openNoAction ? "text-[var(--cmp-text-error)]" : "text-gray-900"} />
            <Kpi label="Recurring types" value={k.recurringTypes} sub="repeat across periods" tone={k.recurringTypes ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-success)]"} />
          ...
          <p className="text-[11px] text-gray-400 leading-relaxed">
            ...
            Signal-to-competency-domain correlation is owned by <Link href="/competency-office/quality-feedback" className="text-[var(--cmp-text-success)] hover:underline">the Quality → Competency Feedback Loop (COMP-028)</Link>, and remediation by <Link href="/competency-office/remediation" className="text-[var(--cmp-text-success)] hover:underline">the Remediation Centre</Link>. Per the CGR mandate, AI may propose links and identify themes but never confirms causation or creates mandatory competencies without governance review.
          </p>
        </div>
      )}
    </div>
