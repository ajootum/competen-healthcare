    ...
    <div className="max-w-6xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">How competencies map across assessment, evidence, learning, skills and CPUs — coverage and gaps, computed live.</p>
        </div>
      ...
      {!map.provisioned ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-8 text-center text-sm text-gray-500">No competencies to map yet — author frameworks and competencies in the Studio first.</div>
      ) : (
        <>
          {/* Dimension coverage */}
          <h2 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Mapping dimensions · {map.total} competencies</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
            {map.dimensions.map(d => (
              <div key={d.key} className={`bg-white rounded-xl border border-gray-100 p-4 ${d.pending ? "opacity-70" : ""}`}>
                ...
                <p className="text-[10px] text-gray-500">{d.pending ? "No store yet — standards mapping is CST-108" : `${d.links} link${d.links === 1 ? "" : "s"}`}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 mb-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[9px] text-gray-500">% of each framework&apos;s competencies traced to the dimension · lowest first</span>
            </div>
                ...
                <tbody>
                  {map.matrix.map(m => (
                    <tr key={m.id} className="border-t border-gray-50">
                      ...
                      <td className="py-1.5 px-2 text-center text-gray-500">{m.competencies}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[9px] text-gray-500">competencies mapped to the fewest object types</span>
            </div>
            {map.gaps.length === 0 ? (
              <p className="text-xs text-gray-500">Every competency is traced across all dimensions. 🎉</p>
            ) : (
              <div className="flex flex-col divide-y divide-gray-50">
                {map.gaps.map((g, i) => (
                  <div key={i} className="flex items-center gap-3 py-2 text-xs">
                    ...
                    {g.framework && <span className="text-[10px] text-gray-500 truncate hidden md:inline">{g.framework}</span>}
