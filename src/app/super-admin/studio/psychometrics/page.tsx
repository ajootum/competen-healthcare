    ...
    <div className="max-w-6xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Item difficulty, discrimination, distractor effectiveness and reliability — computed live from real attempts.</p>
        </div>
      ...
      {!overview.provisioned ? (
        <div className="bg-white rounded-xl border border-gray-100 p-8 text-center text-sm text-gray-400">Question bank tables are not available in this environment.</div>
      ) : overview.banks.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 p-8 text-center text-sm text-gray-400">No question banks yet — author one in the <Link href="/super-admin/studio/questions" className="text-teal-600 hover:underline">Question Builder</Link>.</div>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
            <div className="bg-white rounded-xl border border-gray-100 p-3.5"><p className="text-xl font-bold text-gray-900">{overview.totalBanks}</p><p className="text-[10px] text-gray-400 font-medium mt-0.5">Question banks</p></div>
            <div className="bg-white rounded-xl border border-gray-100 p-3.5"><p className="text-xl font-bold text-teal-600">{overview.analysable}</p><p className="text-[10px] text-gray-400 font-medium mt-0.5">Analysable (≥{MIN_ATTEMPTS} attempts)</p></div>
            {detail?.found && !detail.insufficient && detail.reliability && (
              <>
                <div className="bg-white rounded-xl border border-gray-100 p-3.5"><p className={`text-xl font-bold ${relTone(detail.reliability.kr20)}`}>{detail.reliability.kr20 ?? "—"}</p><p className="text-[10px] text-gray-400 font-medium mt-0.5">KR-20 (this bank)</p></div>
                <div className="bg-white rounded-xl border border-gray-100 p-3.5"><p className="text-xl font-bold text-gray-900">{detail.reliability.sem ?? "—"}</p><p className="text-[10px] text-gray-400 font-medium mt-0.5">Std error (SEM)</p></div>
              </>
            )}
          </div>
          ...
          <div className="grid lg:grid-cols-[260px_1fr] gap-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-3 h-fit">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1 mb-2">Question banks</p>
              <div className="flex flex-col gap-0.5 max-h-[560px] overflow-y-auto">
                {overview.banks.map(b => (
                  <Link key={b.id} href={`/super-admin/studio/psychometrics?bank=${b.id}`}
                    ...
                    <p className="text-[10px] text-gray-400">{b.attempts} attempt{b.attempts === 1 ? "" : "s"}{b.attempts >= MIN_ATTEMPTS ? ` · ${b.meanScore}% mean · ${b.passRate}% pass` : ""}</p>
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="min-w-0">
              {!detail?.found ? (
                <div className="bg-white rounded-xl border border-gray-100 p-8 text-center text-sm text-gray-400">Select a bank.</div>
              ) : detail.insufficient ? (
                <div className="bg-white rounded-xl border border-gray-100 p-8 text-center">
                  <p className="text-sm text-gray-500 font-medium">{detail.bank.name}</p>
                  <p className="text-xs text-gray-400 mt-1">{detail.attempts} attempt{detail.attempts === 1 ? "" : "s"} — need at least {MIN_ATTEMPTS} for reliable item statistics.</p>
                  <p className="text-[11px] text-gray-400 mt-2">Psychometrics require a candidate sample; this bank hasn&apos;t been taken enough yet.</p>
                </div>
              ) : (
                <>
                  <div className="bg-white rounded-xl border border-gray-100 p-4 mb-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-[10px] text-gray-400">{detail.attempts} attempts · {detail.reliability!.items} items</span>
                    </div>
                  ...
                  <div className="bg-white rounded-xl border border-gray-100 p-4">
                    ...
                    <p className="text-[10px] text-gray-400 mt-3">Difficulty p = proportion correct (0.4–0.8 optimal). Discrimination = point-biserial (≥0.3 good, &lt;0.2 review). Computed from real attempts using the same grading as delivery.</p>
                  </div>
                </>
              )}
            </div>
