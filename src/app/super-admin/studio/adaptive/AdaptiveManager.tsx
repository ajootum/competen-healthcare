    ...
    <div className="flex flex-col gap-4">
      <div className="bg-white rounded-xl border border-gray-100 p-5">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <label className="text-[10px] text-gray-500 ml-2">Start</label>
      ...
      {exams.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 p-6 text-center text-xs text-gray-500">No adaptive exams yet — create a blueprint over a question-bank item pool.</div>
      ) : (
          ...
          <div className="flex flex-col divide-y divide-gray-50">
            {exams.map((e: any) => (
              <div key={e.id} className="flex items-center gap-2 py-2.5 text-xs flex-wrap">
                ...
                {e.bankName ? <span className="text-[10px] text-gray-500 truncate max-w-[22%]">📚 {e.bankName} ({e.poolSize})</span> : <span className="text-[10px] text-gray-500">no pool</span>}
                ...
                <div className="ml-auto flex items-center gap-1.5">
                  ...
                  {e.status === "active" ? <button onClick={() => setStatus(e.id, "archived")} disabled={busy} className="text-[10px] font-semibold text-gray-500 hover:underline">Archive</button>
                  ...
                  <button onClick={() => del(e.id)} disabled={busy} className="text-gray-500 hover:text-red-500" title="Delete">✕</button>
                </div>
              </div>
            ))}
          </div>
