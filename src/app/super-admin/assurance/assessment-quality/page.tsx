...
const dTone = (d: number | null) => (d == null ? "text-gray-500" : d < 0 ? "text-[var(--cmp-text-error)]" : d < 0.15 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-success)]");
    ...
    <div className="max-w-5xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Classical item analysis — how hard each question is and how well it separates strong from weak candidates — over the real attempt record.</p>
        </div>
      ...
      {!q.provisioned ? (
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-6"><p className="text-sm text-gray-500">No scored question attempts yet. Once learners answer questions, item difficulty and discrimination populate here automatically.</p></div>
      ) : (
        <>
          ...
          <div className={`${card} overflow-hidden mb-5`}>
            <div className="px-4 py-2.5 border-b border-gray-50 flex items-center justify-between"><p className="text-[11px] font-semibold text-gray-500">Item review queue</p><span className="text-[10px] text-gray-500">items too easy/hard or discriminating poorly</span></div>
            {q.flagged.length === 0 ? (
              <p className="text-xs text-gray-500 px-4 py-6 text-center">No items flagged — the question bank is performing well. 🎯</p>
            ) : (
              <div className="divide-y divide-gray-50">
                {q.flagged.map((it: any) => (
                  <div key={it.id} className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <span className="text-[10px] text-gray-500 shrink-0 hidden sm:inline">{it.category}</span>
                    ...
                    <span className="text-[11px] text-gray-500 tabular-nums shrink-0 w-12 text-right">{it.attempts}×</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className={`${card} p-4`}>
              ...
              {q.byCategory.length === 0 ? <p className="text-xs text-gray-500">No categories.</p> : (
                <div className="space-y-2.5">
                  {q.byCategory.map((c: any) => (
                    <div key={c.category}>
                      <div className="flex items-center justify-between text-[12px] mb-0.5"><span className="text-gray-700 truncate pr-2">{c.category}</span><span className="text-gray-500 tabular-nums shrink-0">{c.items}</span></div>
                      <div className="flex items-center gap-2 text-[10px] text-gray-500"><span>diff {c.avgDifficulty}</span><span>·</span><span className={c.avgDiscrimination != null ? dTone(c.avgDiscrimination) : ""}>disc {c.avgDiscrimination ?? "—"}</span></div>
                    </div>
                  ))}
                </div>
              )}
              <p className="text-[10px] text-gray-500 mt-4 leading-relaxed border-t border-gray-50 pt-3">Difficulty (p-value) = share of attempts correct; ideal ≈ 0.3–0.85. Discrimination = strong-candidate correctness minus weak-candidate correctness (≥0.3 good, &lt;0.15 weak, &lt;0 flag). Decision-support for item review, not an automated edit.</p>
            </div>
          </div>
        </>
      )}
