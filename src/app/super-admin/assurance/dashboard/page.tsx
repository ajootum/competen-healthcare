    ...
    <div className="max-w-5xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">One enterprise assurance score — how much the competency system as a whole can be trusted right now — consolidated from every live assurance signal.</p>
        </div>
      ...
      {!q.provisioned ? (
        <div className="bg-white border border-gray-100 rounded-xl p-6"><p className="text-sm text-gray-500">No assurance signals yet. Once competency decisions, assessments and corrective actions exist, the assurance score consolidates them here automatically.</p></div>
      ) : (
        <>
          {/* Assurance score + headline */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div className={`${card} p-5 flex flex-col items-center justify-center text-center`}>
              <p className="text-[11px] text-gray-500 uppercase tracking-wide mb-1">Assurance score</p>
              ...
              <p className="text-[10px] text-gray-500 mt-1">mean of {q.scoredCount} assured domains</p>
            </div>
            <div className="md:col-span-2 grid grid-cols-2 gap-3">
              {[
                ...
                <div key={k.label} className={`${card} p-4`}><p className={`text-2xl font-bold tabular-nums ${k.tone ?? "text-gray-900"}`}>{k.value}</p><p className="text-[11px] text-gray-500 font-medium mt-0.5">{k.label}</p></div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                {q.domains.map((d: any) => (
                  <Link key={d.key} href={d.href} className="block group">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-xs font-bold tabular-nums ${d.score == null ? "text-gray-500" : scoreTone(d.score)}`}>{d.score == null ? "—" : d.score}</span>
                    </div>
            ...
            {/* Consolidated risks */}
            <div className={`${card} p-4`}>
              <div className="flex items-center justify-between mb-3"><p className="text-[11px] font-semibold text-gray-500">Top assurance risks</p><span className="text-[10px] text-gray-500">{q.risks.length} open</span></div>
              {q.risks.length === 0 ? (
                <p className="text-sm text-gray-500 py-8 text-center">No open assurance risks — every signal is within tolerance. 🎯</p>
              ) : (
          ...
          {q.trend.length > 0 && (
            <div className={`${card} p-4 mt-4`}>
              <div className="flex items-center justify-between mb-3"><p className="text-[11px] font-semibold text-gray-500">Assessment outcome trend</p><span className="text-[10px] text-gray-500">% achieved, by month</span></div>
