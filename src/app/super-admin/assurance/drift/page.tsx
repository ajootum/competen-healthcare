    ...
    <div className="max-w-5xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">How workforce competency is changing over time — decay vs improvement across reassessments, expiry pressure, and where drift concentrates.</p>
        </div>
      ...
      {!q.provisioned ? (
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-6"><p className="text-sm text-gray-500">No competency decisions recorded yet. Once assessments produce decisions over time, drift analytics populate here automatically.</p></div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
            ...
            <div className={`${card} p-4`}>
              <div className="flex items-center justify-between mb-3"><p className="text-[11px] font-semibold text-gray-500">Assessment outcome trend</p><span className="text-[10px] text-gray-500">% achieved, by month decided</span></div>
              {q.trend.length === 0 ? <p className="text-xs text-gray-500 py-6 text-center">Not enough dated decisions to trend.</p> : (
                <div className="flex items-end gap-2 h-32">
                  {q.trend.map((t: any) => (
                    <div key={t.month} className="flex-1 flex flex-col items-center gap-1 min-w-0">
                      ...
                      <span className="text-[8px] text-gray-500 truncate w-full text-center">{t.month.slice(2)}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className={`${card} p-4`}>
              ...
              {q.reassessed === 0 ? (
                <p className="text-xs text-gray-500 py-6 text-center">No competencies have been reassessed more than once yet, so decay vs improvement can&apos;t be measured. It populates as people are re-assessed over time.</p>
              ) : (
                <>
                  <div className="flex items-center gap-3 mb-3">
                    <div className="flex-1 text-center"><p className="text-2xl font-bold text-[var(--cmp-text-success)] tabular-nums">{q.kpis.improved}</p><p className="text-[10px] text-gray-500">improved ↑</p></div>
                    <div className="flex-1 text-center"><p className="text-2xl font-bold text-[var(--cmp-text-error)] tabular-nums">{q.kpis.decayed}</p><p className="text-[10px] text-gray-500">decayed ↓</p></div>
                    <div className="flex-1 text-center"><p className="text-2xl font-bold text-gray-700 tabular-nums">{q.reassessed}</p><p className="text-[10px] text-gray-500">reassessed</p></div>
                  </div>
                  <div className="divide-y divide-gray-50 border-t border-gray-50">
                    {q.recentDecays.slice(0, 6).map((d: any, i: number) => (
                      <div key={i} className="flex items-center gap-2 py-1.5">
                        ...
                        <span className="text-[10px] text-gray-500 truncate max-w-[120px]">{d.competency}</span>
                        <span className="text-[9px] font-semibold text-[var(--cmp-text-error)] shrink-0">{fmt(d.from)} → {fmt(d.to)}</span>
                      </div>
                    ))}
                    {q.recentDecays.length === 0 && <p className="text-[11px] text-gray-500 py-2 text-center">No decays — movement is upward. 🎉</p>}
                  </div>
                </>
              )}
            </div>
          ...
          <div className={`${card} overflow-hidden`}>
            <div className="px-4 py-2.5 border-b border-gray-50 flex items-center justify-between"><p className="text-[11px] font-semibold text-gray-500">Drift hotspots</p><span className="text-[10px] text-gray-500">competencies with the highest share expired / failing</span></div>
            {q.hotspots.length === 0 ? (
              <p className="text-xs text-gray-500 px-4 py-6 text-center">No competency is drifting — currency and outcomes are holding across the board. 🎯</p>
            ) : (
              <div className="divide-y divide-gray-50">
                {q.hotspots.map((h: any, i: number) => (
                  <div key={i} className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <span className="text-[11px] text-gray-500 tabular-nums w-16 text-right shrink-0">{h.drifting}/{h.total}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <p className="text-[11px] text-gray-500 mt-4 leading-relaxed">Drift index = share of currently-held competencies that are expired, expiring or failing (0 = stable). Decay/improvement compares each person&apos;s latest reassessment against their prior one (Benner maturity or a lapse to expired). Real over <code className="text-[10px]">competency_decisions</code> — decision-support for targeted reassessment, not an automated action.</p>
        </>
      )}
    </div>
  );
