    ...
    <div className="max-w-5xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">How each assessor scores relative to their peers — leniency vs severity, consistency, and inter-rater agreement — over the real assessment record.</p>
        </div>
      ...
      {!q.provisioned ? (
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-6"><p className="text-sm text-gray-500">No scored assessments recorded yet. Once assessors score competencies, per-assessor reliability populates here automatically.</p></div>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
            {[
              ...
              <div key={k.label} className={`${card} p-3.5`}><p className={`text-xl font-bold tabular-nums ${k.tone}`}>{k.value}</p><p className="text-[10px] text-gray-500 font-medium mt-0.5 leading-tight">{k.label}</p></div>
            ))}
          </div>

          {/* Calibration watchlist */}
          <div className={`${card} overflow-hidden mb-5`}>
            <div className="px-4 py-2.5 border-b border-gray-50 flex items-center justify-between"><p className="text-[11px] font-semibold text-gray-500">Calibration watchlist</p><span className="text-[10px] text-gray-500">outliers past peer tolerance — candidates for a calibration session</span></div>
            {q.watchlist.length === 0 ? (
              <p className="text-xs text-gray-500 px-4 py-6 text-center">No assessors outside tolerance. Scoring is well-calibrated across the peer group. 🎯</p>
            ) : (
              <div className="divide-y divide-gray-50">
                {q.watchlist.map((a: any) => (
                  <div key={a.id} className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <span className="text-[11px] text-gray-500 tabular-nums shrink-0">{a.n} scored</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {q.assessors.slice(0, 50).map((a: any) => (
                      <tr key={a.id}>
                        <td className="py-2 px-4 text-gray-800 truncate max-w-[160px]">{a.name}{a.lowConfidence && <span className="ml-1 text-[9px] text-gray-500">·low n</span>}</td>
                        ...
                        <td className={`py-2 px-2 tabular-nums text-right font-medium ${a.deviation > 0 ? "text-[var(--cmp-text-warning)]" : a.deviation < 0 ? "text-[var(--cmp-text-error)]" : "text-gray-500"}`}>{a.deviation > 0 ? "+" : ""}{a.deviation}</td>
                        ...
                        <td className={`py-2 px-4 tabular-nums text-right font-bold ${a.lowConfidence ? "text-gray-500" : relTone(a.reliability)}`}>{a.lowConfidence ? "—" : a.reliability}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className={`${card} p-4`}>
              ...
              {q.interRater.sampledItems === 0 ? (
                <p className="text-xs text-gray-500">No items were scored by more than one assessor yet, so inter-rater agreement can&apos;t be measured. It populates once the same skill is co-assessed.</p>
              ) : (
                <>
                  <div className="text-center py-2">
                    <p className="text-3xl font-bold text-indigo-600 tabular-nums">{q.interRater.agreement}%</p>
                    <p className="text-[11px] text-gray-500 mt-0.5">of co-scored items agree within 1 point</p>
                  </div>
              ...
              <p className="text-[10px] text-gray-500 mt-4 leading-relaxed border-t border-gray-50 pt-3">Deviation is each assessor&apos;s mean score minus the peer mean (0–6 scale). Reliability is a labelled heuristic (peer-bias + consistency), decision-support for calibration — not a psychometric verdict.</p>
            </div>
          </div>
        </>
      )}
