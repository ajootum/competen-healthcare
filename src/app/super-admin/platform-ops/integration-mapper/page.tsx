  ...
  const header = (
    <>
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Data Source &amp; Integration Mapper <span className="text-gray-500 font-medium text-lg">(NCP-010)</span></h1>
  ...
  return (
    <div className="space-y-5 max-w-6xl">
      ...
      <div className={`${card} p-5`}>
        ...
        <p className="text-[11px] text-gray-500 mb-4">The connector families a mapping can bind to (§5). Live connector provisioning + credential vault is next-phase.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className={`${card} p-5`}>
          ...
          {d.sources.length ? (
            <div className="space-y-1.5">{d.sources.map((src: any) => <div key={src.key} className="flex items-center justify-between gap-2 text-[11px]"><code className="text-gray-700 font-mono truncate">{src.key}</code><span className="text-gray-500 shrink-0">{src.n} object{src.n === 1 ? "" : "s"}</span></div>)}</div>
          ) : <p className="text-xs text-gray-500 py-6 text-center">No objects reference a data source yet — bind them below to power live data.</p>}
        </div>
        <div className={`${card} p-5`}>
          <h2 className="font-semibold text-gray-900 text-sm mb-1">Binding Gap</h2>
          <p className="text-[11px] text-gray-500 mb-3">Configurable objects with no data source — bind these so the runtime can render live data.</p>
          {d.gap.length ? (
            <>
