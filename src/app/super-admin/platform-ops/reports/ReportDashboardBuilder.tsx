      ...
      <div className="space-y-1 mb-1.5">
        {rows.map((f, i) => (
          <div key={f.key} className="flex items-center gap-1.5">
            ...
            <span className="text-[9px] text-gray-500 font-mono">{f.key}</span>
            <button onClick={() => setRows(r => r.filter((_, j) => j !== i))} className="text-gray-500 hover:text-[var(--cmp-text-error)] text-xs">✕</button>
          </div>
        ))}
      </div>
...
export default function ReportDashboardBuilder({ objects, metrics }: { objects: Obj[]; metrics: Metric[] }) {
  ...
  if (!objects.length) return <div className={`${card} p-8 text-center`}><p className="text-sm text-gray-500">No dashboard or report objects yet.</p><p className="text-xs text-gray-500 mt-1">Author a <b>Dashboard</b> or <b>Report</b> in the <a href="/super-admin/platform-ops/studio" className="text-indigo-700 underline">Configuration Studio</a> first, then compose it here.</p></div>;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-1 max-h-[520px] overflow-y-auto">
          {objects.map(o => (
            <button key={o.object_key} onClick={() => pick(o.object_key)} className={`w-full text-left rounded-lg px-2.5 py-1.5 transition-colors ${selKey === o.object_key ? "bg-indigo-50 ring-1 ring-indigo-200" : "hover:bg-gray-50"}`}>
              ...
              <p className="text-[10px] text-gray-500 truncate">{o.object_type === "DASHBOARD" ? `${o.definition?.tiles?.length ?? 0} tile(s)` : `${o.definition?.sections?.length ?? 0} section(s)`}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className={`${card} p-5 lg:col-span-3`}>
        {!selO ? <p className="text-sm text-gray-500 py-16 text-center">Select a dashboard or report.</p> : (
          <>
            ...
            {isDash ? (
              <>
                ...
                {tiles.length === 0 ? <p className="text-xs text-gray-500 py-4 text-center">Add tiles to compose the dashboard.</p> : (
                  <div className="space-y-1.5 mb-4">
                    {tiles.map((t, i) => (
                      <div key={t.key} className="flex items-center gap-1.5 border border-gray-100 rounded-lg px-2.5 py-1.5 flex-wrap">
                        ...
                        <button onClick={() => setTiles(ts => ts.filter((_, j) => j !== i))} className="text-gray-500 hover:text-[var(--cmp-text-error)] text-xs">✕</button>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {tiles.length > 0 && (
                  <div className="rounded-lg bg-gray-50 border border-gray-100 p-3">
                    <p className="text-[11px] font-semibold text-gray-500 mb-2">Preview <span className="font-normal text-gray-500">· 12-col grid</span></p>
                    <div className="grid gap-1.5" style={{ gridTemplateColumns: "repeat(12, minmax(0, 1fr))" }}>
                      {tiles.map(t => (
                        <div key={t.key} className="rounded-md bg-white border border-gray-200 px-2 py-1.5 min-h-[46px]" style={{ gridColumn: `span ${t.span} / span ${t.span}` }}>
                          <p className="text-[10px] text-gray-500">{VM[t.viz]?.i} {VM[t.viz]?.l}</p>
                          <p className="text-xs font-medium text-gray-800 truncate">{t.title || <span className="text-gray-500">untitled</span>}</p>
              ...
              <>
                ...
                {sections.length === 0 ? <p className="text-xs text-gray-500 py-4 text-center">Add sections to compose the report.</p> : (
                  <div className="space-y-1.5 mb-4">
                    {sections.map((s, i) => (
                      <div key={s.key} className="flex items-center gap-1.5 border border-gray-100 rounded-lg px-2 py-1.5 flex-wrap">
                        <div className="flex flex-col leading-none"><button onClick={() => move(i, -1)} disabled={i === 0} className="text-gray-500 hover:text-gray-600 disabled:opacity-30 text-[10px]">▲</button><button onClick={() => move(i, 1)} disabled={i === sections.length - 1} className="text-gray-500 hover:text-gray-600 disabled:opacity-30 text-[10px]">▼</button></div>
                        ...
                        <span className="text-[9px] text-gray-500 font-mono">{s.key}</span>
                        <button onClick={() => setSections(ss => ss.filter((_, j) => j !== i))} className="text-gray-500 hover:text-[var(--cmp-text-error)] text-xs">✕</button>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {sections.length > 0 && (
                  <div className="rounded-lg bg-gray-50 border border-gray-100 p-3">
                    <p className="text-[11px] font-semibold text-gray-500 mb-2">Preview <span className="font-normal text-gray-500">· {d.page?.size} {d.page?.orientation}</span></p>
                    <div className="mx-auto bg-white border border-gray-200 rounded-md p-3 max-w-sm space-y-1.5">
                      {sections.map(s => s.type === "page_break"
                        ? <div key={s.key} className="border-t border-dashed border-gray-300 my-1 text-center text-[9px] text-gray-500">page break</div>
                        : <div key={s.key}><p className="text-[9px] text-gray-500 uppercase tracking-wide">{SM[s.type]}</p><p className="text-xs font-medium text-gray-800">{s.title || <span className="text-gray-500">untitled</span>}</p>{metricName(s.metric) && <p className="text-[10px] text-indigo-500">↳ {metricName(s.metric)}</p>}</div>)}
                    </div>
                  </div>
                )}
              </>
