...
export default async function AiGatewayConsole() {
  ...
  const healthTone = !status.configured ? "text-gray-500" : s.errors24h > 0 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-success)]";
    ...
    <div data-wide className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5`}>
          ...
          {tiers.length === 0 ? <p className="text-sm text-gray-500">No provider configured.</p> : (
          ...
          <p className="text-[10px] text-gray-500 mt-3 pt-2 border-t border-gray-50">Tiers resolve per job (cheap / reasoning / heavy) via lib/ai/config; override with AI_MODEL_* env vars.</p>
        </div>
        ...
        <div className={`${card} p-5`}>
          ...
          {!s.ready || gov.byModel.length === 0 ? <p className="text-sm text-gray-500 py-4 text-center">{s.ready ? "No AI requests in 24h." : "Telemetry off."}</p> : (
        ...
        <div className={`${card} p-5`}>
          ...
          {!s.ready || gov.byOperation.length === 0 ? <p className="text-sm text-gray-500 py-4 text-center">{s.ready ? "No attributed operations." : "Telemetry off."}</p> : (
            <div className="space-y-1.5">
      ...
      <div className={`${card} p-5`}>
        ...
        {!s.ready || gov.recent.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">{s.ready ? "No AI requests recorded yet." : "Run the migration to record usage."}</p> : (
      ...
      <p className="text-[11px] text-gray-500 pb-4">Every server-side generation routes through one gateway (lib/ai/client → generate), which records model, tokens, latency, status and estimated cost. Per-user rate limiting is live ({hourlyLimit}/hr, lib/ai/quota); token budgets, response caching and per-tenant AI policies are the next governance layer. Cost is estimated from provider list pricing and excludes prompt-cache discounts.</p>
    </div>
  );
}
