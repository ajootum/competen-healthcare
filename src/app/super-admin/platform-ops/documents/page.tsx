...
function Donut({ segs, total }: { segs: { n: number; color: string }[]; total: number }) {
  ...
  return <div className="relative w-24 h-24 shrink-0"><div className="w-24 h-24 rounded-full" style={{ background: sum > 0 ? `conic-gradient(${stops})` : "#f1f5f9" }} /><div className="absolute inset-[22%] rounded-full bg-white flex flex-col items-center justify-center"><span className="text-lg font-bold text-gray-900">{total}</span><span className="text-[8px] text-gray-500">Docs</span></div></div>;
}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className={`${card} p-5`}>
          ...
          {d.byType.length === 0 ? <p className="text-sm text-gray-500">No documents.</p> : (
        ...
        <div className={`${card} p-5`}>
          ...
          <div className="space-y-2">{d.bySource.map(x => (<div key={x.label} className="text-xs"><div className="flex items-center justify-between mb-0.5"><span className="text-gray-700">{x.label}</span><span className="text-gray-500">{x.n} ({x.pct}%)</span></div><div className="w-full h-1.5 rounded-full bg-gray-100 overflow-hidden"><div className="h-full rounded-full bg-teal-500" style={{ width: `${x.pct}%` }} /></div></div>))}</div>
        </div>
        <div className={`${card} p-5`}>
          ...
          <div className="space-y-1.5 text-xs">
            ...
            <div className="flex items-center justify-between"><span className="text-gray-500">Versioning</span><span className="text-gray-500 text-[10px]">Next phase</span></div>
            <div className="flex items-center justify-between"><span className="text-gray-500">Retention policy</span><span className="text-gray-500 text-[10px]">Next phase</span></div>
          </div>
        </div>
      </div>
