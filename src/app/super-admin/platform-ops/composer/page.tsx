...
const classTone = (c: string) => (c === "mandatory_locked" ? "bg-gray-800 text-white" : c.startsWith("mandatory") ? "bg-rose-50 text-rose-700" : c === "optional" ? "bg-emerald-50 text-emerald-700" : c === "conditional" ? "bg-amber-50 text-amber-700" : "bg-gray-50 text-gray-500");
const actionTone = (t: string) => (t === "rose" ? "bg-rose-500" : t === "amber" ? "bg-amber-400" : t === "sky" ? "bg-sky-400" : "bg-gray-300");
...
export default async function TenantExperienceComposer({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!d.registryProvisioned) return (
    <div data-wide className="space-y-4">{header}
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800"><span className="font-semibold">Registry not provisioned.</span> WCE-003 configures the objects catalogued by WCE-002 — run <code className="font-mono text-[12px] bg-amber-100 px-1 rounded">migration 092</code> and sync the <Link href="/super-admin/platform-ops/registry" className="underline">Configuration Registry</Link> first.</div>
    </div>
  );
  ...
  return (
    <div data-wide className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
        ...
        <Stat label="Local Overrides" value={d.summary.localOverrides} tone={d.summary.localOverrides ? "text-sky-600" : "text-gray-400"} />
        <Stat label="Draft Changes" value={d.summary.draftChanges} tone={d.summary.draftChanges ? "text-amber-600" : "text-gray-400"} />
        <Stat label="Validation Errors" value={d.summary.validationErrors} tone={d.summary.validationErrors ? "text-rose-600" : "text-emerald-600"} />
      </div>
      ...
      <div className={`${card} p-5`}>
        ...
        <div className="space-y-4">
          {d.tree.map((ws: any) => (
            <div key={ws.key}>
              <div className="flex items-center gap-2 mb-1.5"><h3 className="text-sm font-bold text-gray-800">{ws.name}</h3>{ws.route && <span className="text-[10px] text-gray-400 font-mono">{ws.route}</span>}<span className={`text-[9px] px-1.5 py-0.5 rounded ${ws.wired ? "bg-emerald-50 text-emerald-600" : "bg-gray-100 text-gray-500"}`}>{ws.wired ? "runtime-wired" : "catalogued"}</span></div>
...
function ObjectRow({ o, bold }: { o: any; bold?: boolean }) {
  return (
    <div className="flex items-center gap-2 px-2.5 py-1.5 text-xs">
      <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${o.enabled ? "bg-emerald-500" : "bg-gray-300"}`} title={o.enabled ? "Enabled" : "Disabled"} />
      ...
      {o.hasLocalOverride && <span className="text-[9px] text-sky-600 shrink-0">override here</span>}
      {o.hasDraft && <span className="text-[9px] text-amber-600 shrink-0">draft</span>}
