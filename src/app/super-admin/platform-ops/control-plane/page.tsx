...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return <div className="flex justify-between gap-4 py-1.5 border-b border-gray-50 last:border-0 text-sm"><span className="text-gray-500">{label}</span><span className="text-gray-800 text-right font-medium">{value ?? <span className="text-gray-500">—</span>}</span></div>;
}
    ...
    <div data-wide className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        {kpis.map(k => (
          <div key={k.label} className={`${card} p-4`}>
            ...
            <p className={`text-2xl font-bold mt-1.5 tabular-nums capitalize ${(k as any).muted ? "text-gray-500" : (k as any).tone ?? "text-gray-900"}`}>{k.value}</p>
        ...
        <Panel title="Release & Deployments" href="/platform/control-plane/deployments" badge={release.recorded ? `${release.count} recorded` : undefined}>
          {!release.recorded ? (
            <div className="py-4 text-center">
              ...
              <p className="text-[11px] text-gray-500 mt-2">Deployment history activates when releases are recorded to the pipeline.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {release.recent.map((d: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  ...
                  <span className="flex items-center gap-2"><span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-600 capitalize">{d.channel}</span><span className="text-[10px] text-gray-500">{relTime(d.released_at ?? d.created_at)}</span></span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="space-y-1 max-h-32 overflow-y-auto">
            {products.slice(0, 8).map((p: any) => (
              <div key={p.code} className="flex items-center justify-between text-xs">
                ...
                <span className={`shrink-0 ${p.is_core ? "text-[var(--cmp-text-success)]" : "text-gray-500"}`}>{p.is_core ? "Core" : "Optional"}</span>
              </div>
            ))}
          </div>
        </Panel>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {[
            ...
            <div key={col.plane} className="rounded-lg border border-gray-100 p-4">
              ...
              {col.rows.map(([l, n]) => <Row key={l as string} label={l as string} value={n == null ? <span className="text-gray-500">—</span> : (n as number).toLocaleString()} />)}
            </div>
          ))}
        </div>
      </Panel>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Panel title="Regions & Residency" badge={cp.regionsReady ? `${regions.length}` : undefined}>
          {!cp.regionsReady || regions.length === 0 ? <p className="text-sm text-gray-500 py-4 text-center">{cp.regionsReady ? "No regions configured." : "Region registry not available."}</p> : (
            <div className="space-y-2">
              {regions.map((r: any) => (
                <div key={r.code} className="flex items-center justify-between text-sm">
                  <div><span className="text-gray-800 font-medium">{r.name}</span> <span className="text-[10px] text-gray-500 font-mono">{r.code}</span><p className="text-[10px] text-gray-500">{r.hosting_provider ?? "—"}{r.residency_policy ? ` · ${r.residency_policy}` : ""}</p></div>
                  <span className={`text-[10px] shrink-0 ${r.is_active ? "text-[var(--cmp-text-success)]" : "text-gray-500"}`}>{r.is_active ? "active" : "inactive"}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <Panel title="Platform Events" href="/platform/control-plane/events" badge={cp.events24h != null ? `${cp.events24h} in 24h` : undefined}>
          {!eventsReady || events.length === 0 ? <p className="text-sm text-gray-500 py-4 text-center">{eventsReady ? "No platform events recorded." : "Event stream not available."}</p> : (
            <div className="space-y-1.5 max-h-40 overflow-y-auto">
              {events.slice(0, 10).map((e: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-[10px] text-gray-500 shrink-0">{relTime(e.created_at)}</span>
                </div>
              ))}
            </div>
          )}
