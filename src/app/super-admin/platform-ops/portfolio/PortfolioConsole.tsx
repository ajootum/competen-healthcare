...
export default function PortfolioConsole({ data }: { data: any }) {
  ...
  return (
    <div className="space-y-5">
      {msg && <div className="bg-rose-50 border border-rose-200 text-rose-700 text-[12px] rounded-lg px-3 py-2">{msg}</div>}
      ...
      <div className={`${card} p-4`}>
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button onClick={() => f.pfName && call("POST", { body: { type: "portfolio", name: f.pfName } })} disabled={pending || !f.pfName} className={`${btn} bg-blue-600 text-white hover:bg-blue-500 disabled:opacity-40`}>+ Portfolio</button>
        </div>
          ...
          {data.tree.map((pf: any) => (
            <div key={pf.id} className="border border-gray-100 rounded-xl p-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`text-[10px] rounded-full px-1.5 py-0.5 ${pf.status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>{pf.status}</span>
                <span className="flex-1" />
                {pf.status === "active" && <button onClick={() => call("DELETE", { query: `type=portfolio&id=${pf.id}` })} disabled={pending} className="text-[11px] text-gray-400 hover:text-rose-600">Archive</button>}
              </div>
                ...
                {pf.suites.map((s: any) => (
                  <div key={s.id} className="bg-gray-50/50 rounded-lg p-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="flex-1" />
                      <button onClick={() => call("DELETE", { query: `type=suite&id=${s.id}` })} disabled={pending} className="text-[10px] text-gray-400 hover:text-rose-600">Archive</button>
                    </div>
                    {/* products in suite */}
