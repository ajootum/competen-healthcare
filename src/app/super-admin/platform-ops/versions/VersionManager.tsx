    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
      <div className={`${card} p-4`}>
        ...
        <div className="space-y-1 max-h-[540px] overflow-y-auto">
          ...
          {list.length === 0 && <p className="text-[11px] text-gray-500 py-2">No objects match.</p>}
        </div>
      </div>

      <div className={`${card} p-5 lg:col-span-3`}>
        {!selO ? <p className="text-sm text-gray-500 py-16 text-center">Select an object to see its version history.</p> : (
          <>
            ...
            {loading ? <p className="text-xs text-gray-500 py-4">Loading…</p> : snaps.length === 0 ? <p className="text-xs text-gray-500 py-4">No snapshots yet — save this object&apos;s definition in its designer, or capture one now.</p> : (
              <>
                {/* Compare bar */}
                ...
                {diff && (
                  <div className="rounded-lg bg-gray-50 border border-gray-100 p-3 mb-4">
                    ...
                    {diff.length === 0 ? <p className="text-[11px] text-gray-500">Identical.</p> : (
                      <div className="space-y-1 max-h-52 overflow-y-auto">
                        {diff.map((d, i) => (
                          <div key={i} className="text-[11px] flex items-start gap-1.5">
                            ...
                            <span className="text-gray-500">{d.kind === "changed" ? <>{short(d.before)} <span className="text-gray-500">→</span> {short(d.after)}</> : d.kind === "added" ? short(d.after) : short(d.before)}</span>
                          </div>
                        ))}
                      </div>
                    )}
                ...
                <div className="space-y-1 max-h-72 overflow-y-auto">
                  {snaps.map((s, i) => (
                    <div key={s.version} className="flex items-center gap-2 border border-gray-100 rounded-lg px-2.5 py-1.5">
                      ...
                      <span className="text-[10px] text-gray-500">{s.actor_name ?? "—"}</span>
                      <span className="text-[10px] text-gray-500">{when(s.created_at)}</span>
                      <span className="text-[9px] text-gray-500 font-mono">{s.checksum}</span>
