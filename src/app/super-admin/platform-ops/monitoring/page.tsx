    ...
    <div data-wide className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Panel title="Subsystem Health" badge={`${servicesSummary.operational}/${servicesSummary.total} up`}>
            ...
            {services.map(s => (
              <div key={s.key} className="flex items-center justify-between text-sm py-1">
                <span className="flex items-center gap-2 min-w-0">
                  ...
                  {s.core && <span className="text-[9px] text-gray-500 border border-gray-200 rounded px-1 shrink-0">core</span>}
                </span>
          ...
          <p className="text-[10px] text-gray-500 mt-3 pt-2 border-t border-gray-50">Live reachability + round-trip latency per subsystem. Deep telemetry (CPU, memory, error rates) activates with the monitoring agent.</p>
        </Panel>
        ...
        <Panel title="Active Alerts" badge={alertsSummary.ready ? `${alertsSummary.total} open` : undefined} href="/supervisor/clinical-safety" linkLabel="Safety board">
          {!alertsSummary.ready ? <p className="text-sm text-gray-500 py-6 text-center">Alert sources unavailable.</p>
            : alerts.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">✅ No active alerts.</p> : (
                ...
                {alerts.slice(0, 12).map((a: any, i: number) => (
                  <div key={i} className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-500 capitalize">{a.source} · {a.meta}</p>
                    </div>
                    <div className="flex flex-col items-end shrink-0">
                      <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded ${TIER_STYLE[a.tier].chip}`}>{a.tier}</span>
                      <span className="text-[10px] text-gray-500 mt-0.5">{relTime(a.at)}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {alertsSummary.ready && alerts.length > 0 && (
            <p className="text-[10px] text-gray-500 mt-3 pt-2 border-t border-gray-50">{alertsSummary.escalations} escalation{alertsSummary.escalations === 1 ? "" : "s"} · {alertsSummary.safety} safety alert{alertsSummary.safety === 1 ? "" : "s"} across tenants.</p>
          )}
        </Panel>

        {/* Event stream */}
        <Panel title="Event Stream" href="/super-admin/audit" linkLabel="Audit log">
          {!eventsReady || events.length === 0 ? <p className="text-sm text-gray-500 py-6 text-center">{eventsReady ? "No recorded events yet." : "Event log unavailable."}</p> : (
              ...
              {events.map((e: any, i: number) => (
                <div key={i} className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-500 truncate">{e.plane}{e.actor ? ` · ${e.actor}` : ""}</p>
                  </div>
