  ...
  return (
    <div className="grid lg:grid-cols-3 gap-4">
            ...
            <tbody>
              {planRows.map(p => (
                <tr key={p.id} onClick={() => { setModal({ mode: "edit", plan: p }); setErr(""); }} className="border-b border-gray-50 hover:bg-gray-50/60 cursor-pointer">
                  <td className="px-4 py-3"><div className="flex items-center gap-2"><span className="w-7 h-7 rounded-lg bg-violet-50 flex items-center justify-center text-sm shrink-0">🧾</span><div><p className="font-medium text-gray-900">{p.name}</p><p className="text-[10px] text-gray-500">{p.code}</p></div></div></td>
                  <td className="px-4 py-3 text-gray-600 tabular-nums">{p.currency} {p.price}<span className="text-gray-500">/mo</span></td>
                  <td className="px-4 py-3 text-gray-600 tabular-nums">{p.entitlements?.max_users ?? "∞"}</td>
                  <td className="px-4 py-3">{p.entitlements?.api_access ? <span className="text-[var(--cmp-text-success)]">✓</span> : <span className="text-gray-500">—</span>}</td>
                  ...
                  <td className="px-4 py-3 text-right"><span className={`text-[10px] font-medium px-2 py-0.5 rounded ${p.active ? "bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)]" : "bg-gray-100 text-gray-600"}`}>{p.active ? "Active" : "Inactive"}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className={cardClass}>
        ...
        {renewals.length === 0 ? <p className="text-sm text-gray-500">No scheduled renewals.</p> : (
          <div className="space-y-2">
            {renewals.map((r: any, i: number) => (
              <div key={i} className="flex items-center justify-between text-sm">
                <div className="min-w-0"><p className="text-gray-800 truncate">{r.tenant}</p><p className="text-[10px] text-gray-500">{r.plan}</p></div>
        ...
        <p className="text-[10px] text-gray-500 mt-3 pt-3 border-t border-gray-50">Billing &amp; invoicing history activates when the billing provider is connected — MRR shown is plan list-price × live subscriptions.</p>
      </div>
    ...
    <Modal open title={`${mode} plan`} onClose={onClose}>
      <div className="flex flex-col gap-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><label className="text-xs font-semibold text-gray-600 mb-1 block">Code {mode === "edit" && <span className="text-gray-500">(fixed)</span>}</label><input value={form.code} onChange={set("code")} disabled={mode === "edit"} className={`${input} disabled:bg-gray-50 disabled:text-gray-500`} placeholder="professional" /></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
        ...
        <p className="text-[11px] font-semibold text-gray-500 pt-1">Entitlements <span className="font-normal text-gray-500">(blank = unlimited)</span></p>
        <div className="grid grid-cols-2 gap-3">
