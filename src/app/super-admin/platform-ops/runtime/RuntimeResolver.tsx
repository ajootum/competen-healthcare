...
function NavTree({ items, depth = 0 }: { items: any[]; depth?: number }) {
  return <>{items.map((it: any) => (
    <div key={it.key}>
      <div className={`flex items-center gap-1.5 text-[11px] py-0.5 ${it.visible ? "text-gray-700" : "text-gray-500 line-through"}`} style={{ paddingLeft: depth * 14 }}>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
      ...
      <div className={`${card} p-5 lg:col-span-3`}>
        ...
        {!res ? <p className="text-sm text-gray-500 py-16 text-center">Pick an object and a context, then resolve its effective configuration.</p> : (
          <>
            <div className="flex items-center gap-2 mb-3"><h3 className="text-sm font-semibold text-gray-900">{res.object.name}</h3><span className="text-[10px] text-gray-500 font-mono">{res.object.key}</span></div>
            ...
            <p className="text-[11px] font-semibold text-gray-500 mb-1.5">Resolution trace <span className="font-normal text-gray-500">· least → most specific; later layers win</span></p>
            <div className="space-y-1.5 mb-4">
              {res.trace.map((t: any, i: number) => (
                <div key={i} className="flex items-start gap-2 border border-gray-100 rounded-lg px-2.5 py-1.5">
                  <span className="text-[9px] text-gray-500 w-4 tabular-nums pt-0.5">{i + 1}</span>
                  ...
                  {t.note && <span className="text-[10px] text-gray-500 shrink-0">{t.note}</span>}
                </div>
              ))}
            </div>

            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] text-gray-500">Allowed levels:</span>
              ...
              <span className="text-[10px] text-gray-500 ml-auto font-mono">{res.cacheKey}</span>
            </div>

            {composed && (
              <div className="mt-5 pt-4 border-t border-gray-100">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[11px] font-semibold text-gray-500">Composed runtime model <span className="font-normal text-gray-500">· what renders in this context</span></p>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-gray-500">{composed.stats.included} shown · {composed.stats.excluded} hidden · {composed.stats.references} ref(s)</span>
                      ...
                      <div key={ri} className="grid gap-1.5" style={{ gridTemplateColumns: `repeat(${composed.model.grid}, minmax(0,1fr))` }}>
                        {row.columns.map((col: any, ci: number) => (
                          <div key={ci} className={`rounded-md border px-2 py-1.5 min-h-[40px] ${col.shown ? "bg-white border-gray-200" : "bg-gray-50 border-dashed border-gray-200 opacity-50"}`} style={{ gridColumn: `span ${col.span} / span ${col.span}` }}>
                            <p className={`text-[11px] font-medium truncate ${col.shown ? "text-gray-800" : "text-gray-500 line-through"}`}>{col.widget?.name ?? "—"}</p>
                  ...
                  <div className="grid gap-1.5" style={{ gridTemplateColumns: "repeat(12, minmax(0,1fr))" }}>
                    {composed.model.tiles.map((t: any) => (
                      <div key={t.key} className={`rounded-md border px-2 py-1.5 min-h-[44px] ${t.shown ? "bg-white border-gray-200" : "bg-gray-50 border-dashed border-gray-200 opacity-50"}`} style={{ gridColumn: `span ${t.span || 4} / span ${t.span || 4}` }}>
                        <p className={`text-[11px] font-medium truncate ${t.shown ? "text-gray-800" : "text-gray-500 line-through"}`}>{t.title || t.key}</p>
