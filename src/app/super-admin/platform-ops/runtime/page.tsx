  ...
  return (
    <div className="space-y-5 max-w-6xl">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Runtime &amp; Resolution Engine <span className="text-gray-300 font-medium text-lg">(NCP-015)</span></h1>
      ...
      <div className={`${card} p-4`}>
        <p className="text-[11px] font-semibold text-gray-500 mb-2">Resolution precedence <span className="font-normal text-gray-400">· most specific wins per attribute</span></p>
        <div className="flex flex-wrap items-center gap-1.5">
          {PRECEDENCE.map((s, i) => <span key={s} className="flex items-center gap-1.5"><span className="text-[11px] bg-indigo-50 text-indigo-700 rounded px-2 py-1">{s}</span>{i < PRECEDENCE.length - 1 && <span className="text-gray-300 text-[10px]">→</span>}</span>)}
        </div>
      </div>
