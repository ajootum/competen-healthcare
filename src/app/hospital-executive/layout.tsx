...
export default async function ExecutiveLayout({ children }: { children: React.ReactNode }) {
  ...
  const apptOk = roleOk ? false : await holdsOfficeAppointment(admin, "executive", (profile?.hospital_id as string) ?? null, userRoles.includes("super_admin"), user.id);
  if (!roleOk && !apptOk) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm mt-1">The Hospital Executive workspace is for hospital leadership and executive teams.</p>
    ...
    <div className="min-h-screen bg-gray-50 font-[family-name:var(--font-geist-sans)]">
      <header className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#0a2e38] shadow-lg">
        <div className="h-12 flex items-center gap-2 px-3">
          ...
          <span className="min-w-0">
            ...
            <span className="block text-teal-300/60 text-[10px] leading-tight">Hospital Executive</span>
          </span>
      ...
      <div className="flex">
        <aside data-sidebar className="hidden md:flex w-56 h-screen bg-[#0a2e38] flex-col py-6 px-4 fixed top-0 left-0 z-20">
          <SidebarToggle />
          <Link href="/hospital-executive" className="flex items-center gap-2 mb-6 px-2" data-sb-item>
          ...
          <div className="px-3 mb-4" data-sb-label>
            <span className="text-[10px] font-bold text-teal-400/70 uppercase tracking-widest">Hospital Executive</span>
          </div>
