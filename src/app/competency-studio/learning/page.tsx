...
export default async function StudioLearningPage() {
  ...
  const lp = await loadLearningPaths(admin, hid, isSuper);

  return (
    <div className="max-w-6xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          <p className="text-[11px] font-semibold text-indigo-500 uppercase tracking-widest mb-0.5">CST-005 · Learning Path Studio</p>
          <h1 className="text-xl font-bold text-gray-900">Learning Paths &amp; Curricula</h1>
          <p className="text-gray-400 text-sm mt-0.5">Reusable programmes, the learning-resource library and competency coverage — the learning side of the Studio.</p>
        </div>
        <Link href="/admin/resources" className="text-xs font-semibold text-white bg-teal-700 hover:bg-teal-800 rounded-lg px-3 py-2">Resource library →</Link>
      </div>

      {!lp.provisioned ? (
        <div className="bg-white rounded-xl border border-gray-100 p-8 text-center text-sm text-gray-400">Learning tables are not available in this environment.</div>
      ) : lp.empty ? (
        <div className="bg-white rounded-xl border border-gray-100 p-8 text-center text-sm text-gray-400">No curricula or learning resources yet — author them in the <Link href="/admin/resources" className="text-teal-600 hover:underline">resource library</Link>.</div>
      ) : (
        <>
          {/* KPIs */}
          ...
          <div className="grid md:grid-cols-3 gap-5 mb-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5 md:col-span-2">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[9px] text-gray-400">authored curricula — competency &amp; module counts</span>
              </div>
              {lp.programmes.length === 0 ? (
                <p className="text-xs text-gray-400">No curricula authored yet.</p>
              ) : (
                <div className="flex flex-col divide-y divide-gray-50">
                  {lp.programmes.map(p => (
                    <div key={p.id} className="flex items-center gap-3 py-2 text-xs">
                      ...
                      {p.targetRole && <span className="text-[10px] text-gray-400 truncate hidden md:inline">{p.targetRole}</span>}
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              ...
              {lp.resourceTypes.length === 0 ? <p className="text-xs text-gray-400">No resources yet.</p> : lp.resourceTypes.map(t => (
              ...
              <p className="text-[10px] text-gray-400 mt-3">{lp.coverageDetail.covered} of {lp.coverageDetail.total} competencies have a linked learning resource.</p>
            </div>
          </div>
