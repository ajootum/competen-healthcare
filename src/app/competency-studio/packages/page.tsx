...
import PackageManager from "@/app/super-admin/studio/packages/PackageManager";
...
export default async function StudioPackagesPage() {
  ...
  const pkg = await loadPackages(admin, hid, isSuper);
  ...
  return (
    <div className="max-w-6xl">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          <p className="text-[11px] font-semibold text-indigo-500 uppercase tracking-widest mb-0.5">CST-109 · Package Manager</p>
      ...
      {!pkg.provisioned ? (
        <div className="bg-amber-50 border border-amber-100 rounded-xl p-6 text-sm text-amber-800">Run migration 130 (<code className="text-[11px]">competency_packages</code>) to enable the Package Manager.</div>
      ) : (
        <>
