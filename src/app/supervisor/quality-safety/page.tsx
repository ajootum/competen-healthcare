...
const RISK_TONE: Record<string, string> = { high: "bg-rose-200 text-rose-800", medium: "bg-amber-200 text-amber-800", low: "bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)]", normal: "bg-gray-50 text-gray-400" };
const DOT: Record<string, string> = { rose: "bg-[var(--cmp-color-error)]", amber: "bg-[var(--cmp-color-warning)]", orange: "bg-[var(--cmp-color-warning)]", purple: "bg-purple-500" };
const ESC_TONE: Record<string, string> = { open: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", acknowledged: "bg-[var(--cmp-surface-information)] text-blue-700", resolved: "bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)]" };
...
export default async function QualitySafety() {
  ...
  const kpis = [
    ...
    ["High Risk Patients", k.highRiskPatients, "Require close monitoring", k.highRiskPatients ? "text-[var(--cmp-text-error)]" : ""],
    ["Open Incidents", k.openIncidents, `${k.incidentsCritical} critical · ${k.incidentsHigh} high`, k.openIncidents ? "text-[var(--cmp-text-warning)]" : ""],
    ["Escalations Active", k.escalationsActive, `${k.escInProgress} in progress · ${k.escNew} new`, k.escalationsActive ? "text-[var(--cmp-text-warning)]" : ""],
    ["Observation Compliance", k.obsCompliance == null ? "—" : `${k.obsCompliance}%`, "Target 95%", scoreTone(k.obsCompliance)],
    ["Overdue Tasks (Safety)", k.overdueSafetyTasks, `${k.criticalSafetyTasks} critical`, k.overdueSafetyTasks ? "text-[var(--cmp-text-error)]" : ""],
  ];
  ...
  return (
    <div data-wide className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div>
              ...
              {d.heatMap.length === 0 ? <p className="text-xs text-gray-400">No beds configured.</p> : (<>
                ...
                <div className="flex flex-wrap gap-2 mt-2 text-[9px] text-gray-500"><span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-rose-200" />High</span><span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-amber-200" />Medium</span><span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-[var(--cmp-surface-success)]" />Low</span></div>
              </>)}
            </div>
            <div>
        ...
        <div className={`${card} p-5`}>
          ...
          {d.criticalAlerts.length === 0 ? <p className="text-sm text-gray-400">No critical alerts.</p> : (
            <div className="space-y-2">
              {d.criticalAlerts.map((a: any, i: number) => (<div key={i} className={`rounded-lg border p-2.5 ${a.tone === "high" ? "border-[var(--cmp-color-error)] bg-[var(--cmp-surface-error)]/40" : "border-[var(--cmp-color-warning)] bg-[var(--cmp-surface-warning)]/40"}`}><div className="flex items-center gap-2"><span className="text-xs font-medium text-gray-800 truncate flex-1 capitalize">{a.title}</span><span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded ${a.tone === "high" ? "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]" : "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]"}`}>{tc(a.tone)}</span></div><p className="text-[10px] text-gray-400">{relTime(a.at)}{a.sub ? ` · ${a.sub}` : ""}</p></div>))}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className={`${card} p-5`}>
          ...
          {d.escalation.list.length === 0 ? <p className="text-sm text-gray-400">No open escalations.</p> : (
            <div className="space-y-1.5">
              {d.escalation.list.map((e: any, i: number) => (<div key={i} className="flex items-center gap-2 rounded-lg border border-gray-100 px-2.5 py-1.5"><span className={`w-2 h-2 rounded-full shrink-0 ${e.level >= 4 ? "bg-[var(--cmp-color-error)]" : "bg-[var(--cmp-color-warning)]"}`} /><div className="min-w-0 flex-1"><p className="text-xs text-gray-800 truncate">{e.patient ? `${e.patient} — ` : ""}{e.summary}</p><p className="text-[10px] text-gray-400">{relTime(e.at)}</p></div><span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded shrink-0 ${ESC_TONE[e.status] ?? "bg-gray-100 text-gray-600"}`}>{tc(e.status)}</span></div>))}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
        <div className={`${card} p-5`}>
          <div className="flex items-center gap-2 mb-2"><span className="w-6 h-6 rounded-lg bg-[var(--cmp-surface-information)] text-blue-700 flex items-center justify-center text-[11px] font-bold">2</span><h3 className="text-xs font-bold text-gray-900 leading-tight">Observation &amp; Monitoring</h3></div>
          ...
          <div className="flex items-center justify-between text-xs"><span className="text-gray-600">Overdue observations</span><span className={`font-semibold ${d.observation.overdue ? "text-[var(--cmp-text-error)]" : "text-gray-900"}`}>{d.observation.overdue}</span></div>
          <p className="text-[10px] text-gray-400 mt-2">Hourly compliance trend needs per-hour history.</p>
        </div>

        <div className={`${card} p-5`}>
          <div className="flex items-center gap-2 mb-2"><span className="w-6 h-6 rounded-lg bg-[var(--cmp-surface-warning)] text-orange-700 flex items-center justify-center text-[11px] font-bold">3</span><h3 className="text-xs font-bold text-gray-900 leading-tight">Incident &amp; Event Management</h3></div>
        ...
        <div className={`${card} p-5`}>
          <div className="flex items-center gap-2 mb-2"><span className="w-6 h-6 rounded-lg bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)] flex items-center justify-center text-[11px] font-bold">4</span><h3 className="text-xs font-bold text-gray-900 leading-tight">Escalation Centre</h3></div>
        ...
        <div className={`${card} p-5`}>
          <div className="flex items-center gap-2 mb-2"><span className="w-6 h-6 rounded-lg bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)] flex items-center justify-center text-[11px] font-bold">5</span><h3 className="text-xs font-bold text-gray-900 leading-tight">Quality Improvement</h3></div>
