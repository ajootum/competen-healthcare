  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        <Kpi label="Pending" value={rows.length - reviewed} tone={rows.length - reviewed ? "text-[var(--cmp-text-warning)]" : undefined} />
        <Kpi label="Reviewed" value={reviewed} tone="text-[var(--cmp-text-success)]" sub={`${rows.length ? Math.round((reviewed / rows.length) * 100) : 0}%`} />
        <Kpi label="High Risk" value={d.kpis.critical} tone={d.kpis.critical ? "text-[var(--cmp-text-error)]" : undefined} />
        <Kpi label="Escalations" value={d.escalations.length} tone={d.escalations.length ? "text-violet-600" : undefined} />
        <Kpi label="Questions" value={pendingClar} sub="Awaiting" tone={pendingClar ? "text-[var(--cmp-text-warning)]" : undefined} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-4 xl:col-span-1`}>
          ...
          <div className="space-y-1 max-h-[500px] overflow-y-auto">{rows.map((p: any) => (<Link key={p.patientId} href={`/supervisor/handover/incoming?patient=${p.patientId}`} className={`block rounded-lg border p-2 text-xs ${selected?.patientId === p.patientId ? "border-emerald-400 bg-[var(--cmp-surface-success)]/40" : "border-gray-100 hover:border-[var(--cmp-color-success)]"}`}><div className="flex items-center justify-between"><span className="font-semibold text-gray-800">{p.bed ? `Bed ${p.bed}` : p.label}</span>{p.reviewed ? <span className="text-[var(--cmp-text-success)]">✓</span> : <span className="text-amber-500">●</span>}</div><div className="flex items-center justify-between mt-0.5"><span className="text-gray-500 truncate">{p.label}</span><span className={`text-[9px] px-1 rounded ${RISK_BADGE[p.risk]}`}>PEWS {p.pews ?? "—"}</span></div></Link>))}</div>
        </div>
        ...
        <div className={`${card} p-5 xl:col-span-2`}>
          {!selected ? <p className="text-sm text-gray-400 py-8 text-center">No patients in scope.</p> : (<>
            ...
            {selClar.length === 0 ? <p className="text-[11px] text-gray-400">No questions raised. Use the Board to ask the outgoing supervisor.</p> : <div className="space-y-1.5">{selClar.map((c: any) => (<div key={c.id} className="rounded-lg border border-gray-100 p-2"><p className="text-[11px] text-gray-700"><b>Q:</b> {c.question}</p>{c.answer ? <p className="text-[11px] text-emerald-700 mt-0.5"><b>A:</b> {c.answer}</p> : <div className="flex items-center justify-between mt-0.5"><span className="text-[10px] text-[var(--cmp-text-warning)]">Pending</span><AnswerClarification id={c.id} /></div>}</div>))}</div>}
