...
export default function JbiAudit({ patientId, patientLabel }: { patientId: string; patientLabel: string }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between mb-1"><h3 className="text-sm font-bold text-gray-900">JBI Checklist · {patientLabel}</h3><span className={`text-sm font-bold ${pct >= 85 ? "text-[var(--cmp-text-success)]" : pct >= 70 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-error)]"}`}>{pct}% · {cls}</span></div>
      {DOMAINS.map(([k, label], i) => (
        <div key={k} className="flex items-center justify-between gap-2 py-1 border-b border-gray-50">
          ...
          <div className="flex gap-1">{[0, 1, 2, 3, 4, 5].map(n => (<button key={n} onClick={() => setScores({ ...scores, [k]: n })} className={`w-6 h-6 rounded text-[10px] font-semibold ${scores[k] === n ? (n >= 4 ? "bg-[var(--cmp-color-success)] text-white" : n >= 3 ? "bg-[var(--cmp-color-warning)] text-white" : "bg-[var(--cmp-color-error)] text-white") : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}>{n}</button>))}</div>
        </div>
      ))}
      ...
      <div className="flex items-center justify-between"><span className="text-[11px] text-gray-500">Score: <b>{total}/{MAX}</b></span><button onClick={submit} disabled={busy} className="text-xs font-semibold rounded-lg py-2 px-4 bg-[var(--cmp-color-success)] text-white disabled:opacity-50">{busy ? "Recording…" : "Submit Audit"}</button></div>
