  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
        <Kpi label="Handover Status" value={pct === 100 ? "Complete" : accepted.length ? "In Progress" : "Awaiting"} tone={pct === 100 ? "text-[var(--cmp-text-success)]" : "text-[var(--cmp-text-warning)]"} />
        <Kpi label="Patients to Accept" value={toAccept.length} sub={`${highRisk.length} high risk`} tone={toAccept.length ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-success)]"} />
        <Kpi label="Tasks to Acknowledge" value={d.tasks.length} />
        <Kpi label="Accepted" value={accepted.length} sub={`${pct}%`} tone="text-[var(--cmp-text-success)]" />
      ...
      <div className={`${card} p-4`}>
        <div className="flex items-center gap-2 flex-wrap">{STEPS.map(([t, sub], i) => { const stepDone = i === 0 ? rows.some((r: any) => r.reviewed) : i === 1 ? accepted.length > 0 : i === 2 ? items.every(x => x.ok) : pct === 100; return (<div key={t} className="flex items-center gap-2"><span className={`w-6 h-6 rounded-full text-[11px] font-bold flex items-center justify-center ${stepDone ? "bg-[var(--cmp-color-success)] text-white" : "bg-gray-100 text-gray-400"}`}>{i + 1}</span><div><p className={`text-xs font-medium ${stepDone ? "text-gray-800" : "text-gray-400"}`}>{t}</p><p className="text-[10px] text-gray-400">{sub}</p></div>{i < STEPS.length - 1 && <span className="text-gray-300">→</span>}</div>); })}</div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5 xl:col-span-2`}>
          ...
          <div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="text-gray-400 text-left border-b border-gray-100"><th className="py-2 pr-3 font-medium">Patient</th><th className="py-2 pr-3 font-medium">Bed</th><th className="py-2 pr-3 font-medium">Risk</th><th className="py-2 pr-3 font-medium">Status</th><th className="py-2 font-medium">Action</th></tr></thead>
            <tbody>{rows.slice(0, 10).map((p: any) => (<tr key={p.patientId} className="border-b border-gray-50"><td className="py-2 pr-3 text-gray-800 font-medium">{p.label}</td><td className="py-2 pr-3 text-gray-600">{p.bed ?? "—"}</td><td className="py-2 pr-3"><span className={`text-[10px] px-1.5 py-0.5 rounded ${RISK_BADGE[p.risk]}`}>{p.risk}</span></td><td className="py-2 pr-3">{p.accepted ? <span className="text-[var(--cmp-text-success)]">✓ Accepted</span> : p.reviewed ? <span className="text-[var(--cmp-text-warning)]">Reviewed</span> : <span className="text-gray-400">Pending</span>}</td><td className="py-2"><AcceptButton patientId={p.patientId} patientLabel={p.label} accepted={p.accepted} /></td></tr>))}</tbody></table>
        ...
        <div className="space-y-4 xl:col-span-1">
          <div className={`${card} p-5`}>
            ...
            <div className="w-full h-2 rounded-full bg-gray-100 overflow-hidden mb-3"><div className="h-full bg-[var(--cmp-color-success)]" style={{ width: `${pct}%` }} /></div>
            <div className="space-y-1 text-[11px]"><div className="flex justify-between"><span className="text-gray-600">Accepted</span><b className="text-[var(--cmp-text-success)]">{accepted.length}</b></div><div className="flex justify-between"><span className="text-gray-600">Pending</span><b className="text-[var(--cmp-text-warning)]">{toAccept.length}</b></div></div>
          </div>
          <div className={`${card} p-5`}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className={`${card} p-5`}>
          ...
          <div className="space-y-1.5">{items.map(it => (<div key={it.label} className="flex items-center justify-between text-xs"><span className="text-gray-700">{it.label}</span><span className={it.ok ? "text-[var(--cmp-text-success)]" : "text-[var(--cmp-text-warning)]"}>{it.ok ? "✓ Confirmed" : "● Pending"}</span></div>))}</div>
      ...
      <div className="bg-[var(--cmp-surface-success)]/50 border border-[var(--cmp-color-success)] rounded-xl p-3 text-[11px] text-gray-600 flex items-center gap-2"><span>🔒</span>By signing off you confirm you have reviewed the handover and accept responsibility for the patients, tasks and clinical decisions from this point forward. All sign-offs are legally binding and fully auditable.</div>
