      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-4">
        ...
        <div className={`${card} p-4 xl:col-span-1`}>
          ...
          <div className="space-y-1 max-h-[520px] overflow-y-auto">{d.rows.map((p: any) => (
            <Link key={p.patientId} href={`/supervisor/handover/sbar?patient=${p.patientId}`} className={`block rounded-lg border p-2 text-xs ${selected?.patientId === p.patientId ? "border-emerald-400 bg-[var(--cmp-surface-success)]/40" : "border-gray-100 hover:border-[var(--cmp-color-success)]"}`}>
