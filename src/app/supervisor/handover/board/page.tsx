...
const RISK_BAR: Record<string, string> = { "High Risk": "bg-[var(--cmp-color-error)]", "At Risk": "bg-[var(--cmp-color-warning)]", "Stable": "bg-[var(--cmp-color-success)]" };
...
const SBAR_META: [string, string, string][] = [["S", "Situation", "bg-[var(--cmp-color-success)]"], ["B", "Background", "bg-violet-500"], ["A", "Assessment", "bg-[var(--cmp-color-warning)]"], ["R", "Recommendation", "bg-[var(--cmp-color-error)]"]];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
        ...
        <Kpi label="Pending Review" value={counts.pending} tone={counts.pending ? "text-[var(--cmp-text-warning)]" : undefined} />
        <Kpi label="Reviewed" value={counts.reviewed} tone="text-[var(--cmp-text-success)]" />
        <Kpi label="High Risk" value={counts.high} tone={counts.high ? "text-[var(--cmp-text-error)]" : undefined} />
        <Kpi label="Escalations" value={counts.esc} tone={counts.esc ? "text-violet-600" : undefined} />
      </div>

      <div className="flex gap-1 flex-wrap">
        {FILTERS.map(f => <Link key={f} href={`/supervisor/handover/board?filter=${f}${selId ? `&patient=${selId}` : ""}`} className={`text-[11px] px-2.5 py-1 rounded-full ${filter === f ? "bg-[var(--cmp-color-success)] text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}>{f} {f === "All" ? `(${counts.all})` : f === "Pending" ? `(${counts.pending})` : f === "Reviewed" ? `(${counts.reviewed})` : f === "High Risk" ? `(${counts.high})` : `(${counts.esc})`}</Link>)}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="xl:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3 content-start">
          {rows.length === 0 ? <div className={`${card} p-8 text-center sm:col-span-2`}><p className="text-sm text-gray-400">No patients match this filter.</p></div> : rows.slice(0, 12).map((p: any) => (
            <Link key={p.patientId} href={`/supervisor/handover/board?filter=${filter}&patient=${p.patientId}`} className={`${card} p-3.5 hover:border-[var(--cmp-color-success)] transition-colors ${selected?.patientId === p.patientId ? "border-emerald-400 ring-1 ring-emerald-200" : ""}`}>
              ...
              <div className="flex items-center gap-3 mt-2 text-[10px] text-gray-400">
                ...
                <span className="ml-auto">{p.reviewed ? <span className="text-[var(--cmp-text-success)]">✓ Reviewed</span> : <span className="text-[var(--cmp-text-warning)]">● Pending</span>}</span>
              </div>
        ...
        <div className={`${card} p-5 xl:col-span-1`}>
          {!selected ? <div className="text-center py-8"><p className="text-2xl mb-2">🗂️</p><p className="text-sm text-gray-400">No patient selected.</p></div> : (
            <>
              ...
              <p className="text-[10px] font-semibold text-gray-500 uppercase mt-3 mb-1">SBAR Summary {selected.sbarEdited ? <span className="text-[var(--cmp-text-success)] normal-case">· edited</span> : <span className="text-gray-400 normal-case">· auto</span>}</p>
              ...
              {selAlerts.length === 0 ? <p className="text-[11px] text-gray-400">No active alerts.</p> : <div className="space-y-0.5">{selAlerts.slice(0, 3).map((a: any) => <div key={a.id} className="text-[11px] text-[var(--cmp-text-error)] flex gap-1.5"><span>🚩</span>{a.category ?? "Alert"}{a.message ? ` — ${a.message}` : ""}</div>)}</div>}
