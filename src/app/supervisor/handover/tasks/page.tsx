  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <Kpi label="Overdue" value={overdue.length} sub="Require attention" tone={overdue.length ? "text-[var(--cmp-text-error)]" : undefined} />
        <Kpi label="Due Within 2h" value={due2h.length} sub="High priority" tone={due2h.length ? "text-[var(--cmp-text-warning)]" : undefined} />
        <Kpi label="Completed" value={completedToday} sub="Last 12h" tone="text-[var(--cmp-text-success)]" />
        <Kpi label="Assigned to Me" value={mine.length} sub="My responsibility" />
        <Kpi label="Urgent" value={tasks.filter((t: any) => t.priority === "urgent").length} sub="Priority" tone={tasks.some((t: any) => t.priority === "urgent") ? "text-[var(--cmp-text-error)]" : undefined} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
          <div className={`${card} p-5`}><h3 className="text-sm font-bold text-gray-900 mb-3">Task Overview</h3><div className="space-y-2">{byStatus.map(x => (<div key={x.label} className="text-xs"><div className="flex items-center justify-between mb-0.5"><span className="text-gray-700 capitalize">{x.label}</span><span className="text-gray-400">{x.n}</span></div><div className="w-full h-1.5 rounded-full bg-gray-100 overflow-hidden"><div className="h-full rounded-full bg-[var(--cmp-color-success)]" style={{ width: `${tasks.length ? (x.n / tasks.length) * 100 : 0}%` }} /></div></div>))}{overdue.length > 0 && <div className="text-xs"><div className="flex items-center justify-between mb-0.5"><span className="text-[var(--cmp-text-error)]">overdue</span><span className="text-gray-400">{overdue.length}</span></div><div className="w-full h-1.5 rounded-full bg-gray-100 overflow-hidden"><div className="h-full rounded-full bg-[var(--cmp-color-error)]" style={{ width: `${tasks.length ? (overdue.length / tasks.length) * 100 : 0}%` }} /></div></div>}</div></div>
          <div className={`${card} p-5`}><h3 className="text-sm font-bold text-gray-900 mb-3">Overdue Tasks</h3>{overdue.length === 0 ? <p className="text-sm text-gray-400">None overdue. 🎉</p> : <div className="space-y-1.5">{overdue.slice(0, 5).map((t: any) => (<div key={t.id} className="flex items-center gap-2 text-xs"><span className="text-rose-500">⏰</span><span className="text-gray-700 flex-1 truncate">{t.description}</span><span className="text-[var(--cmp-text-error)] whitespace-nowrap">{relDue(t.due_at)}</span></div>))}</div>}</div>
        </div>
      </div>
