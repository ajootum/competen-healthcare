...
export default function BedTurnaroundPanel({ turnaround, cleaningBeds, configReady }: {
  ...
  return (
    <div className={cardClass}>
      ...
      {msg && <div className={`mb-2 text-sm rounded-lg px-3 py-1.5 ${msg.kind === "ok" ? "bg-[var(--cmp-surface-success)] text-green-800" : "bg-[var(--cmp-surface-warning)] text-amber-800"}`}>{msg.text}</div>}

      {!configReady ? (
        <p className="text-[11px] text-[var(--cmp-text-warning)] bg-[var(--cmp-surface-warning)] border border-[var(--cmp-color-warning)] rounded-lg px-3 py-2">Apply migration <code className="bg-[var(--cmp-surface-warning)] px-1 rounded font-mono">049-bed-turnaround.sql</code> to track turnaround stages. Beds in cleaning status: {cleaningBeds.length}.</p>
      ) : (
        <>
          ...
          <ul className="space-y-3">
            {turnaround.map((t: any) => {
              ...
              return (
                <li key={t.id} className="rounded-lg border border-[var(--cmp-color-warning)] bg-[var(--cmp-surface-warning)]/40 px-3 py-2.5">
                  ...
                  <div className="flex items-center gap-1 mt-2">
                    {ORDER.map((stg, i) => (
                      <span key={stg} className="flex items-center gap-1 flex-1 last:flex-none">
                        <span className={`w-2 h-2 rounded-full shrink-0 ${i < idx ? "bg-teal-500" : i === idx ? "bg-[var(--cmp-color-warning)] ring-2 ring-orange-200" : "bg-gray-200"}`} title={LABEL[stg]} />
