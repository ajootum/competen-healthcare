...
const RISK_TONE: Record<string, string> = { High: "text-[var(--cmp-text-error)]", Medium: "text-[var(--cmp-text-warning)]", Low: "text-[var(--cmp-text-success)]", "—": "text-gray-300", Increasing: "text-[var(--cmp-text-warning)]", Stable: "text-gray-600" };
const RISK_BADGE: Record<string, string> = { High: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", Medium: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", Low: "bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)]", Increasing: "bg-[var(--cmp-surface-warning)] text-orange-700", Stable: "bg-gray-100 text-gray-600" };
const PRIO_BADGE: Record<string, string> = { Immediate: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", High: "bg-[var(--cmp-surface-warning)] text-orange-700", Medium: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]" };
const FEED_DOT: Record<string, string> = { high: "bg-[var(--cmp-color-error)]", amber: "bg-[var(--cmp-color-warning)]", blue: "bg-[var(--cmp-color-information)]", green: "bg-[var(--cmp-color-success)]" };
    ...
    <div data-wide className="space-y-4">
      ...
      <div className={`${card} p-5`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-7 gap-3">
          ...
          <div className="rounded-lg border border-gray-100 p-3"><p className="text-[9px] text-gray-400 uppercase">Critical Patients</p><p className="text-lg font-bold text-[var(--cmp-text-error)]">{c.criticalPatients}</p><p className="text-[10px] text-gray-400">Close monitoring</p></div>
        ...
        <div className="xl:col-span-3 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className={`${card} p-5`}>
              ...
              <p className="text-[10px] text-gray-400 uppercase">High Risk Patients</p><p className="text-2xl font-bold text-[var(--cmp-text-error)]">{p.highRisk}</p>
              <div className="mt-2 space-y-1">{metric("Deterioration risk (next 2h)", `${p.deterioration} patients`, p.deterioration ? "text-[var(--cmp-text-error)]" : "text-gray-900")}{metric("PEWS escalation likely", `${p.pewsLikely}`)}{metric("Likely ICU transfer", `${p.icuTransfer}`)}{metric("Ward congestion risk", p.wardCongestion, RISK_TONE[p.wardCongestion])}</div>
            </div>
            <div className={`${card} p-5`}>
              <div className="flex items-center gap-2 mb-3"><span className="w-6 h-6 rounded-lg bg-[var(--cmp-surface-warning)] text-orange-700 flex items-center justify-center text-[11px] font-bold">3</span><h3 className="text-xs font-bold text-gray-900">Safety Intelligence</h3></div>
              ...
              <div className="mt-2 space-y-1">{metric("Observation compliance risk", s.obsComplianceRisk, RISK_TONE[s.obsComplianceRisk])}{metric("Medication error risk", s.medicationRisk, RISK_TONE[s.medicationRisk])}{metric("Falls risk", s.fallsRisk, RISK_TONE[s.fallsRisk])}{metric("Pressure injury risk", s.pressureInjuryRisk, RISK_TONE[s.pressureInjuryRisk])}{metric("Open safety alerts", s.openAlerts, s.openAlerts ? "text-[var(--cmp-text-error)]" : "text-gray-900")}</div>
            </div>
            <div className={`${card} p-5`}>
              <div className="flex items-center gap-2 mb-3"><span className="w-6 h-6 rounded-lg bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)] flex items-center justify-center text-[11px] font-bold">4</span><h3 className="text-xs font-bold text-gray-900">Operational Intelligence</h3></div>
              ...
              <div className="mt-2 space-y-1">{metric("Workflow bottlenecks", o.workflowBottlenecks)}{metric("Task delays", o.taskDelays, o.taskDelays ? "text-[var(--cmp-text-error)]" : "text-gray-900")}{metric("Escalations in progress", o.escalations)}{metric("Communication load", o.commsLoad, RISK_TONE[o.commsLoad])}{metric("Bed utilisation", o.bedUtilisation == null ? "—" : `${o.bedUtilisation}%`)}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className={`${card} p-5`}>
              ...
              {!d.explain ? <p className="text-sm text-gray-400">No high-impact recommendation right now.</p> : (<>
                <div className="flex items-center gap-2 mb-2"><p className="text-xs font-medium text-gray-800 flex-1">{d.explain.recommendation}</p><span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]">{d.explain.impact} Impact</span></div>
            ...
            <div className={`${card} p-5`}>
              <div className="flex items-center gap-2 mb-3"><span className="w-6 h-6 rounded-lg bg-[var(--cmp-surface-information)] text-blue-700 flex items-center justify-center text-[11px] font-bold">8</span><h3 className="text-xs font-bold text-gray-900">AI Insights Feed</h3></div>
      ...
      <div className={`${card} p-4`}>
        ...
        <div className="flex flex-wrap gap-2">{d.agents.map((a: string) => (<div key={a} className="flex items-center gap-1.5 rounded-lg border border-gray-100 bg-gray-50/60 px-2.5 py-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[var(--cmp-color-success)]" /><span className="text-[11px] text-gray-700">{a}</span></div>))}</div>
      </div>
