...
const ACUITY: Record<string, string> = { High: "text-[var(--cmp-text-error)]", Medium: "text-[var(--cmp-text-warning)]", Low: "text-[var(--cmp-text-success)]" };
const ACUITY_DOT: Record<string, string> = { High: "bg-[var(--cmp-color-error)]", Medium: "bg-[var(--cmp-color-warning)]", Low: "bg-[var(--cmp-color-success)]" };
...
export default function AssignmentBoard({ columns, staff, shiftId, showUnassignedOnly }: { columns: Column[]; staff: Staff[]; shiftId: string | null; showUnassignedOnly?: boolean }) {
  ...
  return (
    <div>
      {err && <p className="text-[11px] text-[var(--cmp-text-error)] mb-2">{err}</p>}
              ...
              <div className="space-y-2">
                {shown.map(p => (
                  <div key={p.id} className="rounded-lg border border-gray-200 bg-white p-2.5">
                    ...
                    {p.assigned ? (
                      <div className="mt-1.5 flex items-center justify-between rounded-md bg-[var(--cmp-surface-success)]/60 px-2 py-1.5">
                        <span className="text-[11px] text-gray-700 truncate">👤 {p.assigned.name}{!p.assigned.validated && <span className="text-[var(--cmp-text-warning)]" title="Override — no validated competency"> ⚠</span>}</span>
                        <span className="flex items-center gap-1.5"><button onClick={() => { setOpen(open === p.id ? null : p.id); setPick(""); }} className="text-[10px] text-gray-500 hover:text-emerald-700">Reassign</button><button disabled={busy} onClick={() => unassign(p.assigned!.assignmentId)} className="text-[10px] text-rose-400 hover:text-[var(--cmp-text-error)] disabled:opacity-50" title="Unassign">✕</button></span>
                      </div>
                    ) : (
                      <button onClick={() => { setOpen(open === p.id ? null : p.id); setPick(""); }} className="mt-1.5 w-full text-[11px] font-semibold rounded-md border border-dashed border-[var(--cmp-color-success)] text-emerald-700 py-1.5 hover:bg-[var(--cmp-surface-success)]/50">+ Assign clinician</button>
                    )}
                    {open === p.id && (
                      <div className="mt-1.5 flex items-center gap-1.5">
                        ...
                        <button disabled={!pick || busy} onClick={() => doAssign(p.id)} className="text-[11px] font-semibold rounded bg-[var(--cmp-color-success)] text-white px-2 py-1.5 disabled:opacity-50">{busy ? "…" : "Go"}</button>
                      </div>
                    )}
                  </div>
                ))}
