  ...
  if (done) return (
    <div className={`${card} p-5`}>
      <div className="flex items-start gap-3">
        <span className="w-8 h-8 rounded-full bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)] flex items-center justify-center text-lg shrink-0">✓</span>
        <div>
  ...
  return (
    <div className={`${card} p-5`}>
      <div className="flex items-center justify-between">
        ...
        {!open && <button onClick={() => setOpen(true)} disabled={!ready} className="text-xs font-medium text-white bg-[var(--cmp-color-error)] hover:bg-rose-700 rounded-lg px-3 py-2 disabled:opacity-50 shrink-0">+ Report</button>}
      </div>

      {!ready && <p className="text-[11px] text-[var(--cmp-text-warning)] mt-2">Incident reporting activates once the operations store (migration 073) is provisioned.</p>}
      ...
      {open && (
        <div className="mt-4 space-y-3">
          ...
          {err && <p className="text-xs text-[var(--cmp-text-error)]">{err}</p>}
          <div className="flex items-center justify-end gap-2">
            <button onClick={() => { setOpen(false); setErr(null); }} className="text-xs text-gray-500 hover:underline">Cancel</button>
            <button onClick={submit} disabled={busy || !f.description.trim()} className="text-sm font-medium text-white bg-[var(--cmp-color-error)] hover:bg-rose-700 rounded-lg px-4 py-2 disabled:opacity-50">{busy ? "Submitting…" : "Submit report"}</button>
          </div>
        </div>
      )}
    </div>
