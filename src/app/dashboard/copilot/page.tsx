...
const TONE: Record<string, string> = { violet: "bg-violet-50 border-violet-100", amber: "bg-[var(--cmp-surface-warning)] border-[var(--cmp-color-warning)]", rose: "bg-[var(--cmp-surface-error)] border-[var(--cmp-color-error)]", blue: "bg-[var(--cmp-surface-information)] border-[var(--cmp-color-information)]", emerald: "bg-[var(--cmp-surface-success)] border-[var(--cmp-color-success)]" };
const QUICK = [
  ...
  return (
    <div className="max-w-[1500px] mx-auto space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full font-medium shrink-0 ${ai.configured ? "text-[var(--cmp-text-success)] bg-[var(--cmp-surface-success)]" : "text-gray-500 bg-gray-100"}`}><span className={`w-1.5 h-1.5 rounded-full ${ai.configured ? "bg-[var(--cmp-color-success)] animate-pulse" : "bg-gray-400"}`} />{ai.configured ? "Online" : "Not configured"}</span>
      </div>
      ...
      <div className="grid lg:grid-cols-[240px_minmax(0,1fr)_300px] gap-5 items-start">
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="space-y-1.5">
              {QUICK.map(a => (
                <Link key={a.label} href={`/dashboard/copilot?scenario=${encodeURIComponent(a.prompt)}`} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] text-gray-700 hover:bg-[var(--cmp-surface-information)] hover:text-blue-700"><span>{a.icon}</span>{a.label}</Link>
              ))}
            </div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3"><h3 className="text-sm font-semibold text-gray-900">My Tasks (AI Prioritised)</h3><Link href="/dashboard/tasks" className="text-[11px] font-medium text-[var(--cmp-text-information)] hover:underline">All →</Link></div>
            {d.tasks.length > 0 ? (
              <div className="space-y-2">
                {d.tasks.map((t: any) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
                  <Link key={t.id} href="/dashboard/tasks" className="flex items-center gap-2 group">
                    ...
                    {t.overdue && <span className="text-[10px] font-medium text-[var(--cmp-text-error)] shrink-0">Overdue</span>}
                  </Link>
                ))}
              </div>
            ...
            {d.recommended.length > 0 ? (
              <div className="space-y-2">
                {d.recommended.map((r: any, i: number) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
                  <Link key={i} href={r.href} className="block border border-gray-100 rounded-lg p-2.5 hover:border-[var(--cmp-color-information)] hover:bg-[var(--cmp-surface-information)]/40">
                    <p className="text-[10px] font-semibold text-[var(--cmp-text-information)] uppercase tracking-wide">{r.kind}</p>
