...
const GRADE_LABELS: Record<number, { level: string; color: string }> = {
...
type Tab = "assessment" | "concurrent" | "chart";
type MatchValue = "yes" | "no" | "partial";
type InfoField = { key: string; label: string; placeholder: string };

// ── Helpers ───────────────────────────────────────────────────────────────────
function avgToLevel(avg: number): string {
...
// ── Assessment Audit (Sheet 1) ───────────────────────────────────────────────
const ASSESSMENT_INFO_FIELDS: InfoField[] = [
  { key: "nurse",    label: "Nurse Being Assessed",    placeholder: "Full name" },
  { key: "assessor", label: "Assessor / Supervisor",   placeholder: "Full name" },
  { key: "unit",     label: "Unit / Ward",             placeholder: "e.g. Paediatric ICU" },
  { key: "date",     label: "Date",                    placeholder: "DD/MM/YYYY" },
  { key: "coworker", label: "Co-worker Number",        placeholder: "Staff ID" },
  { key: "patient",  label: "Patient (de-identified)", placeholder: "e.g. Bed 4, Rm 12" },
];
...
function AssessmentAudit() {
  ...
  const setGrade = (num: number, g: number) =>
    setGrades(prev => ({ ...prev, [num]: g }));

  const graded     = Object.keys(grades).length;
  const totalItems = allItems.length;
  const avgScore   = graded > 0 ? Object.values(grades).reduce((a, b) => a + b, 0) / graded : null;
  ...
  const getSectionResult = (section: typeof ASSESSMENT_SECTIONS[0]) => {
    const gi = section.items.filter(i => grades[i.num] !== undefined);
    if (gi.length === 0) return null;
    const avg = gi.reduce((a, i) => a + grades[i.num], 0) / gi.length;
    return { avg, level: avgToLevel(avg), count: gi.length };
  };

  return (
    <div>
      <InfoFields
        fields={ASSESSMENT_INFO_FIELDS}
        values={info}
        onChange={(k, v) => setInfo(prev => ({ ...prev, [k]: v }))}
      />

      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-5 text-sm text-blue-800">
        <strong>How to use:</strong> Observe the nurse performing a full patient assessment. For each item, grade performance 0–6. Each section is averaged to a competency level, then all sections are averaged for the overall level.
      </div>
      ...
      <div className="bg-[#0a2e38] rounded-xl p-5 mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-teal-200 text-sm">{graded} of {totalItems} items graded</p>
          {avgScore !== null && (
            <p className="text-white text-sm mt-0.5">Average: <strong>{avgScore.toFixed(2)} / 6</strong></p>
          )}
        </div>
        <div className="flex flex-col items-end gap-2">
          {overallLevel ? (
            <span className={`text-lg font-bold px-4 py-2 rounded-xl border ${LEVEL_COLORS[overallLevel]}`}>
              {overallLevel}
            </span>
          ) : (
            <span className="text-teal-400/50 text-sm">Grade items to see level</span>
          )}
          <div className="flex gap-2">
            <button
              onClick={() => generateAssessmentReport(info, grades, assessorComments, assesseeComments)}
              disabled={graded === 0}
            ...
            <button onClick={() => setGrades({})} className="text-xs text-teal-400 hover:text-white border border-teal-700 px-3 py-1.5 rounded-lg transition-colors">
              Reset
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {Object.entries(GRADE_LABELS).map(([g, { level, color }]) => (
          <span key={g} className={`text-[10px] font-bold px-2 py-0.5 rounded ${color}`}>{g} — {level}</span>
        ))}
      </div>
            ...
            <div key={section.title} className="bg-white border border-gray-100 rounded-xl overflow-hidden">
              <button onClick={() => toggleSection(section.title)}
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {result ? `${result.count}/${section.items.length} graded` : `${section.items.length} items`}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {result && (
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-lg border ${LEVEL_COLORS[result.level]}`}>
                      {result.avg.toFixed(1)} — {result.level}
                    </span>
                  )}
              ...
              {isOpen && (
                <div className="border-t border-gray-100 divide-y divide-gray-50">
                    ...
                    return (
                      <div key={item.num} className="px-5 py-3 flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {[0, 1, 2, 3, 4, 5, 6].map(n => (
                            <button key={n} onClick={() => setGrade(item.num, n)}
                              className={`w-8 h-8 rounded-lg text-xs font-bold transition-colors ${
                                g === n
                                  ? n <= 1 ? "bg-red-500 text-white"
                                  : n <= 2 ? "bg-amber-400 text-white"
                                  : n <= 4 ? "bg-blue-500 text-white"
                                  : n === 5 ? "bg-teal-500 text-white"
                        ...
                        {g !== undefined && (
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded shrink-0 ${GRADE_LABELS[g].color}`}>
                            {GRADE_LABELS[g].level}
                          </span>
                        )}
                      </div>
                    );
                  ...
                  {/* Section competency level summary row */}
                  {result && (
                    <div className={`px-5 py-3 flex items-center justify-between border-t-2 ${
                      result.level === "Expert"            ? "border-green-200 bg-green-50/60"
                      : result.level === "Proficient"      ? "border-teal-200 bg-teal-50/60"
                      : result.level === "Competent"       ? "border-blue-200 bg-blue-50/60"
                      : result.level === "Advanced Beginner" ? "border-amber-200 bg-amber-50/60"
                      : "border-red-200 bg-red-50/60"
                    }`}>
                      <span className="text-xs font-semibold text-gray-500">Section Competency Level</span>
                      <span className={`text-sm font-bold px-3 py-1 rounded-lg border ${LEVEL_COLORS[result.level]}`}>
                        {result.level} &nbsp;·&nbsp; avg {result.avg.toFixed(2)} / 6
                      </span>
                    </div>
                  )}
                </div>
...
export default function AuditPage() {
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-900">Nursing Assessment Audit Tools</h1>
        <p className="text-gray-400 text-sm mt-0.5">Three standardised audit instruments for clinical competency assessment.</p>
      </div>

      {/* Tabs */}
      <div className="flex flex-col sm:flex-row gap-1 mb-6 bg-gray-100 rounded-xl p-1">
        {([
          { key: "assessment" as Tab, label: "1. Nursing Assessment Audit",  sub: "Assessor grades nurse's patient assessment · 62 items · 0–6 scale" },
          { key: "concurrent" as Tab, label: "2. Concurrent Audit",           sub: "Compare nurse's findings vs assessor's findings · 51 items" },
          { key: "chart"      as Tab, label: "3. Retrospective Chart Audit",  sub: "Review patient file for documentation completeness · 48 items" },
        ]).map(({ key, label, sub }) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex-1 px-4 py-2.5 rounded-lg text-left transition-colors ${
              tab === key ? "bg-white shadow-sm" : "hover:bg-gray-200/50"
            }`}>
            <p className={`text-sm font-semibold ${tab === key ? "text-gray-900" : "text-gray-500"}`}>{label}</p>
            <p className="text-[10px] text-gray-400 mt-0.5">{sub}</p>
          </button>
        ))}
      </div>

      {tab === "assessment" && <AssessmentAudit />}
