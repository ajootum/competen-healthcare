...
// PW-014 WS2 / P3 — the Personal Dashboard's widgets as INDIVIDUALLY ISOLATABLE async server components. Each is
// async so it participates in streaming: wrapped by the page in <WidgetBoundary><Suspense>, a throw in one widget
// is caught and shows a fallback while its siblings render (PW-AC-07). Which widgets appear + their order come
// from the resolved manifest (config, PW-AC-06). Data comes pre-loaded from loadPersonalWorkspace (fail-soft) —
// the JSX is lifted verbatim from the former monolithic page, so the visual output is unchanged.
/* eslint-disable @typescript-eslint/no-explicit-any */

export type WidgetCtx = { firstName: string; greeting: string; now: number; workspaces: any[] };
...
export async function AiBriefingWidget({ d, ctx }: WidgetProps) {
  return (
    <div className="bg-gradient-to-br from-indigo-50 to-violet-50 rounded-xl border border-indigo-100 p-5 h-full">
      <h2 className="font-semibold text-gray-900 text-sm mb-2 flex items-center gap-1.5">✨ AI Shift Briefing</h2>
      <p className="text-xs text-gray-600 mb-2">{ctx.greeting} {ctx.firstName}.</p>
...
export async function PrioritiesWidget({ d }: WidgetProps) {
  return (
    <div className={`${card} p-5 h-full`}>
      ...
      <Link href="/dashboard/tasks" className="text-[10px] text-indigo-600 font-medium mt-2 inline-block">View all tasks →</Link>
    </div>
  );
}

export async function PatientsWidget({ d }: WidgetProps) {
  return (
    <div className={`${card} p-5 h-full`}>
...
export async function TasksWidget({ d, ctx }: WidgetProps) {
  return (
    <div className={`${card} p-5 h-full`}>
...
export async function PerformanceWidget({ d }: WidgetProps) {
  return (
    <div className={`${card} p-5 h-full`}>
...
export async function CompetenciesWidget({ d }: WidgetProps) {
...
export async function ScheduleWidget({ d }: WidgetProps) {
  return (
...
export async function NotificationsWidget({ d }: WidgetProps) {
  return (
    <div className={`${card} p-5`}>
...
export async function MessagesWidget({ d }: WidgetProps) {
  return (
    <div className={`${card} p-5`}>
      <div className="flex items-center justify-between mb-3"><h2 className="font-semibold text-gray-900 text-sm">💬 Messages</h2><Link href="/dashboard/messages" className="text-[10px] text-indigo-600">View All</Link></div>
...
export async function WorkspacesWidget({ ctx }: WidgetProps) {
  ...
  return (
    <div className={`${card} p-5`}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Link href="/dashboard/launcher" className="flex flex-col items-center justify-center gap-1 rounded-lg border border-gray-100 p-2.5 w-24 hover:bg-gray-50 hover:border-indigo-200 transition-colors"><span className="text-lg">➕</span><span className="text-[9px] text-gray-600 text-center leading-tight">All Workspaces</span></Link>
      </div>
    </div>
  );
}
