...
export default async function OperationsPage() {
  ...
  let shifts: any[] = [], shiftStaff: any[] = [], beds: any[] = [], patients: any[] = [], assignments: any[] = [], escalations: any[] = [], alerts: any[] = [];
  if (ready) {
    const [sh, bd, pt, asg, esc, al] = await Promise.all([
    ...
    shifts = sh.data ?? []; beds = bd.data ?? []; patients = pt.data ?? []; assignments = asg.data ?? []; escalations = esc.data ?? []; alerts = al.data ?? [];
  ...
  return (
    <OperationsConsole
      ready={ready}
      data={{ shifts, shiftStaff, beds, patients, assignments, escalations, alerts }}
      support={{ departments: depts.data ?? [], staff: staff.data ?? [] }}
    />
  );
}
