  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className={card}><div className="text-3xl font-bold tabular-nums text-[var(--cmp-text-success)]">{checks.filter(c => c.ok).length}/{checks.length}</div><div className="text-xs text-gray-500 mt-1">Core services healthy</div></div>
      ...
      <div className={card}>
        ...
        <div className="space-y-2">
          {checks.map((c) => (
            <div key={c.name} className={`flex items-center gap-3 border rounded-lg px-4 py-3 ${c.ok ? "bg-[var(--cmp-surface-success)] border-[var(--cmp-color-success)]" : "bg-gray-50 border-gray-200"}`}>
              <span className={`w-2.5 h-2.5 rounded-full ${c.ok ? "bg-[var(--cmp-color-success)]" : "bg-gray-300"}`} />
              ...
              <span className={`text-[10px] font-semibold uppercase tracking-wide shrink-0 ${c.ok ? "text-[var(--cmp-text-success)]" : "text-gray-500"}`}>{c.ok ? "Healthy" : "Off"}</span>
            </div>
          ))}
        </div>
      </div>
