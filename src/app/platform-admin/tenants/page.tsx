  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <Link href="/super-admin/organisations" className="shrink-0 text-sm font-medium text-white bg-[var(--cmp-color-error)] hover:bg-rose-700 rounded-lg px-4 py-2">Provision / manage →</Link>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className={card}><div className="text-3xl font-bold tabular-nums text-[var(--cmp-text-success)]">{summary.activeTenants}</div><div className="text-xs text-gray-500 mt-1">Active</div></div>
        <div className={card}><div className={`text-3xl font-bold tabular-nums ${summary.inactiveTenants ? "text-[var(--cmp-text-warning)]" : "text-gray-900"}`}>{summary.inactiveTenants}</div><div className="text-xs text-gray-500 mt-1">Suspended / inactive</div></div>
      </div>
            ...
            <tbody>
              ...
              {tenants.map((t) => (
                <tr key={t.id} className="border-b last:border-0">
                  ...
                  <td className="pr-3">{t.active ? <span className="text-[10px] bg-[var(--cmp-surface-success)] text-[var(--cmp-text-success)] rounded-full px-2 py-0.5">Active</span> : <span className="text-[10px] bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)] rounded-full px-2 py-0.5">Inactive</span>}</td>
      ...
      <p className="text-[11px] text-gray-400">Tenant lifecycle actions — create, suspend, restore, archive — are performed in <Link href="/super-admin/organisations" className="text-[var(--cmp-text-error)] hover:underline">Organisations</Link>; facilities in <Link href="/super-admin/hospitals" className="text-[var(--cmp-text-error)] hover:underline">All Facilities</Link>.</p>
    </div>
  );
}
