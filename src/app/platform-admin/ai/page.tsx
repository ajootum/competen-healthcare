  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div className={card}><div className={`text-3xl font-bold ${ai.live ? "text-[var(--cmp-text-success)]" : "text-gray-400"}`}>{ai.live ? "Live" : "Off"}</div><div className="text-xs text-gray-500 mt-1">Operational status</div></div>
      ...
      <div className={card}>
        ...
        {ai.live ? (
          <p className="text-sm text-[var(--cmp-text-success)]">✅ AI is operational — a supported model provider (Anthropic) is configured and wired end-to-end. {ai.events30d} AI-assisted actions were recorded in the last 30 days.</p>
        ) : ai.configured ? (
          <p className="text-sm text-[var(--cmp-text-warning)]">⚠️ A provider key is present but not wired end-to-end. AI-assisted features will report as not configured until a supported provider (Anthropic) is set.</p>
        ) : (
        ...
        <p className="text-[11px] text-gray-400 mt-3">Configure the provider and models under <Link href="/super-admin/studio" className="text-[var(--cmp-text-error)] hover:underline">Studio</Link>. Per-tenant AI budgets, model routing and cost governance are a later PSA phase.</p>
      </div>
    </div>
  );
}
