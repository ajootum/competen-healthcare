...
export default async function IntegrationsPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Integration Centre</h1>
        <p className="text-sm text-gray-500 mt-1">Platform engines run natively; external systems connect here as they are provisioned.</p>
      </div>
          ...
          {integrations.map((i) => {
            ...
            return (
              <div key={i.name} className={`flex items-center gap-3 border rounded-lg px-4 py-3 ${b.cls}`}>
                ...
                <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-800">{i.name}</p><p className="text-xs text-gray-500">{i.detail}</p></div>
