  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <select value={staff} onChange={e => setStaff(e.target.value)} className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-[var(--cmp-color-success)]"><option value="">— staff member —</option>{picker.map((p: any) => <option key={p.id} value={p.id}>{p.full_name}</option>)}</select>
        <select value={type} onChange={e => setType(e.target.value)} className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-[var(--cmp-color-success)]"><option value="">— availability —</option>{TYPES.map(t => <option key={t.v} value={t.v}>{t.l}</option>)}</select>
      ...
      <div className="flex items-center gap-2">
        <button disabled={busy} onClick={submit} className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-[var(--cmp-color-success)] text-white hover:bg-emerald-700 disabled:opacity-40">{busy ? "Recording…" : "Record availability"}</button>
        {ok && <span className="text-[11px] text-[var(--cmp-text-success)]">Recorded ✓</span>}
