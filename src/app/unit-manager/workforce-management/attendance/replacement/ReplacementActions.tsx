...
const PRI: Record<string, string> = { critical: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", high: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", normal: "bg-gray-100 text-gray-600", low: "bg-gray-100 text-gray-400" };
const ST: Record<string, string> = { identified: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", candidates_generated: "bg-[var(--cmp-surface-information)] text-[var(--cmp-text-information)]", offered: "bg-[var(--cmp-surface-information)] text-[var(--cmp-text-information)]", accepted: "bg-[var(--cmp-surface-success)] text-emerald-700", filled: "bg-[var(--cmp-surface-success)] text-emerald-700", redeployed: "bg-[var(--cmp-surface-success)] text-emerald-700", declined: "bg-gray-100 text-gray-500", cancelled: "bg-gray-100 text-gray-400", escalated: "bg-[var(--cmp-surface-warning)] text-orange-700" };
...
export default function ReplacementActions({ gaps, open, pool }: { gaps: any[]; open: any[]; pool: any[] }) {
  ...
  return (
    <div className="space-y-4">
      {err && <div className="text-xs text-[var(--cmp-text-error)] bg-[var(--cmp-surface-error)] border border-[var(--cmp-color-error)] rounded-lg px-3 py-2">{err}</div>}
      ...
      {gaps.filter((g: any) => !g.hasRequest).length > 0 && (
        <div>
          ...
          <div className="space-y-1.5">{gaps.filter((g: any) => !g.hasRequest).map((g: any) => (
            <div key={g.id} className="flex items-center justify-between rounded-lg border border-[var(--cmp-color-error)] bg-[var(--cmp-surface-error)]/30 p-2.5">
              ...
              <button disabled={busy === g.id} onClick={() => raise(g)} className="text-[10px] font-semibold px-2.5 py-1 rounded border border-[var(--cmp-color-error)] text-[var(--cmp-text-error)] hover:bg-[var(--cmp-surface-error)] disabled:opacity-40">Raise replacement</button>
            </div>))}</div>
        </div>
      )}
          ...
          <div className="overflow-x-auto"><table className="w-full text-xs">
            ...
            <tbody>{open.map((r: any) => { const b = busy === r.id; return (
              <tr key={r.id} className="border-b border-gray-50">
                ...
                <td className="py-2 pr-3"><select value={pick[r.id] ?? ""} onChange={e => setPick(p => ({ ...p, [r.id]: e.target.value }))} className="text-[11px] border border-gray-200 rounded px-1.5 py-1 max-w-[9rem] focus:outline-none focus:border-[var(--cmp-color-success)]"><option value="">— candidate —</option>{pool.map((p: any) => <option key={p.id} value={p.id}>{p.full_name} ({ROLE_LABEL[p.role] ?? p.role})</option>)}</select></td>
                <td className="py-2"><div className="flex gap-1"><button disabled={b || !pick[r.id]} onClick={() => progress(r.id, "fill")} className="text-[10px] px-2 py-1 rounded border border-[var(--cmp-color-success)] text-emerald-700 hover:bg-[var(--cmp-surface-success)] disabled:opacity-40">Fill</button><button disabled={b} onClick={() => progress(r.id, "cancel")} className="text-[10px] px-2 py-1 rounded border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40">Cancel</button></div></td>
              </tr>); })}</tbody>
          </table></div>
        )}
      </div>
