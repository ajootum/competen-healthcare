  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
        <Kpi label="Overall coverage" value={covPct != null ? `${covPct}%` : "—"} sub={covPct != null ? (covPct >= 90 ? "Good" : covPct >= 75 ? "Fair" : "Low") : "n/a"} icon="📈" tone={covPct != null && covPct >= 90 ? "text-[var(--cmp-text-success)]" : covPct != null && covPct >= 75 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-error)]"} />
        <Kpi label="Total staff on shift" value={`${ov.present}/${ov.planned}`} sub="Filled positions" icon="👥" />
        <Kpi label="Under-staffed roles" value={understaffed} sub="Require action" icon="🧑‍⚕️" tone={understaffed ? "text-[var(--cmp-text-error)]" : "text-[var(--cmp-text-success)]"} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div className={`${card} p-5`}>
          ...
          <div className="overflow-x-auto"><table className="w-full text-xs">
            ...
            <tbody>{rows.map((r: any) => (<tr key={r.role} className="border-b border-gray-50"><td className="py-2 pr-3 text-gray-700">{r.label}</td><td className="py-2 pr-3 text-right text-gray-600">{r.required ?? "—"}</td><td className="py-2 pr-3 text-right text-gray-600">{r.planned}</td><td className="py-2 pr-3 text-right text-gray-600">{r.present}</td><td className="py-2 pr-3 text-right font-semibold">{r.coverage != null ? `${r.coverage}%` : "—"}</td><td className={`py-2 pr-3 text-right ${r.variance != null && r.variance < 0 ? "text-[var(--cmp-text-error)] font-semibold" : "text-gray-500"}`}>{r.variance ?? "—"}</td><td className="py-2 text-right"><span className={`text-[9px] px-1.5 py-0.5 rounded ${statusLabel(r) === "Adequate" ? "bg-[var(--cmp-surface-success)] text-emerald-700" : "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]"}`}>{statusLabel(r)}</span></td></tr>))}</tbody>
            <tfoot><tr className="border-t border-gray-200 font-bold"><td className="py-2 pr-3 text-gray-800">Total</td><td className="py-2 pr-3 text-right">{ov.required ?? "—"}</td><td className="py-2 pr-3 text-right">{ov.planned}</td><td className="py-2 pr-3 text-right">{ov.present}</td><td className="py-2 pr-3 text-right text-[var(--cmp-text-success)]">{covPct != null ? `${covPct}%` : "—"}</td><td className={`py-2 pr-3 text-right ${ov.variance != null && ov.variance < 0 ? "text-[var(--cmp-text-error)]" : ""}`}>{ov.variance ?? "—"}</td><td className="py-2 text-right"><span className={`text-[9px] px-1.5 py-0.5 rounded ${covPct != null && covPct >= 85 ? "bg-[var(--cmp-surface-success)] text-emerald-700" : "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]"}`}>{covPct != null && covPct >= 85 ? "Good" : "Review"}</span></td></tr></tfoot>
          </table></div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5 xl:col-span-1`}>
          ...
          {alerts.length === 0 ? <p className="text-sm text-gray-400">No staffing alerts. 🎉</p> : <div className="space-y-2">{alerts.slice(0, 6).map((a, i) => (<div key={i} className="flex items-start gap-2.5"><span className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm shrink-0 ${a.tone === "rose" ? "bg-[var(--cmp-surface-error)]" : a.tone === "amber" ? "bg-[var(--cmp-surface-warning)]" : "bg-gray-50"}`}>{a.icon}</span><div><p className="text-xs font-semibold text-gray-800">{a.title}</p><p className="text-[11px] text-gray-500">{a.sub}</p></div></div>))}</div>}
        </div>
      </div>
