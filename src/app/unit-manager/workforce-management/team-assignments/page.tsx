...
const WL: Record<string, string> = { High: "bg-[var(--cmp-color-error)]", Medium: "bg-[var(--cmp-color-warning)]", Good: "bg-[var(--cmp-color-success)]" };
const WL_BADGE: Record<string, string> = { High: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", Medium: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", Good: "bg-[var(--cmp-surface-success)] text-emerald-700", "At risk": "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]" };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <Kpi label="Overall coverage" value={k.overallCoverage != null ? `${k.overallCoverage}%` : "—"} sub={k.overallCoverage != null ? (k.overallCoverage >= 90 ? "Good" : "Review") : "n/a"} icon="🛡️" tone={k.overallCoverage != null && k.overallCoverage >= 90 ? "text-[var(--cmp-text-success)]" : undefined} />
        ...
        <Kpi label="High acuity" value={k.highAcuity} sub={`${k.highAcuityNeedReview} need review`} icon="❤️" tone={k.highAcuity ? "text-[var(--cmp-text-error)]" : undefined} />
        <Kpi label="Unassigned patients" value={k.unassigned} sub="Require action" icon="🧑‍🤝‍🧑" tone={k.unassigned ? "text-[var(--cmp-text-error)]" : "text-[var(--cmp-text-success)]"} />
        <Kpi label="Critical alerts" value={k.criticalAlerts} sub="Require attention" icon="🔔" tone={k.criticalAlerts ? "text-[var(--cmp-text-error)]" : undefined} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5`}>
          ...
          {d.overrides.length === 0 ? <p className="text-sm text-gray-400">No assignment overrides recorded.</p> : <div className="space-y-1.5">{d.overrides.map((o: any, i: number) => (<div key={i} className="text-xs rounded-lg border border-gray-100 p-2"><div className="flex items-center justify-between"><span className="text-gray-800 font-medium">{o.patient} · {o.staff}</span>{o.today && <span className="text-[9px] text-[var(--cmp-text-warning)]">Today</span>}</div><p className="text-[11px] text-gray-500 truncate">{o.reason}</p></div>))}</div>}
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className={`${card} p-5`}>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div><p className="text-[10px] text-gray-500 uppercase">Compliance rate</p><p className={`text-xl font-bold ${d.policy.complianceRate != null && d.policy.complianceRate >= 90 ? "text-[var(--cmp-text-success)]" : "text-[var(--cmp-text-warning)]"}`}>{d.policy.complianceRate != null ? `${d.policy.complianceRate}%` : "—"}</p><div className="w-full h-1.5 rounded-full bg-gray-100 overflow-hidden mt-1"><div className="h-full bg-[var(--cmp-color-success)] rounded-full" style={{ width: `${d.policy.complianceRate ?? 0}%` }} /></div></div>
            ...
            <div><p className="text-[10px] text-gray-500 uppercase">Violations</p><p className={`text-xl font-bold ${d.policy.violations ? "text-[var(--cmp-text-error)]" : "text-[var(--cmp-text-success)]"}`}>{d.policy.violations}</p><p className="text-[10px] text-gray-400">Unvalidated · high-acuity</p></div>
          </div>
