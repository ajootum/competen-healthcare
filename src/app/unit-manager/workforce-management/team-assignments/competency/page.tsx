...
export default async function CompetencyMatching() {
  ...
  if (!d.ready) return <div className="space-y-4">{header}<div className="bg-[var(--cmp-surface-warning)] border border-[var(--cmp-color-warning)] rounded-xl p-6"><p className="font-semibold text-amber-900">⚙️ No operational data</p><p className="text-sm text-amber-800 mt-1">Competency matching activates once operational assignments are running.</p></div></div>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3">
        <Kpi label="Match rate" value={k.matchRate != null ? `${k.matchRate}%` : "—"} sub="Validated ÷ requiring" tone={k.matchRate != null && k.matchRate >= 90 ? "text-[var(--cmp-text-success)]" : k.matchRate != null && k.matchRate >= 75 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-error)]"} />
        <Kpi label="Critical gaps" value={k.criticalGaps} sub="Gap on high-acuity" tone={k.criticalGaps ? "text-[var(--cmp-text-error)]" : "text-[var(--cmp-text-success)]"} />
        <Kpi label="Competency gaps" value={k.gaps} sub="Validated = false" tone={k.gaps ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-success)]"} />
        <Kpi label="Unknown" value={k.unknown} sub="No validation record" tone={k.unknown ? "text-[var(--cmp-text-warning)]" : undefined} />
        ...
        <div className={`${card} p-5 xl:col-span-3`}>
          ...
          {d.byWard.length === 0 ? <p className="text-sm text-gray-400">No active assignments requiring competency match yet.</p> : (
            <div className="overflow-x-auto"><table className="w-full text-xs">
              ...
              <tbody>{d.byWard.map((w: any) => (<tr key={w.ward} className="border-b border-gray-50"><td className="py-2 pr-3 text-gray-700">{w.ward}</td><td className="py-2 pr-3 text-right text-gray-600">{w.total}</td><td className="py-2 pr-3 text-right text-[var(--cmp-text-success)] font-semibold">{w.validated}</td><td className={`py-2 pr-3 text-right ${w.gap ? "text-[var(--cmp-text-error)] font-semibold" : "text-gray-400"}`}>{w.gap || "—"}</td><td className={`py-2 pr-3 text-right ${w.unknown ? "text-[var(--cmp-text-warning)]" : "text-gray-400"}`}>{w.unknown || "—"}</td><td className="py-2 pr-3 text-right font-semibold">{w.pct != null ? `${w.pct}%` : "—"}</td><td className="py-2"><span className={`text-[9px] px-1.5 py-0.5 rounded ${ST[w.status]}`}>{w.status}</span></td></tr>))}</tbody>
            </table></div>
          )}
        ...
        <div className="space-y-4 xl:col-span-2">
          ...
          <div className={`${card} p-5`}>
            ...
            <div className="mt-2.5 space-y-1.5">
              <Link href="/unit-manager/scheduling-engine/competency-matching" className="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2 hover:border-[var(--cmp-color-success)] hover:bg-[var(--cmp-surface-success)]/30"><span className="text-xs text-gray-700">Competency Matching Engine</span><span className="text-gray-300">↗</span></Link>
              <Link href="/unit-manager/competency-validations" className="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2 hover:border-[var(--cmp-color-success)] hover:bg-[var(--cmp-surface-success)]/30"><span className="text-xs text-gray-700">Competency Validations</span><span className="text-gray-300">↗</span></Link>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`${card} p-5`}>
        ...
        {d.gapQueue.length === 0 ? <p className="text-sm text-gray-400">No competency gaps — every active assignment is validated. 🎉</p> : (
          <div className="overflow-x-auto"><table className="w-full text-xs">
            ...
            <tbody>{d.gapQueue.map((g: any, i: number) => (<tr key={i} className="border-b border-gray-50"><td className="py-2 pr-3 text-gray-800 font-medium">{g.patient}</td><td className="py-2 pr-3 text-gray-600">{g.staff}</td><td className="py-2 pr-3 text-gray-600">{g.ward}</td><td className={`py-2 pr-3 capitalize ${["critical", "high"].includes(g.acuity) ? "text-[var(--cmp-text-error)]" : "text-gray-500"}`}>{g.acuity}</td><td className="py-2 pr-3"><span className="text-[10px] text-gray-500">{g.kind === "gap" ? "Validation failed" : "No record"}</span>{g.reason && <span className="block text-[10px] text-[var(--cmp-text-warning)]">override: {g.reason}</span>}</td><td className="py-2"><span className={`text-[9px] px-1.5 py-0.5 rounded ${GAP_SEV[g.severity]}`}>{g.severity}</span></td></tr>))}</tbody>
          </table></div>
        )}
