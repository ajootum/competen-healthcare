...
const ST: Record<string, string> = { Pass: "bg-[var(--cmp-surface-success)] text-emerald-700", Warning: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", Override: "bg-[var(--cmp-surface-warning)] text-orange-700", Blocked: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]" };
const SEV: Record<string, string> = { Pass: "text-gray-400", Medium: "text-[var(--cmp-text-information)]", High: "text-[var(--cmp-text-warning)]", Critical: "text-[var(--cmp-text-error)]" };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3">
        <Kpi label="Compliance score" value={`${k.complianceScore}%`} tone={k.complianceScore >= 95 ? "text-[var(--cmp-text-success)]" : k.complianceScore >= 80 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-error)]"} />
        <Kpi label="Critical (blocking)" value={k.critical} tone={k.critical ? "text-[var(--cmp-text-error)]" : "text-[var(--cmp-text-success)]"} />
        <Kpi label="Warnings" value={k.warnings} tone={k.warnings ? "text-[var(--cmp-text-warning)]" : undefined} />
        <Kpi label="Blocked posts" value={k.blocked} tone={k.blocked ? "text-[var(--cmp-text-error)]" : "text-[var(--cmp-text-success)]"} />
        <Kpi label="Override requests" value={k.overrideRequests} tone={k.overrideRequests ? "text-[var(--cmp-text-warning)]" : undefined} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="space-y-4 xl:col-span-1">
          <div className={`${card} p-5`}>
            ...
            {d.byUnit.length === 0 ? <p className="text-sm text-gray-400">No units.</p> : <div className="space-y-2">{d.byUnit.map((u: any) => (<div key={u.unit} className="flex items-center gap-2 text-xs"><span className="text-gray-600 w-24 truncate">{u.unit}</span><div className="flex-1 h-1.5 rounded-full bg-gray-100 overflow-hidden"><div className={`h-full rounded-full ${u.pct >= 95 ? "bg-[var(--cmp-color-success)]" : u.pct >= 80 ? "bg-[var(--cmp-color-warning)]" : "bg-[var(--cmp-color-error)]"}`} style={{ width: `${u.pct}%` }} /></div><span className="text-gray-600 w-8 text-right font-medium">{u.pct}%</span></div>))}</div>}
          </div>
          <div className={`${card} p-5`}>
