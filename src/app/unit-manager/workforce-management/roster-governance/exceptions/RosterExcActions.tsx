...
const ST: Record<string, string> = { detected: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", under_review: "bg-[var(--cmp-surface-information)] text-[var(--cmp-text-information)]", correction_proposed: "bg-[var(--cmp-surface-information)] text-[var(--cmp-text-information)]", awaiting_approval: "bg-violet-50 text-violet-700", accepted_with_mitigation: "bg-[var(--cmp-surface-warning)] text-orange-700", resolved: "bg-[var(--cmp-surface-success)] text-emerald-700", rejected: "bg-gray-100 text-gray-500", reopened: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]" };
    ...
    <div>
      ...
      <div className="space-y-1.5">{derived.map((d: any) => (
        <div key={d.rule} className="flex items-center justify-between rounded-lg border border-gray-100 p-2.5">
          ...
          <button disabled={busy === d.rule} onClick={() => raise(d)} className="shrink-0 text-[10px] font-semibold px-2.5 py-1 rounded border border-[var(--cmp-color-success)] text-emerald-700 hover:bg-[var(--cmp-surface-success)] disabled:opacity-40">Raise</button>
        </div>))}</div>
    </div>
  );
}
          ...
          <tr key={e.id} className="border-b border-gray-50 align-top">
            ...
            <td className="py-2"><div className="flex flex-col gap-1">
              <div className="flex gap-1 flex-wrap">
                {e.status === "detected" && <button disabled={b} onClick={() => progress(e.id, "review")} className="text-[10px] px-2 py-1 rounded border border-[var(--cmp-color-information)] text-[var(--cmp-text-information)] hover:bg-[var(--cmp-surface-information)] disabled:opacity-40">Review</button>}
                ...
                <button disabled={b} onClick={() => progress(e.id, "override")} className="text-[10px] px-2 py-1 rounded border border-[var(--cmp-color-warning)] text-orange-700 hover:bg-[var(--cmp-surface-warning)] disabled:opacity-40">Override</button>
              ...
              <input value={reason[e.id] ?? ""} onChange={ev => setReason(r => ({ ...r, [e.id]: ev.target.value }))} placeholder="override reason" className="text-[10px] border border-gray-200 rounded px-1.5 py-1 w-40 focus:outline-none focus:border-[var(--cmp-color-warning)]" />
            </div></td>
          </tr>); })}</tbody>
      </table></div>
    </div>
