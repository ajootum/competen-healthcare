...
export default async function RosterReview() {
  ...
  if (!rw.provisioned) return <div className="space-y-4">{header}<div className="bg-[var(--cmp-surface-warning)] border border-[var(--cmp-color-warning)] rounded-xl p-6"><p className="font-semibold text-amber-900">⚙️ Roster store not provisioned</p></div></div>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`${card} p-4 flex items-center gap-4 flex-wrap text-xs`}>
        ...
        <span className="text-gray-600">Status <span className={`px-1.5 py-0.5 rounded ${rw.roster.status === "published" ? "bg-[var(--cmp-surface-success)] text-emerald-700" : "bg-gray-100 text-gray-600"}`}>{rw.roster.status}</span></span>
      ...
      <div className={`${card} p-5`}>
        ...
        <div className="flex items-center gap-4 flex-wrap text-[11px] text-gray-500 mb-2">
          <span className="inline-flex items-center gap-1"><span className="w-4 h-4 rounded-full bg-[var(--cmp-surface-success)] ring-1 ring-emerald-200" /> validated</span>
          <span className="inline-flex items-center gap-1"><span className="w-4 h-4 rounded-full bg-[var(--cmp-surface-warning)] ring-1 ring-amber-200" /> unvalidated competency</span>
