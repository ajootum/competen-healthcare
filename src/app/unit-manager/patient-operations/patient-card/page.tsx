      ...
      {!selected ? (
        <div className={`${card} p-5`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {po.active.map((p: any) => (
              <Link key={p.id} href={`/unit-manager/patient-operations/patient-card?patient=${p.id}`} className="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2.5 hover:border-[var(--cmp-color-success)] hover:bg-[var(--cmp-surface-success)]/30 transition-colors">
        ...
        <>
          ...
          <div className={`${card} p-5`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 mt-4">
              ...
              <Field label="Assigned nurse" value={selected.nurse ? firstName(selected.nurse) ?? "—" : "Unassigned"} tone={selected.nurse ? "" : "text-[var(--cmp-text-error)]"} />
              <Field label="Last obs" value={fmtTime(selected.lastObs)} />
              <Field label="Next review" value={fmtTime(selected.nextReview)} tone={selected.overdueObs ? "text-[var(--cmp-text-error)]" : ""} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className={`${card} p-5`}>
              ...
              {(selected.alerts.length === 0 && selected.escalations.length === 0) ? <p className="text-sm text-gray-400">No open alerts or escalations.</p> : (
                <div className="space-y-1.5">
                  {selected.alerts.map((a: any) => <div key={a.id} className="flex items-center gap-2 text-xs"><span className="w-1.5 h-1.5 rounded-full bg-[var(--cmp-color-warning)]" /><span className="text-gray-700">{titleCase(a.category)}</span><span className="text-gray-400 ml-auto">{a.severity}</span></div>)}
                  {selected.escalations.map((e: any) => <div key={e.id} className="flex items-center gap-2 text-xs"><span className="w-1.5 h-1.5 rounded-full bg-[var(--cmp-color-error)]" /><span className="text-gray-700">Escalation L{e.level}</span><span className="text-gray-400 ml-auto">{e.status}</span></div>)}
                </div>
              )}
            </div>
            ...
            <div className={`${card} p-5 flex flex-col`}>
              ...
              <div className="flex flex-col gap-2 mt-3">
                <Link href={`/supervisor/patient-card/${selected.id}`} className="text-sm rounded-lg bg-[var(--cmp-color-success)] text-white px-3.5 py-2 text-center hover:bg-emerald-700 transition-colors">Open full card →</Link>
                <Link href="/unit-manager/patient-operations/timeline" className="text-sm rounded-lg border border-gray-200 text-gray-700 px-3.5 py-2 text-center hover:border-[var(--cmp-color-success)] hover:text-emerald-700 transition-colors">Unit timeline</Link>
              </div>
            </div>
          </div>
        </>
