...
const STATE_BADGE: Record<string, string> = { "Fully Covered": "bg-[var(--cmp-surface-success)] text-emerald-700", "At Risk": "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", "Uncovered": "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", "—": "bg-gray-100 text-gray-500" };
...
const TAG: Record<string, string> = { "High Impact": "bg-[var(--cmp-surface-success)] text-emerald-700", Cost: "bg-[var(--cmp-surface-information)] text-blue-700", Supervisor: "bg-violet-50 text-violet-700", Risk: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", OK: "bg-gray-100 text-gray-500" };
  ...
  const header = (
    <>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2"><span className="flex items-center gap-1.5 text-[11px] text-emerald-700"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-success)]" />Online</span><UnitFilters departments={departments} /><RosterControls week={weekStart} roster={roster} /></div>
      </div>
      <SchedulingTabs />
    </>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 xl:grid-cols-5 gap-3">
        <div className={`${card} p-4`}><p className="text-xs font-semibold text-gray-700 mb-2">Coverage summary</p><div className="flex items-center gap-3"><Ring pct={c.score} label="Coverage" tone="#10b981" /><div className="text-[11px] space-y-0.5"><div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-success)]" />Fully covered <b>{c.fullyCovered}</b></div><div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-warning)]" />At risk <b>{c.atRisk}</b></div><div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-error)]" />Uncovered <b>{c.uncovered}</b></div></div></div></div>
        <div className={`${card} p-4`}><p className="text-xs font-semibold text-gray-700 mb-2">Required vs assigned</p><div className="flex items-center justify-around"><div className="text-center"><p className="text-2xl font-bold text-gray-900">{dm.required}</p><p className="text-[10px] text-gray-400">Required</p></div><div className="text-center"><p className="text-2xl font-bold text-gray-900">{dm.assigned}</p><p className="text-[10px] text-gray-400">Assigned</p></div></div><p className={`text-center text-xs mt-1 ${dm.variance < 0 ? "text-[var(--cmp-text-error)]" : "text-[var(--cmp-text-success)]"}`}>Variance {dm.variance > 0 ? "+" : ""}{dm.variance}</p></div>
        <div className={`${card} p-4`}><p className="text-xs font-semibold text-gray-700 mb-2">Competency match</p><div className="flex items-center gap-3"><Ring pct={cm.score} label="Match" tone="#8b5cf6" /><div className="text-[11px] space-y-0.5"><div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-success)]" />Full <b>{cm.full}</b></div><div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-warning)]" />Override <b>{cm.partial}</b></div><div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-error)]" />No match <b>{cm.none}</b></div></div></div></div>
        ...
        <div className={`${card} p-4`}><p className="text-xs font-semibold text-gray-700 mb-2">Fairness index</p><div className="flex items-center gap-3"><Ring pct={f.score} label="Fairness" tone="#3b82f6" /><div className="text-[11px] space-y-0.5"><div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-success)]" />Balanced <b>{f.balanced}</b></div><div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-warning)]" />High load <b>{f.highLoad}</b></div><div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--cmp-color-error)]" />Over limit <b>{f.overLimit}</b></div></div></div></div>
      </div>
        ...
        <div className={`${card} p-5 xl:col-span-2`}>
          ...
          {d.demandByUnit.length === 0 ? <p className="text-sm text-gray-400">No unit demand computed.</p> : (
            <div className="overflow-x-auto"><table className="w-full text-xs">
              ...
              <tbody>{d.demandByUnit.map((u: any) => (<tr key={u.unit} className="border-b border-gray-50"><td className="py-2 pr-3 text-gray-700">{u.unit}</td><td className="py-2 pr-3 text-right text-gray-600">{u.requiredPerShift}</td><td className="py-2 pr-3 text-right text-gray-600">{u.assigned}</td><td className={`py-2 pr-3 text-right ${u.variance < 0 ? "text-[var(--cmp-text-error)] font-semibold" : "text-gray-500"}`}>{u.variance > 0 ? "+" : ""}{u.variance}</td><td className="py-2 pr-3 text-right font-semibold">{u.coverage != null ? `${u.coverage}%` : "—"}</td><td className="py-2 text-right"><span className={`text-[9px] px-1.5 py-0.5 rounded ${STATE_BADGE[u.state]}`}>{u.state}</span></td></tr>))}</tbody>
              <tfoot><tr className="border-t border-gray-200 font-bold"><td className="py-2 pr-3 text-gray-800">Total</td><td className="py-2 pr-3 text-right">{dm.required}</td><td className="py-2 pr-3 text-right">{dm.assigned}</td><td className={`py-2 pr-3 text-right ${dm.variance < 0 ? "text-[var(--cmp-text-error)]" : ""}`}>{dm.variance > 0 ? "+" : ""}{dm.variance}</td><td className="py-2 pr-3 text-right text-[var(--cmp-text-success)]">{c.score != null ? `${c.score}%` : "—"}</td><td /></tr></tfoot>
            </table></div>
          )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5 xl:col-span-2`}>
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <h3 className="text-sm font-bold text-gray-900">Draft roster preview {roster && <span className={`text-[10px] px-1.5 py-0.5 rounded ${roster.status === "published" ? "bg-[var(--cmp-surface-success)] text-emerald-700" : "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]"}`}>{roster.status === "published" ? "Published" : "Draft"}</span>}</h3>
            {roster && <div className="flex gap-1"><Link href="/unit-manager/scheduling-engine" className={`text-[10px] px-2 py-1 rounded-full ${rshift === "day" ? "bg-[var(--cmp-color-success)] text-white" : "bg-gray-100 text-gray-500"}`}>Day (07–19)</Link><Link href="/unit-manager/scheduling-engine?rshift=night" className={`text-[10px] px-2 py-1 rounded-full ${rshift === "night" ? "bg-[var(--cmp-color-success)] text-white" : "bg-gray-100 text-gray-500"}`}>Night (19–07)</Link></div>}
          </div>
          {!rosterData?.provisioned ? (
              ...
              <tbody>{gridRows.map((row: any, i: number) => (
                <tr key={i} className="border-b border-gray-50">
                  ...
                  {row.cells.map((cell: any, j: number) => {
                    const dot = cell.required === 0 ? "bg-gray-200" : cell.filled >= cell.required ? "bg-[var(--cmp-color-success)]" : cell.filled > 0 ? "bg-[var(--cmp-color-warning)]" : "bg-[var(--cmp-color-error)]";
            ...
            <div className="flex items-center gap-3 mt-2 pt-2 border-t border-gray-100 text-[10px] text-gray-400"><span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-[var(--cmp-color-success)]" />Covered</span><span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-[var(--cmp-color-warning)]" />Partial</span><span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-[var(--cmp-color-error)]" />Uncovered</span><span>* = override (no validated competency)</span><span className="ml-auto">{roster.slots_filled}/{roster.slots_total} posts · {roster.coverage_score}% cover · {roster.competency_score ?? "—"}% competency · <b className="text-gray-700">Quality {rosterQuality}%</b></span></div>
            </div>
          )}
        </div>
          ...
          <div className="overflow-x-auto"><table className="w-full text-[11px]">
            ...
            <tbody>{individualSchedules.slice(0, 15).map((s: any, i: number) => (
              <tr key={i} className="border-b border-gray-50">
                ...
                <td className={`py-1.5 pr-2 text-center font-semibold ${s.count > 4 ? "text-[var(--cmp-text-error)]" : "text-gray-700"}`}>{s.count}</td>
                {rDays.map(dt => { const sh = s.shifts.find((x: any) => x.date === dt); return <td key={dt} className="py-1.5 px-1 text-center">{sh ? <span className={`inline-block w-4 h-4 rounded text-[8px] leading-4 font-bold text-white ${sh.shift === "day" ? "bg-[var(--cmp-color-warning)]" : "bg-indigo-500"}`} title={`${sh.shift} · ${sh.unit}`}>{sh.shift === "day" ? "D" : "N"}</span> : <span className="text-gray-200">·</span>}</td>; })}
              </tr>
            ))}</tbody>
          </table>
          {individualSchedules.length > 15 && <p className="text-[10px] text-gray-400 mt-1">Showing 15 of {individualSchedules.length}.</p>}
          <div className="flex items-center gap-3 mt-2 text-[10px] text-gray-400"><span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-[var(--cmp-color-warning)] inline-block" />Day</span><span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-indigo-500 inline-block" />Night</span><span>Max 4 shifts/week (48h) — over-limit flagged red.</span></div>
          </div>
        </div>
      )}
