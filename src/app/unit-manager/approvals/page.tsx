    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5 xl:col-span-2`}>
          <div className="flex items-center justify-between mb-3"><h3 className="text-sm font-bold text-gray-900">Smart Approval Queue</h3><div className="flex gap-1">{["All", "High Priority", "Due Today", "Overdue"].map((f, i) => <span key={f} className={`text-[10px] px-2 py-0.5 rounded-full ${i === 0 ? "bg-teal-700 text-white" : "bg-gray-100 text-gray-500"}`}>{f}</span>)}</div></div>
          {d.queue.length === 0 ? (
