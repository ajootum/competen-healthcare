  ...
  return (
    <div className="max-w-[1500px] space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
        <div className="bg-white border border-gray-200 rounded-xl p-3.5 flex items-center gap-2"><Ring pct={k.avgConfidence} size={56} /><div><p className="text-[11px] text-gray-500 uppercase tracking-wide leading-tight">AI Confidence</p><p className="text-[11px] text-[var(--cmp-text-information)] font-medium">avg</p></div></div>
        <Kpi label="Recommendations" value={k.recommendations} sub={`${k.highImpact} high impact`} tone="text-[var(--cmp-text-information)]" />
        <Kpi label="Acceptance Rate" value={`${k.acceptanceRate}%`} sub="accepted" />
        <Kpi label="Active Automations" value={k.activeAutomations} sub="running" tone="text-[var(--cmp-text-success)]" />
        <Kpi label="Automation Runs" value={k.totalRuns.toLocaleString()} sub="this month" />
        <Kpi label="High Impact" value={k.highImpact} sub="recommendations" tone={k.highImpact ? "text-[var(--cmp-text-error)]" : undefined} />
        <Kpi label="AI Health" value={`${k.avgConfidence}%`} sub="composite" tone="text-[var(--cmp-text-information)]" />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Card title="Top AI Recommendations" className="xl:col-span-2" right={<span className="text-[11px] text-gray-400">explainable</span>}>
          {d.recommendations.length ? <div className="space-y-2">{d.recommendations.map((r: any, i: number) => (
            <div key={i} className="flex items-start gap-3 border border-gray-100 rounded-lg p-3">
              <span className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-sm ${r.impact === "high" ? "bg-[var(--cmp-surface-error)]" : r.impact === "medium" ? "bg-[var(--cmp-surface-warning)]" : "bg-[var(--cmp-surface-information)]"}`}>{r.category === "governance" ? "🛡️" : r.category === "asset" ? "🖥️" : r.category === "document" ? "📄" : r.category === "configuration" ? "🔧" : r.category === "compliance" ? "✅" : "⚡"}</span>
        ...
        <Card title="Automation Designer" right={<span className="text-[11px] text-gray-400">{k.activeAutomations} active</span>}>
          {d.automations.length ? <div className="space-y-2">{d.automations.map((a: any) => (
            <div key={a.id} className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-[var(--cmp-color-success)] shrink-0" /><div className="min-w-0 flex-1"><p className="text-[12px] text-gray-800 leading-tight truncate">{a.name}</p><p className="text-[10px] text-gray-400">{a.automation_type} · {a.runs} runs</p></div><Pill text={a.status} tone={a.status === "active" ? "emerald" : "amber"} /></div>
