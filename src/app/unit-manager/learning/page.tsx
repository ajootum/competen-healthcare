  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        <Kpi icon="🛡️" tint="bg-[var(--cmp-surface-success)]" label="Overall Compliance" value={`${compliancePct}%`} tone={pctTone(compliancePct)} sub={hasEnrol ? "mandatory completion" : `${d.compliance.current}/${d.compliance.total} current`} href="/unit-manager/learning/mandatory" />
        <Kpi icon="⛔" tint="bg-[var(--cmp-surface-error)]" label="Mandatory Overdue" value={mandatoryOverdue} tone={mandatoryOverdue ? "text-[var(--cmp-text-error)]" : "text-gray-400"} sub={hasEnrol ? "assignments overdue" : "expired competency"} href="/unit-manager/learning/mandatory" />
        <Kpi icon="📅" tint="bg-[var(--cmp-surface-warning)]" label="Due in 30 Days" value={due30} tone={due30 ? "text-[var(--cmp-text-warning)]" : "text-gray-400"} sub={hasEnrol ? "assignments due" : "expiring competency"} href="/unit-manager/learning/mandatory" />
        <Kpi icon="📋" tint="bg-[var(--cmp-surface-information)]" label="Active Assignments" value={hasEnrol ? L.active : "—"} tone={hasEnrol ? (L.active ? "text-[var(--cmp-text-information)]" : "text-gray-400") : "text-gray-300"} sub={hasEnrol ? `${L.activeAssignments} assignment rule(s)` : L.provisioned ? "none assigned yet" : "run migration 089"} href="#enrolment" />
      ...
      <div id="enrolment" className={`${card} p-5 scroll-mt-4`}>
        <div className="flex items-center justify-between mb-3"><h3 className="font-semibold text-gray-900 text-sm">Assignment &amp; Enrolment <span className="text-[10px] font-normal text-gray-400">LDS-001 operational layer</span></h3><Link href="/unit-manager/learning/assign" className="text-[11px] text-[var(--cmp-text-success)] hover:underline">Assign →</Link></div>
        {!L.provisioned ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="grid grid-cols-4 gap-2 text-center content-start">
              <div className="rounded-lg bg-[var(--cmp-surface-success)] py-2"><p className="text-lg font-bold text-emerald-700 tabular-nums">{L.completed}</p><p className="text-[10px] text-[var(--cmp-text-success)]">Completed</p></div>
              <div className="rounded-lg bg-[var(--cmp-surface-information)] py-2"><p className="text-lg font-bold text-[var(--cmp-text-information)] tabular-nums">{L.inProgress}</p><p className="text-[10px] text-[var(--cmp-text-information)]">In progress</p></div>
              ...
              <div className="rounded-lg bg-[var(--cmp-surface-error)] py-2"><p className="text-lg font-bold text-[var(--cmp-text-error)] tabular-nums">{L.overdue}</p><p className="text-[10px] text-[var(--cmp-text-error)]">Overdue</p></div>
            </div>
            <div>
              ...
              {L.overdueList.length === 0 ? <p className="text-sm text-gray-400">No overdue mandatory learning. 🎉</p> : (
                <div className="space-y-1">{L.overdueList.slice(0, 5).map((o: any, i: number) => (<div key={i} className="flex items-center justify-between gap-2 text-xs"><span className="min-w-0 truncate"><span className="text-gray-700">{o.name}</span> <span className="text-gray-400">{o.course}</span></span><span className="text-[var(--cmp-text-error)] shrink-0">{o.due ?? "—"}</span></div>))}</div>
              )}
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div id="priority" className={`${card} p-5 scroll-mt-4`}>
          <div className="flex items-center justify-between mb-3"><h3 className="font-semibold text-gray-900 text-sm">Priority Queue</h3><Link href="/unit-manager/learning/assign" className="text-[11px] text-[var(--cmp-text-success)] hover:underline">Assign learning →</Link></div>
          {d.priorityStaff.length === 0 ? <p className="text-sm text-gray-400">No staff with learning gaps. 🎉</p> : (
            <div className="space-y-1.5">{d.priorityStaff.map((s: any, i: number) => (<div key={i} className="flex items-center justify-between gap-2 text-xs"><div className="flex items-center gap-2 min-w-0"><span className="w-6 h-6 rounded-full bg-[var(--cmp-surface-success)] text-emerald-700 flex items-center justify-center text-[10px] font-bold shrink-0">{(s.name?.[0] ?? "?").toUpperCase()}</span><div className="min-w-0"><p className="text-gray-800 truncate">{s.name}</p><p className="text-[10px] text-gray-400 capitalize">{s.role}</p></div></div><span className="text-[var(--cmp-text-error)] font-semibold tabular-nums shrink-0">{s.gaps} gaps</span></div>))}</div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className={`${card} p-5`}>
          ...
          <div className="border border-dashed border-gray-200 rounded-lg p-5 text-center"><p className="text-2xl mb-1 opacity-40">🚀</p><p className="text-xs text-gray-500">Individual Development Plans, CPD, mentorship and progress reviews need a development-plan store.</p><Link href="/unit-manager/learning/development" className="text-[11px] text-[var(--cmp-text-success)] hover:underline mt-1 inline-block">Details →</Link></div>
        </div>
        <div className={`${card} p-5`}>
          <h3 className="font-semibold text-gray-900 text-sm mb-2">Career Progress</h3>
          <div className="border border-dashed border-gray-200 rounded-lg p-5 text-center"><p className="text-2xl mb-1 opacity-40">🧗</p><p className="text-xs text-gray-500">Role-progression readiness (competencies + experience + milestones) needs a pathway-progression store.</p><Link href="/unit-manager/learning/pathways" className="text-[11px] text-[var(--cmp-text-success)] hover:underline mt-1 inline-block">Details →</Link></div>
        </div>
        <div className={`${card} p-5`}>
