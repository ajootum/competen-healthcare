...
export default async function PdCpd() {
  ...
  if (!d.provisioned) return <div className="space-y-4">{header}<div className="bg-[var(--cmp-surface-warning)] border border-[var(--cmp-color-warning)] rounded-xl p-6"><p className="font-semibold text-amber-900">⚙️ CPD ledger not available</p><p className="text-sm text-amber-800 mt-1">The CPD log store isn&apos;t provisioned for this tenant.</p></div></div>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        <Kpi icon="🏆" tint="bg-[var(--cmp-surface-success)]" label="CPD Points (unit)" value={k.pointsEarned} sub="this year" />
        <Kpi icon="✅" tint="bg-teal-50" label="Approved Activities" value={k.approved} sub="validated" />
        <Kpi icon="🕐" tint="bg-[var(--cmp-surface-warning)]" label="Awaiting Validation" value={k.awaiting} tone={k.awaiting ? "text-[var(--cmp-text-warning)]" : "text-gray-400"} sub="requires review" />
        <Kpi icon="👥" tint="bg-[var(--cmp-surface-information)]" label="Active CPD Learners" value={k.activeLearners} sub={`${k.avgPerStaff} avg pts`} />
        <Kpi icon="🎯" tint="bg-violet-50" label="Meeting Target" value={k.meetingTarget} tone={k.meetingTarget ? "text-[var(--cmp-text-success)]" : "text-gray-400"} sub={`≥${k.target} pts (default)`} />
        <Kpi icon="⚠️" tint="bg-[var(--cmp-surface-error)]" label="Expiring Certificates" value={d.expiringCerts} tone={d.expiringCerts ? "text-[var(--cmp-text-error)]" : "text-gray-400"} sub="within 90 days" href="/competency-office/credentialing" />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className={`${card} p-5`}>
          ...
          <div className="flex items-end justify-between gap-1 h-28">{d.monthlyTrend.map((v: number, i: number) => (<div key={i} className="flex-1 flex flex-col items-center justify-end gap-1"><div className="w-full bg-[var(--cmp-color-success)] rounded-t" style={{ height: `${(v / mMax) * 100}%`, minHeight: v ? "2px" : "0" }} title={`${MONTHS[i]}: ${v}`} /><span className="text-[8px] text-gray-400">{MONTHS[i][0]}</span></div>))}</div>
        </div>
        ...
        <div className={`${card} p-5`}>
          ...
          {d.topContributors.length === 0 ? <p className="text-sm text-gray-400">No CPD activity yet.</p> : (
            <div className="space-y-2">{d.topContributors.map((c: any) => (<div key={c.name} className="text-xs"><div className="flex items-center justify-between mb-0.5"><span className="text-gray-700 truncate">{c.name}</span><span className="text-gray-500 tabular-nums">{+c.points.toFixed(1)}</span></div><div className="w-full h-1.5 rounded-full bg-gray-100 overflow-hidden"><div className="h-full bg-[var(--cmp-color-success)]" style={{ width: `${(c.points / cMax) * 100}%` }} /></div></div>))}</div>
          )}
        </div>
      </div>
      ...
      {/* Recent activities */}
      <div className={`${card} p-5`}>
        <div className="flex items-center justify-between mb-3"><h3 className="font-semibold text-gray-900 text-sm">Recent CPD Activities</h3><Link href="/dashboard/cpd" className="text-[11px] text-[var(--cmp-text-success)] hover:underline">My CPD Log →</Link></div>
        {d.recent.length === 0 ? <p className="text-sm text-gray-400">No CPD activities recorded this year.</p> : (
          <div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="text-[10px] uppercase tracking-wide text-gray-400 text-left border-b border-gray-100"><th className="py-2 font-medium">Staff</th><th className="py-2 font-medium">Activity</th><th className="py-2 font-medium">Category</th><th className="py-2 font-medium">Date</th><th className="py-2 font-medium text-right">Points</th><th className="py-2 font-medium text-right">Validation</th></tr></thead>
            <tbody>{d.recent.map((a: any, i: number) => (<tr key={i} className="border-b border-gray-50"><td className="py-2 text-gray-800">{a.name}</td><td className="py-2 text-gray-600 truncate max-w-[12rem]">{a.title}</td><td className="py-2 text-gray-500">{a.category}</td><td className="py-2 text-gray-500 tabular-nums">{a.date ?? "—"}</td><td className="py-2 text-right text-gray-700 tabular-nums">{+a.points.toFixed(1)}</td><td className="py-2 text-right"><span className={`text-[9px] px-1.5 py-0.5 rounded ${a.verified ? "bg-[var(--cmp-surface-success)] text-emerald-700" : "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]"}`}>{a.verified ? "Validated" : "Pending"}</span></td></tr>))}</tbody>
          </table></div>
        )}
      </div>
