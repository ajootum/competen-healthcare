...
const tone = (s: number | null) => (s == null ? "text-gray-400" : s >= 85 ? "text-[var(--cmp-text-success)]" : s >= 70 ? "text-[var(--cmp-text-warning)]" : "text-[var(--cmp-text-error)]");
const riskBandPill: Record<string, string> = { critical: "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]", high: "bg-[var(--cmp-surface-warning)] text-orange-700", moderate: "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]", low: "bg-[var(--cmp-surface-success)] text-emerald-700" };
...
export default async function ExecutiveQualityCommandCentre() {
  ...
  const k = d.kpis;
  const KPIS = [
    { icon: "🏆", tint: "bg-[var(--cmp-surface-success)]", label: "Enterprise Quality Score", value: k.enterpriseQuality ?? "—", unit: "%", bd: band(k.enterpriseQuality), delta: k.enterpriseDelta, dg: "up" as const },
    { icon: "🛡️", tint: "bg-[var(--cmp-surface-warning)]", label: "Patient Safety Score", value: k.patientSafety ?? "—", unit: "%", bd: band(k.patientSafety), delta: k.safetyDelta, dg: "up" as const },
    { icon: "🚨", tint: "bg-[var(--cmp-surface-error)]", label: "Risk Score", value: k.riskScore ?? "—", unit: "%", bd: band(k.riskScore), delta: k.riskDelta, dg: "up" as const },
    { icon: "📋", tint: "bg-violet-50", label: "Accreditation Score", value: k.accreditation ?? "—", unit: "%", bd: band(k.accreditation) },
    { icon: "💓", tint: "bg-[var(--cmp-surface-information)]", label: "Clinical Performance", value: k.clinical ?? "—", unit: "%", bd: band(k.clinical) },
    { icon: "👥", tint: "bg-teal-50", label: "Workforce Readiness", value: k.workforce ?? "—", unit: "%", bd: band(k.workforce) },
    { icon: "✅", tint: "bg-[var(--cmp-surface-warning)]", label: "Executive Actions Due", value: k.actionsDue, sub: "Due within 7 days" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className={`${qcard} p-5`}>
          ...
          <div className="space-y-2.5">{d.strategicPriorities.map((p: any) => (
            <div key={p.rank} className="flex items-center gap-2"><span className="w-5 h-5 rounded bg-gray-100 flex items-center justify-center text-[10px] font-semibold text-gray-500 shrink-0">{p.rank}</span>
              ...
              <span className={`text-[9px] px-1.5 py-0.5 rounded shrink-0 ${p.status === "On Track" ? "bg-[var(--cmp-surface-success)] text-emerald-700" : "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]"}`}>{p.status}</span>
            </div>
        ...
        <div className={`${qcard} p-5`}>
          ...
          <div className="overflow-x-auto"><table className="w-full text-[11px]"><thead><tr className="text-left text-gray-400 border-b border-gray-100"><th className="py-1 font-medium">Action</th><th className="py-1 font-medium">Owner</th><th className="py-1 font-medium">Due</th><th className="py-1 font-medium">Status</th></tr></thead>
            <tbody>{d.actions.map((a: any, i: number) => (<tr key={i} className="border-b border-gray-50"><td className="py-1.5 text-gray-700 max-w-[120px] truncate" title={a.action}>{a.action}</td><td className="py-1.5 text-gray-500">{a.owner}</td><td className="py-1.5 text-gray-500 tabular-nums">{a.due}</td><td className="py-1.5"><span className={`text-[9px] px-1.5 py-0.5 rounded ${a.dueSoon ? "bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)]" : "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]"}`}>{a.dueSoon ? "Due Soon" : a.status.replace(/_/g, " ")}</span></td></tr>))}{!d.actions.length && <tr><td colSpan={4} className="py-6 text-center text-gray-400">No actions due.</td></tr>}</tbody></table></div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className={`${qcard} p-5`}>
          ...
          {d.regulatory.length ? <div className="overflow-x-auto"><table className="w-full text-[11px]"><thead><tr className="text-left text-gray-400 border-b border-gray-100"><th className="py-1 font-medium">Standard</th><th className="py-1 font-medium">Status</th><th className="py-1 font-medium text-right">Compliance</th></tr></thead>
            <tbody>{d.regulatory.map((r: any, i: number) => (<tr key={i} className="border-b border-gray-50"><td className="py-1.5 text-gray-700 max-w-[110px] truncate">{r.name}</td><td className="py-1.5"><span className={`text-[9px] px-1.5 py-0.5 rounded ${r.status === "Compliant" ? "bg-[var(--cmp-surface-success)] text-emerald-700" : "bg-[var(--cmp-surface-warning)] text-[var(--cmp-text-warning)]"}`}>{r.status}</span></td><td className="py-1.5 text-right tabular-nums text-gray-600">{r.compliance != null ? `${r.compliance}%` : "—"}</td></tr>))}</tbody></table></div> : <NextPhase>Regulatory framework readiness comes from the Accreditation store — provision it (migration 019/061) to populate.</NextPhase>}
        </div>
