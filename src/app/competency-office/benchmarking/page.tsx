...
export default async function BenchmarkingPage() {
  ...
  if (d.empty)
    return (
      <div className="max-w-[1400px] space-y-4">{head}
        <div className="bg-[var(--cmp-surface-information)] border border-[var(--cmp-color-information)] rounded-xl p-3 text-[12px] text-blue-800">No competency domains carry recorded readiness yet — as frameworks are populated and competencies are achieved, this engine ranks each clinical area against its internal peers. {d.frameworksTotal ? `(${d.frameworksTotal} competencies defined; none yet attributed to a scored domain.)` : ""}</div>
  ...
  const k = d.kpis;
  const s = d.stats;
  const tolTone = (v: number) => (v >= 0 ? "text-[var(--cmp-text-success)]" : "text-[var(--cmp-text-error)]");
  ...
  return (
    <div className="max-w-[1400px] space-y-4">
      ...
      {(d.singleUnit || d.noVariance) && (
        <div className="bg-[var(--cmp-surface-warning)] border border-[var(--cmp-color-warning)] rounded-xl p-3 text-[12px] text-amber-800">
          {d.singleUnit
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <Kpi label="Top-quartile ≥" value={`${k.topQuartile}%`} sub="75th percentile" tone="text-[var(--cmp-text-success)]" />
        <Kpi label="Best area" value={`${k.bestPct}%`} sub={k.bestName} tone="text-teal-600" />
        <Kpi label="Widest gap" value={`${k.widestGap} pts`} sub="best vs worst spread" tone={k.widestGap >= 25 ? "text-[var(--cmp-text-error)]" : "text-gray-900"} />
        <Kpi label="Below target" value={k.belowTarget} sub={`< ${s.target}% readiness`} tone={k.belowTarget ? "text-[var(--cmp-text-error)]" : "text-[var(--cmp-text-success)]"} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <Card title="Top performers" right={<span className="text-[11px] text-[var(--cmp-text-success)]">best readiness</span>}>
          <div className="space-y-2">
            {d.top.map((u: any) => (
              <div key={u.name} className="flex items-center gap-3">
                <span className="w-6 h-6 rounded-full bg-[var(--cmp-surface-success)] text-emerald-700 text-[11px] font-bold flex items-center justify-center shrink-0 tabular-nums">{u.rank}</span>
        ...
        <Card title="Areas needing attention" right={<Link href="/competency-office/gaps" className="text-[11px] text-teal-600 hover:underline">Gap management →</Link>}>
          <div className="space-y-2">
            {d.bottom.map((u: any) => (
              <div key={u.name} className="flex items-center gap-3">
                <span className="w-6 h-6 rounded-full bg-[var(--cmp-surface-error)] text-[var(--cmp-text-error)] text-[11px] font-bold flex items-center justify-center shrink-0 tabular-nums">{u.rank}</span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2"><p className="text-[12.5px] font-medium text-gray-800 truncate">{u.name}</p><span className="tabular-nums text-[12px] font-semibold text-gray-900">{u.readiness}%</span></div>
                  <Progress pct={u.readiness} />
