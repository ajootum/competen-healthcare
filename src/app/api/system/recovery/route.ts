...
export async function POST(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ trace_id: c.traceId, actor_id: c.userId, actor_name: me?.full_name ?? null, action: `recovery_${kind}`, entity_type: "recovery_event", entity_id: data.id, entity_name: data.title });
...
export async function PATCH(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ trace_id: c.traceId, actor_id: c.userId, action: `recovery_${data.status}`, entity_type: "recovery_event", entity_id: data.id, entity_name: data.title, new_value: { status: data.status, outcome: data.outcome } });
