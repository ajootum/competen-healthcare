...
// CST-109 — Competency Package Manager write API. POST creates a package; PATCH updates its status
// (draft → published → archived) or version; DELETE removes it (items cascade). Items are managed via
// /api/studio/packages/items. Competency-office tier (educator/admin/super), tenant-scoped, audited.
/* eslint-disable @typescript-eslint/no-explicit-any */

const TYPES = ["specialty", "orientation", "mandatory", "role", "leadership", "accreditation", "simulation", "assessment", "learning", "deployment"];
...
export async function POST(req: Request) {
  ...
  if (!isEducator(c)) return forbidden("Creating a package requires competency-office authority");

  const b = await req.json().catch(() => ({}));
  const name = clean(b.name);
  if (!name) return badRequest("a package name is required");
  const type = TYPES.includes(b.package_type) ? b.package_type : "specialty";

  const { data: me } = await c.admin.from("profiles").select("full_name").eq("id", c.userId).single();
  const { data, error } = await c.admin.from("competency_packages").insert({
    hospital_id: c.hospitalId ?? null, name, description: clean(b.description),
    package_type: type, version: clean(b.version) ?? "1.0.0", status: "draft",
...
export async function PATCH(req: Request) {
  ...
  const { data: row } = await c.admin.from("competency_packages").select("id, hospital_id").eq("id", id).maybeSingle();
