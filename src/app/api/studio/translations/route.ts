...
export async function POST(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ trace_id: c.traceId, actor_id: c.userId, actor_name: me?.full_name ?? null, action: "create_translation", entity_type: "cap_asset_translation", entity_id: data.id, hospital_id: c.hospitalId ?? null, new_value: { asset_label: label, locale } });
...
export async function PATCH(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ trace_id: c.traceId, actor_id: c.userId, action: "update_translation", entity_type: "cap_asset_translation", entity_id: id, hospital_id: row.hospital_id ?? null, new_value: patch });
...
export async function DELETE(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ trace_id: c.traceId, actor_id: c.userId, action: "delete_translation", entity_type: "cap_asset_translation", entity_id: id, hospital_id: row.hospital_id ?? null });
  return NextResponse.json({ ok: true });
}
