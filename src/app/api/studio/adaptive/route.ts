...
export async function POST(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ actor_id: c.userId, actor_name: me?.full_name ?? null, action: "create_adaptive_exam", entity_type: "cst_adaptive_exam", entity_id: data.id, hospital_id: c.hospitalId ?? null, new_value: { name } });
...
export async function PATCH(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ actor_id: c.userId, action: "update_adaptive_exam", entity_type: "cst_adaptive_exam", entity_id: id, hospital_id: row.hospital_id ?? null, new_value: patch });
...
export async function DELETE(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ actor_id: c.userId, action: "delete_adaptive_exam", entity_type: "cst_adaptive_exam", entity_id: id, hospital_id: row.hospital_id ?? null });
  return NextResponse.json({ ok: true });
}
