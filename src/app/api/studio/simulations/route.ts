...
export async function POST(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ actor_id: c.userId, actor_name: me?.full_name ?? null, action: "create_scenario", entity_type: "simulation_scenario", entity_id: data.id, hospital_id: c.hospitalId ?? null, new_value: { name, scenario_type: type } });
...
export async function PATCH(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ actor_id: c.userId, action: "update_scenario", entity_type: "simulation_scenario", entity_id: id, hospital_id: row.hospital_id ?? null, new_value: patch });
...
export async function DELETE(req: Request) {
  ...
  await c.admin.from("audit_log").insert({ actor_id: c.userId, action: "delete_scenario", entity_type: "simulation_scenario", entity_id: id, hospital_id: row.hospital_id ?? null });
  return NextResponse.json({ ok: true });
}
