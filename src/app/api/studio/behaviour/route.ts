...
// CST-040 — Professional Behaviour Assessment write API. POST creates a behaviour assessment; PATCH
// updates status/scale; DELETE removes it (indicators cascade). Indicators via ./indicators.
// Competency-office tier (educator/admin/super), tenant-scoped, audited.
/* eslint-disable @typescript-eslint/no-explicit-any */

const SCALES = ["bars", "likert5", "likert3", "binary", "global"];
...
export async function POST(req: Request) {
  ...
  if (!isEducator(c)) return forbidden("Creating a behaviour assessment requires competency-office authority");

  const b = await req.json().catch(() => ({}));
  const name = clean(b.name);
  if (!name) return badRequest("a name is required");
  const scale = SCALES.includes(b.rating_scale) ? b.rating_scale : "bars";

  const { data: me } = await c.admin.from("profiles").select("full_name").eq("id", c.userId).single();
  const { data, error } = await c.admin.from("cst_behaviour_assessments").insert({
    hospital_id: c.hospitalId ?? null, name, description: clean(b.description), rating_scale: scale, status: "draft",
...
export async function PATCH(req: Request) {
  ...
  const { data: row } = await c.admin.from("cst_behaviour_assessments").select("id, hospital_id").eq("id", id).maybeSingle();
