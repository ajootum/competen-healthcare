...
// Staffing standards (Ward configuration, SSW-001) — required staff per unit /
// shift-type / role, owned by the Director of Nursing. Reads: any operational
// staff. Writes: Director of Nursing or admin. Service-role client → scope enforced here.
...
export async function GET() {
  ...
  let q = admin.from("op_staffing_standards").select("*, departments!department_id(name)").order("shift_type");
...
export async function POST(req: Request) {
  ...
  if (!ROLES.includes(b.role)) return badRequest("valid role required");
  ...
  const min_count = Math.max(0, parseInt(b.min_count, 10) || 0);
  const target_ratio = b.target_ratio ? Number(b.target_ratio) : null;
  // Manual upsert on (hospital_id, department_id, shift_type, role) — null dept is
  // NULLS DISTINCT under the unique index, so we match it explicitly with .is().
  let sel = admin.from("op_staffing_standards").select("id").eq("hospital_id", hospital_id).eq("shift_type", shift_type).eq("role", b.role);
  sel = dept ? sel.eq("department_id", dept) : sel.is("department_id", null);
  const { data: existing } = await sel.maybeSingle();
  if (existing) {
    const { data, error } = await admin.from("op_staffing_standards").update({ min_count, target_ratio, updated_at: new Date().toISOString() }).eq("id", existing.id).select().single();
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json(data);
  }
  const { data, error } = await admin.from("op_staffing_standards").insert({ hospital_id, department_id: dept, shift_type, role: b.role, min_count, target_ratio, created_by: c.userId }).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function DELETE(req: Request) {
  ...
  const admin = c.admin as any;
  const { data: row } = await admin.from("op_staffing_standards").select("hospital_id").eq("id", id).maybeSingle();
  if (!row) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (!isSuper(c) && row.hospital_id !== c.hospitalId) return forbidden("Out of scope");
  const { error } = await admin.from("op_staffing_standards").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
