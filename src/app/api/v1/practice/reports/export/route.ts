...
import { practiceReport, activityCsv } from "@/lib/practice/reports";
...
export async function GET(req: NextRequest) {
  ...
  const report = await practiceReport(auth.caller.admin, auth.ctx, {
  ...
  return new NextResponse(activityCsv(report), {
    status: 200,
    headers: {
