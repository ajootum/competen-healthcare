...
// Tools Command Bar (Tools & Settings spec §"Tools Command Bar"). Light-themed,
// single-turn natural-language prompt over the real grounded /api/ai/assistant
// endpoint. Answer renders inline above the composer.

const SUGGESTED = [
  "Generate quiz on respiration",
  "Create OSCE station for catheter care",
  "Find templates for case study",
  "Show my published items",
  "Recommend resources on sepsis",
];
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      setAnswer(res.ok ? (d.answer ?? "No answer returned.")
        : res.status === 503 ? "The AI assistant isn't configured yet (no API key set). Ask your administrator to enable it — the rest of this hub runs on your live workspace data."
  ...
  return (
    <div className="rounded-2xl bg-white border border-gray-200 shadow-sm p-4">
      <div className="flex items-center gap-1.5 mb-3">
        <span>⌨️</span>
        <p className="text-[11px] font-bold uppercase tracking-widest text-gray-500">Tools Command Bar <span className="normal-case font-normal text-gray-400">(Ask Copilot)</span></p>
      ...
      <div className="flex gap-2">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} disabled={busy}
          placeholder="Ask Copilot to help you with tools, templates, resources or settings…"
