...
// Learning Command Bar (Learning Intelligence spec §"Learning Command Bar").
// Single-turn natural-language prompt over the real grounded /api/ai/assistant
// endpoint — the same educator-gated, cited, audit-logged AI that powers the
// Copilot. Answer renders inline above the composer. Attach/template chips are
// muted ("soon") because object attachment needs a picker + store.

const SUGGESTED = [
  "Who needs support?",
  "Predict assessment failure",
  "Find disengaged learners",
  "Analyse learning barriers",
  "Create remediation plans",
  "Compare cohort performance",
  "Predict programme completion",
];
const ATTACH = ["📎 Attach File", "🧩 Select Learner(s)", "👥 Add Cohort", "📄 Use Template"];
...
export default function CommandBar({ aiConfigured }: { aiConfigured: boolean }) {
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      setAnswer(res.ok ? (dt.answer ?? "No answer returned.")
        : res.status === 503 ? "The AI assistant isn't configured yet (no API key set). Ask your administrator to enable it — the rest of this workspace runs on your live learner data."
        : res.status === 429 ? "AI rate limit reached for this hour — please try again shortly."
        : (dt.error ?? "Something went wrong. Please try again."));
    } catch {
  ...
  return (
    <div className="rounded-2xl bg-white/[0.03] border border-white/10 p-4">
      <div className="flex items-center gap-1.5 mb-3">
        <span>⌨️</span>
        <p className="text-[11px] font-bold uppercase tracking-widest text-slate-300">Learning Command Bar</p>
      ...
      <div className="flex gap-2">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} disabled={busy}
          placeholder="Ask Copilot to analyse, predict or recommend anything about learning…"
