...
// Educator Command Bar (Educator Intelligence spec §19). Single-turn natural-
...
const SUGGESTED = [
  "Who is overloaded?",
  "Analyse teaching effectiveness",
  "Predict staffing gaps",
  "Find feedback bottlenecks",
  "Compare workload",
  "Create capacity plan",
  "Identify succession candidates",
];
const ATTACH = ["📎 Attach File", "🧑‍🏫 Select Educators", "🧩 Add Context", "📄 Use Template"];
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      setAnswer(res.ok ? (dt.answer ?? "No answer returned.")
        : res.status === 503 ? "The AI assistant isn't configured yet (no API key set). Ask your administrator to enable it — the rest of this workspace runs on your live educator activity data."
  ...
  return (
    <div className="rounded-2xl bg-white/[0.03] border border-white/10 p-4">
      <div className="flex items-center gap-1.5 mb-3">
        <span>⌨️</span>
        <p className="text-[11px] font-bold uppercase tracking-widest text-slate-300">Educator Command Bar</p>
      ...
      <div className="flex gap-2">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} disabled={busy}
          placeholder="Ask Copilot to analyse, predict or improve educator performance…"
