...
// Curriculum Command Bar (Curriculum Intelligence spec §"AI Command Bar").
...
// Copilot. The answer renders inline above the composer. Attach/template chips
// are shown muted ("soon") because object-attachment needs a picker + store —
// no dead controls.

const SUGGESTED = [
  "Analyse this curriculum",
  "Show competency gaps",
  "Check standards compliance",
  "Find duplicated competencies",
...
export default function CommandBar({ aiConfigured }: { aiConfigured: boolean }) {
  ...
  async function send(text?: string) {
    ...
    if (!q || busy) return;
    setInput(""); setAsked(q); setAnswer(null); setBusy(true);
    try {
      ...
      setAnswer(res.ok ? (d.answer ?? "No answer returned.")
        : res.status === 503 ? "The AI assistant isn't configured yet (no API key set). Ask your administrator to enable it — the rest of this workspace runs on your live curriculum data."
  ...
  return (
    <div className="rounded-2xl bg-white/[0.03] border border-white/10 p-4">
      <div className="flex items-center gap-1.5 mb-3">
        <span>⌨️</span>
        <p className="text-[11px] font-bold uppercase tracking-widest text-slate-300">Curriculum Command Bar</p>
      ...
      <div className="flex gap-2">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} disabled={busy}
          placeholder="Ask Copilot to analyse, create, improve or predict anything about your curriculum…"
