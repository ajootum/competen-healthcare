...
// Accreditation Command Bar (Accreditation Intelligence spec §22). Single-turn
// natural-language prompt over the real grounded /api/ai/assistant endpoint — the
// same educator-gated, cited, audit-logged AI that powers the Copilot. Answer
// renders inline above the composer. Attach/template chips are muted.
...
const SUGGESTED = [
  ...
  "Predict survey readiness",
  "Identify expiring evidence",
  "Compare programme compliance",
];
const ATTACH = ["📎 Attach File", "🏛️ Select Context", "🧩 Add Filter", "📄 Use Template"];
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      setAnswer(res.ok ? (dt.answer ?? "No answer returned.")
        : res.status === 503 ? "The AI assistant isn't configured yet (no API key set). Ask your administrator to enable it — the rest of this workspace runs on your live accreditation data."
  ...
  return (
    <div className="rounded-2xl bg-white/[0.03] border border-white/10 p-4">
      <div className="flex items-center gap-1.5 mb-3">
        <span>⌨️</span>
        <p className="text-[11px] font-bold uppercase tracking-widest text-slate-300">Accreditation Command Bar</p>
      ...
      <div className="flex gap-2">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} disabled={busy}
          placeholder="Ask Copilot to analyse, predict or recommend accreditation actions…"
