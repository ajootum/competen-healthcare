...
// Shared header + module tab bar for the Improvement & Action Center workspace.
export default function ImprovementNav({ active }: { active: string }) {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="mt-1 mb-3">
        <EduHeader icon={m?.icon ?? "🎯"} title={m ? m.name : "Improvement & Action Center"} sub={m?.desc ?? "Turn insights into action — drive improvement, reduce risk, achieve excellence."} />
      </div>
      <div className="flex items-center gap-1 overflow-x-auto pb-1">
        <Link href="/educator/analytics/improvement"
        ...
        {MODULES.map(mod => (
          <Link key={mod.id} href={`/educator/analytics/improvement/${mod.id}`}
            ...
            {mod.n}. {mod.name.replace("Improvement ", "").replace("Educational ", "")}
          </Link>
        ))}
      </div>
    </div>
