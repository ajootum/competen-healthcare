        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5">
          ...
          {d.byDomain.length === 0 ? <p className="text-xs text-gray-400">No audits recorded yet.</p> : (
            <div className="flex flex-col gap-2">{d.byDomain.map(a => (
              <div key={a.area} className="flex items-center gap-2"><span className="text-[11px] text-gray-500 w-32 truncate">{a.area}</span><div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${a.pct >= 80 ? "bg-[var(--cmp-color-success)]" : a.pct >= 60 ? "bg-[var(--cmp-color-warning)]" : "bg-[var(--cmp-color-critical)]"}`} style={{ width: `${a.pct}%` }} /></div><span className="text-[10px] font-bold text-gray-600 w-9 text-right">{a.pct}%</span></div>
            ))}</div>
          )}
        </div>
        <div className="bg-white border border-gray-100 rounded-2xl p-5">
