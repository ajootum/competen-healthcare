        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5">
          ...
          {d.bySource.length === 0 ? <p className="text-xs text-gray-400">No actions recorded.</p> : (
            <div className="flex flex-col gap-2">{d.bySource.map(x => (
              <div key={x.label} className="flex items-center gap-2"><span className="text-[10px] text-gray-500 w-32 truncate">{x.label}</span><div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-[var(--cmp-color-warning)] rounded-full" style={{ width: `${(x.n / srcMax) * 100}%` }} /></div><span className="text-[10px] font-bold text-gray-600 w-6 text-right">{x.n}</span></div>
            ))}</div>
          )}
        </div>
