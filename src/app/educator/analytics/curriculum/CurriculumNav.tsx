...
// Shared header + module tab bar for the Curriculum Analytics workspace.
export default function CurriculumNav({ active }: { active: string }) {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="mt-1 mb-3">
        <EduHeader icon={m?.icon ?? "📚"} title={m ? m.name : "Curriculum Analytics"} sub={m?.desc ?? "Curriculum intelligence to evaluate, improve and align curriculum quality and outcomes."} />
      </div>
      <div className="flex items-center gap-1 overflow-x-auto pb-1">
        <Link href="/educator/analytics/curriculum"
        ...
        {MODULES.map(mod => (
          <Link key={mod.id} href={`/educator/analytics/curriculum/${mod.id}`}
            ...
            {mod.n}. {mod.name.replace("Curriculum ", "").replace(" Analytics", "").replace(" Effectiveness", "")}
          </Link>
        ))}
      </div>
    </div>
