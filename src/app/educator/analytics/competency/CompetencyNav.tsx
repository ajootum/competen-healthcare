...
// Shared header + module tab bar for the Competency Analytics workspace.
export default function CompetencyNav({ active }: { active: string }) {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="mt-1 mb-3">
        <EduHeader icon={m?.icon ?? "🎯"} title={m ? m.name : "Competency Analytics"} sub={m?.desc ?? "Competency intelligence to measure coverage, achievement, gaps, mastery and trends."} />
      </div>
      <div className="flex items-center gap-1 overflow-x-auto pb-1">
        <Link href="/educator/analytics/competency"
        ...
        {MODULES.map(mod => (
          <Link key={mod.id} href={`/educator/analytics/competency/${mod.id}`}
            ...
            {mod.n}. {mod.name.replace("Competency ", "").replace(" Performance", "").replace(" Mastery", "")}
          </Link>
        ))}
      </div>
    </div>
