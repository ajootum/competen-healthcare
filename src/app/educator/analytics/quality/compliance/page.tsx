      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 lg:col-span-2">
          ...
          <div className="flex flex-col gap-2">{d.matrix.map(b => (
            <div key={b.label} className="flex items-center gap-2 text-[11px]"><span className="text-gray-500 w-36">{b.label}</span>{b.backed && b.pct !== null ? <><div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${b.pct >= 80 ? "bg-[var(--cmp-color-success)]" : b.pct >= 60 ? "bg-[var(--cmp-color-warning)]" : "bg-[var(--cmp-color-critical)]"}`} style={{ width: `${b.pct}%` }} /></div><span className="font-bold text-gray-700 w-9 text-right">{b.pct}%</span></> : <span className="flex-1 text-right text-[8px] font-bold uppercase text-gray-300">no store</span>}</div>
          ))}</div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-3 gap-2 mb-3 text-center">
            {[["Open", d.capa.open, "text-[var(--cmp-text-warning)]"], ["Closed", d.capa.closed, "text-[var(--cmp-text-success)]"], ["Total", d.capa.total, "text-gray-900"]].map(([l, n, c]) => (
          ...
          {d.alerts.length === 0 ? <p className="text-[11px] text-gray-400">No open compliance alerts. ✅</p> : (
            <div className="flex flex-col gap-1">{d.alerts.map((a, i) => <div key={i} className="flex items-center gap-2 text-[10px]"><span className="w-1.5 h-1.5 rounded-full bg-[var(--cmp-color-warning)]" /><span className="text-gray-600 flex-1 truncate">{a.label}</span></div>)}</div>
          )}
