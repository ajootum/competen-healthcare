...
export default async function Faculty() {
  ...
  return (
    <div className="max-w-[1200px]">
      ...
      <p className="text-[10px] text-[var(--cmp-text-warning)] bg-[var(--cmp-surface-warning)] border border-[var(--cmp-color-warning)] rounded-lg px-3 py-2 mb-5">ℹ️ Faculty quality, satisfaction and turnaround need learner-feedback surveys and validation timestamps that aren&apos;t captured yet. Assessment activity below is live.</p>
          ...
          <div className="overflow-x-auto"><table className="w-full text-left border-collapse">
            ...
            <tbody>{d.ranking.map(f => (
              <tr key={f.id} className="border-b border-gray-50 text-[11px]">
                ...
                <td className="py-2 pr-3"><span className="flex items-center gap-1.5"><span className="w-12 h-1.5 bg-gray-100 rounded-full overflow-hidden inline-block"><span className="block h-full rounded-full bg-[var(--cmp-color-information)]" style={{ width: `${(f.assessments / maxA) * 100}%` }} /></span><span className="text-gray-700 font-bold">{f.assessments}</span></span></td>
