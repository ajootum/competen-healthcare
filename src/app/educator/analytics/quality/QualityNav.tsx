...
// Shared header + module tab bar for the Program Quality workspace.
export default function QualityNav({ active }: { active: string }) {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="mt-1 mb-3">
        <EduHeader icon={m?.icon ?? "🛡️"} title={m ? m.name : "Program Quality"} sub={m?.desc ?? "Executive intelligence for continuous improvement of educational programmes."} />
      </div>
      <div className="flex items-center gap-1 overflow-x-auto pb-1">
        <Link href="/educator/analytics/quality"
        ...
        {MODULES.map(mod => (
          <Link key={mod.id} href={`/educator/analytics/quality/${mod.id}`}
