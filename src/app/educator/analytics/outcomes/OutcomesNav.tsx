...
// Shared header + module tab bar for the Learner Outcomes workspace.
export default function OutcomesNav({ active }: { active: string }) {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="mt-1 mb-3">
        <EduHeader icon={m?.icon ?? "🎓"} title={m ? m.name : "Learner Outcomes"} sub={m?.desc ?? "Measuring what matters most: competence, readiness, certification and lifelong impact."} />
      </div>
      <div className="flex items-center gap-1 overflow-x-auto pb-1">
        <Link href="/educator/analytics/outcomes"
        ...
        {MODULES.map(mod => (
          <Link key={mod.id} href={`/educator/analytics/outcomes/${mod.id}`}
            ...
            {mod.n}. {mod.name.replace(" Readiness", "").replace(" Achievement", "").replace(" Progress", "").replace("Learning ", "")}
          </Link>
        ))}
      </div>
    </div>
