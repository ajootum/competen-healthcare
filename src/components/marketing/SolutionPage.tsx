...
// One template for all five landing pages (WEB-STRAT-001 Phase 1). Five near-identical pages written out
// five times drift within a week -- one changes its CTA, another its heading level, and the set stops
    ...
    <div className="flex flex-col min-h-full bg-white font-[family-name:var(--font-geist-sans)]">
      ...
      <main id="main">
          ...
          <div className={`${container} grid lg:grid-cols-2 gap-10 items-center py-10 lg:py-14`}>
            <div>
              <h1 className="text-[2.2rem] sm:text-[3rem] font-bold tracking-tight text-gray-900 leading-[1.08] text-balance">
                {s.headline.map(line => <span key={line} className="block">{line}</span>)}
              </h1>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-3">
                <Link href={s.primary.href} className="rounded-xl px-6 py-3.5 text-[15px] font-semibold text-white transition-opacity hover:opacity-90"
                  style={{ background: s.accent }}>
                  {s.primary.label}
                </Link>
                <a href={s.secondary.href} className="text-[15px] font-semibold transition-opacity hover:opacity-80" style={{ color: s.accent }}>
                  {s.secondary.label} →
                </a>
              </div>
            </div>
            ...
            <div className="relative rounded-3xl overflow-hidden aspect-[4/3] bg-[var(--cmp-neutral-100)]">
              ...
              <img src={s.image} alt={s.imageAlt} className="w-full h-full object-cover object-top" />
            </div>
          </div>
        </section>
        ...
        <section id="features" className={`${container} py-14 lg:py-16`}>
          <h2 className="text-center text-[1.6rem] font-bold tracking-tight text-gray-900">{s.featuresTitle}</h2>
        ...
        <section className={`${container} pb-16`}>
          <div className="relative overflow-hidden rounded-3xl px-6 py-7 sm:px-9 flex flex-col sm:flex-row sm:items-center gap-5"
            ...
            <div className="relative flex-1">
              <p className="text-[17px] font-bold text-gray-900">{s.closing.title}</p>
              <p className="mt-1 text-[13.5px] leading-relaxed text-gray-600">{s.closing.body}</p>
            </div>
            <Link href={s.primary.href} className="relative shrink-0 rounded-xl px-6 py-3.5 text-[15px] font-semibold text-white text-center transition-opacity hover:opacity-90"
              style={{ background: s.accent }}>
              {s.closing.action}
            </Link>
          </div>
