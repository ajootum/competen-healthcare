...
// Shared notifications centre (§8): stored event notifications plus
// live-derived expiry alerts. Rendered inside both the clinician and assessor
// shells so opening notifications never switches workspaces.
...
export default async function NotificationsCentre({ userId }: { userId: string }) {
  ...
  const [{ data: rows }, { data: expDecisions }, { data: expCredentials }] = await Promise.all([
    ...
    admin.from("competency_decisions")
      ...
      .eq("nurse_id", userId).not("expiry_date", "is", null)
    ...
    <div className="max-w-3xl">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Verification results, decisions, schedules and upcoming expiries.</p>
        </div>
        ...
        {notifications.length === 0 ? (
          <div className="p-10 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              You&apos;ll be notified here about verifications, decisions, credentials and scheduled sessions.
            </p>
          </div>
        ) : (
