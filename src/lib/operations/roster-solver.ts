...
type Plan = { assignments: any[]; scores: { coverage: number | null; competency: number | null; fairness: number; estCost: number }; slotsTotal: number; slotsFilled: number };
...
export function computeRoster(units: any[], pool: Staff[], validSet: Set<string>, weekStart: string, deptIdByName: Map<string, string>): Plan {
      ...
      for (const date of days) {
        for (const shift of SHIFTS) {
          for (let k = 0; k < rr.perShift; k++) {
            ...
            cands.sort((a, b) => ((validSet.has(b.id) ? 1 : 0) - (validSet.has(a.id) ? 1 : 0)) || ((shiftCount.get(a.id) ?? 0) - (shiftCount.get(b.id) ?? 0)));
            const pick = cands[0];
  ...
  return {
    assignments,
    scores: {
      coverage: slotsTotal ? Math.round((slotsFilled / slotsTotal) * 100) : null,
      competency: slotsFilled ? Math.round((validatedFilled / slotsFilled) * 100) : null,
      fairness,
