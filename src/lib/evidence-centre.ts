...
// (/educator/evidence). Queue with checklists, prior submissions, activity
// feed, attached evidence, and real turnaround KPIs.

const nowMs = () => Date.now();

export async function buildEvidenceCentre(userId: string): Promise<{
  entries: EvidenceEntry[]; kpis: CentreKpis; isSenior: boolean;
}> {
  ...
  const [{ data: raw }, { data: reviewedToday }, { data: myVerified }] = await Promise.all([
    admin.from("skill_log_entries")
      ...
      .in("status", ["pending", "changes_requested", "escalated"]).neq("nurse_id", userId)
    ...
    admin.from("skill_log_entries").select("created_at, verified_at")
      .eq("verified_by", userId).not("verified_at", "is", null).limit(200),
  ]);
